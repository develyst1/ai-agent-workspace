...
|----|-------|----------|--------|--------------------|
| REQ-001 | Generate a readable dev-work report from a git repository | HIGH | IN_SPEC — **grew 2026-08-20 17:23: Requirements 16 (committer date shown per commit), 17 (report images not fetched — shown as text), 18 (a route to a new report from a finished one)**, each with an acceptance criterion. All three need TASK lines from Sober; none blocks anything in flight | TASK-002 + TASK-003 + TASK-004 + TASK-006 + TASK-007 + **TASK-008 `DONE` (reviewed 2026-08-20 19:20, commit `f00e78d`)**, **TASK-005 `REVIEW` (commit `a092f99`, 2026-08-20) — now the only item in Sober's review queue.** TASK-001 `IN_PROGRESS` (two DoD items, evidence with the human). TASK-009 `TODO`, waiting on TASK-005 **only** now. Ball: **Sober, for the TASK-005 review** — it is the one thing that unblocks anyone. **Neither engineer has a build task**: TASK-009 is joint and still needs TASK-005 `DONE`, and Fern's Requirement 17 / 18 / `KnowCode` lines are still unwritten. |
...
**Next up: TASK-008 is `DONE` (2026-08-20 19:20), so Sober's review queue has
exactly ONE item — TASK-005 — and NEITHER engineer has a build task.**
**The arithmetic:** TASK-005 was Jason's last unblocked TASK, and it is now
submitted. **TASK-009 is the only remaining TASK and it now waits on TASK-005
alone**, sitting in Sober's queue — so reviewing it is what unblocks the project,
and TASK-009 is a joint BE+FE unit that cannot start until TASK-005 is `DONE`.
**Fern still has no build task** and her situation is unchanged: TASK-009 waits,
and the three FE lines she could do next (Requirement 17, Requirement 18, the
`KnowCode` product name) are still unwritten TASK lines in Sober's parked queue.
**Two DoD lines on TASK-005 are deliberately unticked, and they are Sober's
call, not Jason's** — see Q-BE-9 in the Blocked/waiting table: the DoD asks for
a "local disposable Postgres" and the PAT grep of a DB dump, and no such
database exists for the BE role to use. Jason followed the TASK-002 precedent
(repository interface + in-memory implementation) and did not connect to the
human's database.
TASK-009 also still carries **runs 11 and 12**.
...
**Sober's queue is also these five parked units** — stated so they are not
mistaken for a clean slate. They sit **behind** the TASK-005 review above, which
is the only thing unblocking an engineer. One per session, in this
...
**None of the five blocks an engineer, and the arithmetic changed again
2026-08-20:** with TASK-004 `DONE`, **Jason has work (TASK-005) and Fern does
not.** Items in this queue (the `KnowCode` line) and the queued Requirement 17 /
18 lines are the only FE work that exists — so the first of Sober's queued units
to write, once the TASK-005 review is out of the way, is an FE line.
