...
| Write code in YOUR frontend repo, within TASK scope | Touch the backend repo; create/edit any REQ, SPEC, or TASK scope |
...
Your repo (see `board.md` for the path): `code-report-front`. The backend repo
belongs to Jason (BE); if a TASK needs an API that doesn't exist yet, that's a
question for Sober — never build or modify backend code yourself.
