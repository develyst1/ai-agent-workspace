...
| REQ-006 | Backoffice rebuild — universal "item" model on the shared DB | HIGH | **IN_SPEC** (⏸️ deferred by stakeholder — do AFTER the feature backlog) | Jason/Fern — **4/6 DONE** (021/022/023/024). Left: Jason TASK-025 (data migration); Fern TASK-026 (near-trivial). + untracked integration gap: frontoffice `ops-client.ts` still on `ops`, not `bo` (see log 2026-07-26). To resume after feature backlog + analyzing `smart-scheduler-requirement/20260725-{meeting,todo}.md`. |
| REQ-007 | Freelance income-cap visible on the staff calendar (show + keep bookable) | MEDIUM | **READY_FOR_SA** | Sober — write SPEC (hub UC-016). Reverses "hide over-cap" + reconciles backend confirm-block; over-cap = show color strip + still bookable, book past cap only via per-action override (คุณฟีน 2026-07-11). |
| REQ-008 | Bulk-confirm bookings (multi-select) | MEDIUM | **READY_FOR_SA** | Sober — write SPEC (hub UC-027). Tick-select, one LINE per booking, retry-safe, partial-success summary, no batch rollback. Interlocks with REQ-007 override. |
