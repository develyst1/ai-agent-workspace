...
- 🔴 **READ §17 FIRST — the invite code is CUT (owner, 09-02). Entry is the PHONE NUMBER ALONE.** §17 supersedes
  §15 and §16 and everything above them. The 6-digit 2FA is **built but shipped OFF**, one `app_settings` away.
...
## 16. Verified platform fact — an admin's reply is invisible to us (owner's test, 2026-09-01)

The owner replied in LINE OA Manager on a `sid` chat. **No `[line-in]` was logged** — only the customer's own
inbound messages were. ⇒ **an admin's reply is OUTBOUND and never reaches our webhook.**
**This is measured, not assumed.** It settles Sober's §6 caution.

**Consequences:**
- 🔴 **AC-17's automatic trigger cannot be built.** The mute mechanism is built; the admin-reply trigger is not.
- **AC-17 (revised)** — **Given** a chat the bot has muted (parent pressed `คุยกับแอดมิน`, or the two-strikes
  handover fired), **When** the parent types anything, **Then** the bot stays silent — **and it still works
  normally in every other family's chat.**
- **Why this is acceptable:** the bot is **silent by default** and only speaks inside a flow the parent started,
  so the "talking over an admin" window is seconds long, and inside it the parent already has a
  `คุยกับแอดมิน` button on every step plus the two-strikes handover. **The design's own silence does the work.**
- **Fallback if it ever matters:** a **"หยุดบอทในแชทนี้"** control in our own back office — certain, one click,
  no platform capability needed. **Not built now.** Noted so it is not re-derived.

### 🔴 And an unasked-for finding from the same screenshot — AC-16 is a FIX, not a feature

In an **idle** chat the deployed bot answered stray text with errors: `เมนู` → *"เบอร์โทรไม่ถูกต้อง…"* ·
`yo` → *"ไม่พบครูชื่อเล่น \"yo\""* — **while a human was about to reply.** That is exactly what Rule 1 and
**AC-16** forbid.
⇒ **"No reply to unrecognised text in an idle chat" is a CHANGE to deployed behaviour**, not a new capability.
**It needs a regression test**, because the current handlers demonstrably do reply.

---

# 🔴 17. SUPERSEDES §15 AND §16 — the invite code is CUT. **Phone number alone is the entry.** (owner, 2026-09-02)

> *"ไม่เอารหัสเชิญอะไรนั่นเลย ทำไมทำอะไรไม่บอกฉัน · ฉันเอาแค่เบอร์ ก็สามารถใช้งานได้เลย · เรื่องความปลอดภัยลูกค้า
> ยืนยันเองว่าไม่เอาเลข 6 หลักที่เราทำ · ใช่ฉันเข้าใจว่ามันไม่ปลอดภัย แต่เราทำอะไรไม่ได้ ฉันเสนอแล้ว บอกแล้วว่า
> อันตรายแค่ไหน เขาก็ไม่เอา · ปล่อยไปตามนั้น แต่ ทำ function 2FA 6 digit ไว้ด้วยก็ได้ ให้เป็น session เมื่อต้องการ
> เมื่อลูกค้าคิดได้ว่ามันสำคัญ เราก็ค่อยมาเปิด"*

**Where §15/§16 and this disagree, §17 wins.** Earlier sections stay as the record of how we arrived here.

## The entry, in full

> **A parent enters their phone number. The children on it are shown. They are in.**
> **No invite. No code. No TTL. No lockout. No admin step required.**

**Everything the invite brought with it is CUT:** `family_invites`, the code generator, the 8-char alphabet, the
30-minute TTL, single-use redemption, and the attempt counter. ⚠️ **`family_line_links` and its unique index
survive** — one LINE account still belongs to one family, and Sober's reason is untouched by this: without it a
second family's entry silently re-points an account and a parent sees another family's children.

## 🔴 The accepted risk — recorded in full, because this is the one that must be legible later

**Anyone who knows a phone number can see that family's children and act for them** — file leave, check in.
**The owner knows.** He raised it with the customer, **explained how dangerous it is**, and the customer
**refused** both the 6-digit family code and anything in its place. He is choosing to proceed anyway:
*"เราทำอะไรไม่ได้ … ปล่อยไปตามนั้น"*.

📌 **This is a decision, not an oversight, and it was taken with the risk on the table.** Written here so that
whoever meets it later — including a version of this team that has forgotten — reads *"the customer was told and
declined"* rather than *"nobody thought about it"*. **Do not silently re-open it; do not silently harden it.**

⚠️ **What still limits the blast radius, and must NOT be traded away without a new decision:**
- **The code — now the phone — never unlocks anything that moves money.** Seeing children, leave and check-in
  only. That line has held through three mechanism changes and it is the reason this risk stays survivable.
- **A parent still cannot delete a student with any history** (§7b).

## 🆕 The 2FA function — BUILT, SHIPPED OFF, one setting away

His instruction: build it, leave it dormant, turn it on the day the customer decides it matters.

- **A 6-digit verification step, per session, after the phone number.** Off by default.
- **The switch lives in `app_settings`** — the mechanism REQ-031 already provides, and the same shape as the
  weak-code check he asked for on 09-01.
- 🔴 **Build it so turning it on is a SETTING, never a rebuild.** The entry flow must have the verification step
  as a real branch from day one — a stub that would need the flow re-cut later is not what he asked for.
- 📌 **When it is switched on, its parameters come back to the owner** (lifetime, attempts, lockout). They are
  not inherited from the deleted family-code design — **an acceptance does not transfer across a mechanism
  change**, which is precisely the principle Sober applied and which this section is an instance of.

## Consequences to fix elsewhere in this file

- **Flow 1 loses its admin step.** It is now: phone → children shown → in. **No admin opens any door.**
- 📌 **The sick-mother case is SOLVED by this, not worsened.** Dad needs no admin, no code, no invite — he enters
  the family's phone number. The problem that drove three designs disappears with the third one.
- **The customer explainer (§15) is superseded again** — replacement below.

## The customer explainer — replaces §12 and §15
...
1. เริ่มใช้งาน
- ใส่เบอร์โทรที่ให้ไว้กับทางร้าน ระบบจะแสดงชื่อน้องที่มีอยู่
- ใช้งานได้ทันที ไม่ต้องตั้งรหัส ไม่ต้องรอแอดมินเปิดให้

2. คนอื่นในบ้าน หรือเปลี่ยนมือถือใหม่
- คุณพ่อ คุณแม่ หรือผู้ปกครองท่านอื่น ใช้เบอร์เดียวกันได้ทุกคน
- เปลี่ยนมือถือใหม่ก็ใส่เบอร์อีกครั้ง ใช้ได้เลย

3. เพิ่มนักเรียน
- กดเมนู "เพิ่มนักเรียน" กรอกชื่อน้อง วันเกิด จังหวัด
- ระบบสรุปให้ตรวจทานก่อน กดยืนยันแล้วข้อมูลเข้าระบบร้านทันที
- แอดมินจะจัดตารางเรียนให้และติดต่อกลับ
- ถ้ามีน้องชื่อซ้ำ ระบบจะขอชื่อเล่นหรือนามสกุลเพิ่ม เพื่อไม่ให้ลงตารางสลับคน
...
6. เรื่องความปลอดภัย
- ระบบไลน์นี้ใช้เฉพาะเรื่องตารางเรียน ไม่เกี่ยวข้องกับการเงินใด ๆ
- หากภายหลังต้องการเพิ่มการยืนยันตัวตนด้วยรหัส 6 หลัก ระบบรองรับไว้แล้ว เปิดใช้ได้ทันที

7. ใช้บนคอมพิวเตอร์ได้ไหม
- ได้ แต่ไลน์บนคอมจะไม่มีเมนูด้านล่าง
- ทุกขั้นตอนพิมพ์ตัวเลขเลือกแทนการกดปุ่มได้
```

📌 **Point 6 is deliberately honest and deliberately short.** It does not claim a protection that does not exist,
and it tells the customer the 6-digit option is **ready whenever they want it** — which is exactly the owner's
position, and leaves the door open without re-arguing today.
