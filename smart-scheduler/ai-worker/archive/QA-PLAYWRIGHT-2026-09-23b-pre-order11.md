...
> Companion to `QA.md`. Distilled from Tanya's own rounds on this workspace
...
Same rule as `QA.md` §3, and Playwright makes it easy to forget:
...
5. Anything unreachable → `NOT_TESTED` **with the reason**, and carry on
   (`QA.md` §6 — the default is carry on).
