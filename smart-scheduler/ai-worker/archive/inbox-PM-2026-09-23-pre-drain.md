...
---
### 2026-09-21 — @Sober → @Porter — ✅ **TASK-425 IN — every LINE notice names both DUO children. The batch is green again: to `sid` for Tanya's LINE re-capture. ONE owner-visible side effect to clear before `uat`.**
- **BE 2659/0, tsc 0, 49 = 49 · FE 516/0** — re-run by me. The worker, the two payload writers, and **three more one-name sites the census turned up** (the LIFF check-in screen's name, the teacher's ICS feed, the teacher's LINE "my schedule" reply) all go through the one rule now; a scan pin makes a further path fail the suite. Both families read the same `KKTEST & Prao` in the course's order.
- 🟡 **Owner-visible change beyond DUO (say plainly to him):** the LINE notices used the child's FULL name; the one rule is NICKNAME-first (the app's rule everywhere else since TASK-224). So a Private child WITH a nickname now reads by nickname on every LINE notice, the check-in screen, the ICS and the teacher's schedule reply — e.g. `Student : น้องมิว` instead of `Student : เด็กหญิงมิวริน …`. Tanya's KKTEST has no nickname, so her capture is unchanged. **If the owner wants FULL names on family-facing notices, it is one line (the rule's student part takes a `preferFull` flag for the notice paths) — say before `uat`.** I lean nickname-first: it is what the calendar, the reminder and the deduction message already print.
- **Tanya's re-capture (demo OA):** confirm a DUO session ⇒ `Student : KKTEST & Prao` to BOTH families; a leave from either family ⇒ the leave/make-up notice with both names; an admin cancel ⇒ `class_cancelled_parent` with both; the deduction message with both; the LIFF check-in screen and the coach's "my schedule" reply with both. Then the single `uat` deploy (`0042` → `0048`, verify 49).
- Still open: the ledger dump; the disable-frees-link question; the `uat` DUO-group count; the partner-leave notice; the course default not clearable; **the nickname-vs-full-name call above.**

## 2026-09-21 — Tanya (QA) → @Porter: ✅ **GATE PASSED — TASK-425 FIXED the LINE notice both-names. On the demo OA, every family-facing message now reads "KKTEST & Prao" (nickname-first, course order). The Round-55 defect is closed.**

Fresh DUO (kid1 **KKTEST** + coStudent **Prao**), Prao's family LINE-linked, all actions AFTER the TASK-425 deploy. Delivered notices, verified by screenshot:
- ✅ **📅 CONFIRMED SCHEDULE** (confirm) → `Student : KKTEST & Prao`
- ✅ **💡 COURSE DEDUCTION** (check-in/attend) → `Student : KKTEST & Prao … Remaining : 3/4 HR` (both names **and** the one shared deduction, 4→3)
- ✅ **❌ ยกเลิกคาบเรียน / CLASS CANCELLED** (admin cancel + sick-leave) → `Student : KKTEST & Prao … ระบบเพิ่มคาบชดเชยให้แล้ว` (cancel + make-up note, both names)
- ✅ **📅 CONFIRMED SCHEDULE** for the auto-added **make-up** session → `Student : KKTEST & Prao`

So the notice-builder now names both children on **confirm · deduction · cancel · make-up** — the four family-facing surfaces I could drive. 🔑 This directly closes my Round-55 finding (confirm showed only "KKTEST"): now it reads "KKTEST & Prao".

### 🟠 Two surfaces I did NOT independently capture (same builder — very likely fixed, flagging honestly)
- **LIFF check-in screen** — it's a webview mini-app; I confirmed the **deduction** message (which the check-in fires) shows both names, but didn't drive the LIFF screen itself.
- **Coach "my schedule" reply** — needs a teacher LINE link (one demo LINE was on the family this pass). If you want it verified, I'll link a demo teacher and capture it next.
Both use the same student-labeling path that's now fixed on all four surfaces above, so I'd expect them green — but I'm not marking what I didn't see.

### Also this round (already reported): §13.3 per-session rate ✅ PASS · Bookings table ✅ FIXED.

### Footprint
Demo LINE cleared/free; the test DUO course cancelled; Prao's family archived; device authorized. No `uat`, no code.

**Verdict: the LINE-notice both-names gate is PASSED for confirm/deduction/cancel/make-up. Combined with §13.3 rate PASS + Bookings-table PASS, the DUO family-facing text is green. Only the LIFF-screen + coach-reply surfaces are un-captured (same fixed builder) — your call whether that blocks the single uat deploy or I capture them first.**

---
### 2026-09-21 — @Sober → @Porter — **REQ-101 (ECA manage-plan) + REQ-102 (money visibility) — analysis + sizes in `SPEC-088`. No build. The short form:**
**REQ-101 — the customer's question has a true answer: today an ECA cannot take a teacher after creation.** An ECA/Free/KOL schedule is N independent OTHER rows made in one call — **no series object ties them** (a GROUP series has a key; an OTHER series does not). A schedule CAN hold several teachers (the primary + extras, each slot-checked) — but only at creation; the per-row edit changes kind/heads/rates, not teachers. So the admin creates a new series today. **Design:** a series key on the rows (`0049`, the GROUP precedent; a one-off backfill script for existing ECA rows), a Manage-plan page per series with **Confirm all · Cancel all · Add teacher · Remove teacher · Swap teacher · Add dates · edit title/kind/heads**, each teacher change slot-checked in one tx (a clash refuses naming it). Generalises to Free/KOL by construction. Teacher notices on add/remove = two new placeholder messages. **Sizes: BE M+ (`0049` ⇒ 50, one key `other-cancel-all`, two placeholder kinds) · FE M.** Four decisions (§A3): Cancel-all behind its own key; Add-teacher from today or incl. past rows; the backfill; the notice words.
**REQ-102 — there is NO money-visibility key today; every money figure is visible to whoever holds the menu.** Money sits on six surfaces behind four menus: the Teachers page's **freelance budget/remaining** (`freelance_budgets`), the price cards, the discount block, the posted-sale read, the coach rate (§13.3), and the three reports. Only budgets + coach rates are STAFF-PAY figures; the rest are prices/revenue. ⚠ **One leak worth closing regardless:** `GET /teachers` is also the lookup list the calendar loads — a REQ-097 teacher account receives every coach's budget in the payload (hidden only by the FE not drawing it). **Design:** ONE key `action:money.view`, granted to nobody by default (fail closed), applied as ONE mask where DTOs are built — without it every money field is `null` (shape kept, `—` on screen); reports require it on top of their menu; the two money WRITE acts require it too. **Sizes: BE S–M · FE S, no migration.** Decisions (§B3): one key vs two (pay / sales — recommend one); **whether list PRICES count as money** (a front-desk that sells must see prices — recommend exclude prices, mask pay + discounts + posted sales + reports); the Teacher role never holds it.
- Nothing dispatched. Still open: the ledger dump; the disable-frees-link question; the `uat` DUO-group count; the partner-leave notice; the course default not clearable; the nickname-vs-full-name call.

---
### 2026-09-21 — @Sober → @Porter — **REQ-102 (narrowed) cut contract-first: TASK-426 (BE S–M, key 57, no migration) + TASK-427 (FE S). REQ-101 follows once 426 is in (its contract needs the same engineer).**
- As ruled: ONE key `action:teachers.budget-view`, nobody by default, the Teacher role never (a linked account is masked regardless — that closes the `GET /teachers` leak); the mask sits at the one place the teacher DTO is built, so every list that embeds a teacher is covered by construction; the booking rule's booleans (`overLimit`, `setupIncomplete`) survive the mask; the two budget writes need both keys (a user who cannot see the number cannot set it). Nothing else touched — pinned by absence.
- **One question I may bring back:** if a report line prints a teacher's CEILING itself (Jason lists every emitter), the owner said reports are out — I'll ask rather than guess.
- Chain: Jason (426). Still open: the ledger dump; the disable-frees-link question; the `uat` DUO-group count; the partner-leave notice; the course default not clearable; the nickname-vs-full-name call.

---
### 2026-09-21 — @Sober → @Porter — **TASK-426 confirmed (Jason building). Two things settled inside the contract the owner should know:**
1. **The coach's `hourlyRate` (the freelance pay rate on the teacher card) is masked with the same key** — it is more sensitive than the ceiling and the ceiling is derivable from it (`ceiling = hours × rate`). If the owner meant strictly "ceiling and remaining, the rate stays visible", say so — one line.
2. **The dashboard's "near cap" attention line** (`<nickname> · เหลือ N ชม.`) drops the hours number for a user without the key (the item stays so the nag still works); the LINE admin digest — admins only, no viewer — is unchanged.
- Verified on the way: no report prints a ceiling (the reports are untouched, as ruled). The build: one builder that REQUIRES a viewer (a new caller cannot forget it), the two budget writes need both keys, a linked teacher account is masked whatever its role grants — the leak closes by construction. Chain: Jason (426) → Fern (427) → REQ-101's tasks.

---
### 2026-09-21 — @Sober → @Porter — ✅ **TASK-426 (REQ-102 BE) IN — reviewed; Fern wiring TASK-427. TASK-428 (REQ-101 BE, contract first) cut to Jason.**
- **426:** BE 2670/0, tsc 0, no migration; key 57. The mask file names only the four figures (hourly rate · budget · remaining · reorder); the builder REQUIRES a viewer (null ⇒ masked); the two budget writes need both keys; a linked teacher token holding all 57 keys reads nulls — proven through the root app. The near-cap attention label is figureless without the key; the LINE digest untouched. **Deploy note:** after the deploy the super admin grants `teachers.budget-view` to the roles that manage coach pay — nobody has it by default, so the Teachers page shows `—` to everyone else until he does.
- **428 (REQ-101):** cut with the owner's four rulings; the notice bytes = your REQ-101 §4 draft by value. Three things Jason decides in the contract (I'll bring back anything owner-facing): the shape of cancel-all's coach message (one summary per teacher vs N per-row notices); whether the header edit may change the series' TIME (I lean no — move rows individually); the backfill's grouping key (whether two coaches' same-named series must stay separate — I lean yes).
- Chain: Fern (427) · Jason (428 contract). Still open: the ledger dump; the disable-frees-link question; the `uat` DUO-group count; the partner-leave notice; the course default not clearable; the nickname-vs-full-name call.

---
### 2026-09-21 — @Sober → @Porter — ✅ **TASK-427 IN — REQ-102 complete both sides. Jason on TASK-428 (REQ-101 contract).**
- **FE 521/0, tsc 0, build ok · BE 2670/0** — re-run by me. Masked figures draw `—` (zero draws `฿0`); Fern's walk caught three places that would have printed `฿0` for a masked figure and pinned the pattern out of the codebase; a masked freelancer keeps its badge and its not-bookable rule; the budget doors need both keys.
- **Tanya (REQ-102):** the super admin grants `ดูงบ/เพดานค่าจ้างครู` to ONE role; a user without it ⇒ the Teachers page's freelancer cards read `—/hr` and `— / —`, the over-cap freelancer still wears the red badge and stays off the grid, no `Set budget`/`Top up` even with `ตั้ง/เติมงบครู`; the calendar's freelance columns wear a grey `— / —`; the dashboard's near-cap line has no number; a linked teacher account sees the same nulls whatever its role; with the key everything as before and a ฿0 balance reads `฿0`.
- The batch now: the whole earlier list + REQ-102; REQ-101 follows (428 → its FE). Still open: the ledger dump; the disable-frees-link question; the `uat` DUO-group count; the partner-leave notice; the course default not clearable; the nickname-vs-full-name call.

## 2026-09-21 — @Sober → @Porter: REQ-101 BE contract confirmed (TASK-428 building), FE TASK-429 cut. Three items for the owner's list:
1. **📖 Cancel-all coach notice words** — a cancel-all tells each coach ONCE (`other_series_cancelled`: title · reason · the dates), not N per-row messages; that copy is NET-NEW (the owner accepted only ADDED/REMOVED) ⇒ ships as a placeholder; please draft TH+EN in the REQ-101 §4 shape for the owner.
2. **🔴 Finding A (Jason): a plain row MOVE (`PATCH /bookings/:id` — date/time or teacher) tells NO coach today, for lessons as well as Other rows.** Only the course-plan edit sends `teacher_(un)assigned`, and only on a teacher change. REQ-101 "notify on move like a normal course" is therefore met as: silence on a date/time move, old + new coach on a teacher swap. If the owner wants coaches told on a row move, that is a separate small task (all types) — an owner call, not built now.
3. The notice date line follows the customer's `DD-MM-YYYY` on the new kinds too (Jason's draft said `DD/MM/YYYY`; corrected).

## 2026-09-21 — @Sober → @Porter: ✅ REQ-101 BE (TASK-428) DONE — 2690/0, migration `0049` (50 = 50), key 58 `ยกเลิกตารางอื่นๆ ทั้งชุด`, three coach kinds. FE (TASK-429) wiring now. For the owner's deploy list (after `uat`): `db:migrate` → verify **50** → `bun scripts/backfill-other-series.ts` (dry-run, read the groups) → `--apply`; grant key 58 to the role that may wipe a series. The cancel-all notice words are still a placeholder (item 1 of my previous message).

## 2026-09-21 — @Sober → @Porter: ✅ **REQ-101 COMPLETE both sides** (TASK-428 BE 2690/0 · TASK-429 FE 528/0). The batch is now REQ-094…099 + 101 + 102, green on both repos (BE 2690/0, 204 files; FE 528/0, 52 files; migrations 50 = 50, `0042 → 0049` for `uat`; keys 58). **Nothing further in the chain — awaiting the owner's `sid` restart → Tanya's round → the single `uat` deploy.**
**Tanya's REQ-101 checks (Fern's list, TASK-429 §2):** create an ECA ⇒ the toast's `Manage plan` ⇒ the page (title · kind · heads · time · counts · primary + extras); a row opens the ordinary modal; `Confirm all (n pending)`; `Add teacher` B from today ⇒ future rows `A + B`, past rows untouched; `Remove` B; `Swap` to a busy coach ⇒ the `SLOT_TAKEN` sentence, nothing moved; `Add dates` with one already live ⇒ `DATE_EXISTS`; `Cancel all` absent without key 58 even with `calendar.status`, with it ⇒ reason radios, `k attended kept`; a pre-backfill OTHER row shows no `Manage plan` until the script runs; `Series in range` under the calendar header. **LINE (owner only, via you):** the ADDED / REMOVED coach notices with the owner's bytes, one line per date `DD-MM-YYYY`; the cancel-all notice is a placeholder until the owner's words arrive.
**Deploy steps for `uat` (the owner's):** `db:migrate` → verify **50** → `ensure-items` (+8) → `bun scripts/backfill-other-series.ts` dry-run → `--apply` → grants: Teacher role, `budget-view`, key 58 · `camp_reminder_enabled` off.

## 2026-09-22 — Tanya (QA) → @Porter: ✅ **REQ-101 (ECA Manage plan) + REQ-102 (money mask/leak) both PASS on `sid` (verify 50). All API-testable items green, incl. the ⭐ budget leak.**

### ⭐ REQ-102 — money visibility mask — ✅ PASS (incl. the leak)
Tested with a real budget on a throwaway freelance teacher (rate 30000 / budget set):
- **Super-admin** ⇒ figures VISIBLE (`hourlyRate/budgetMinor/remainingMinor/reorderMinor`).
- **Role WITHOUT `teachers.budget-view`** ⇒ all four figures **null** (masked); the booleans (`overLimit`, `setupIncomplete`) **stay**.
- **Role WITH the key** ⇒ figures VISIBLE again.
- ⭐ **THE LEAK — a LINKED teacher account (REQ-097) with a budget-view role ⇒ `GET /teachers` still returns all figures null.** A scoped account is masked *regardless of grants* — the leak is closed. (`฿0` would read `฿0`, not `—`, by the mask rule — figures null only when unset or masked.)

### REQ-101 — ECA Manage plan — ✅ PASS
- **Create** ECA series ⇒ 201; **manage-read** `GET /other-series/:key` returns title `QA-ECA-plan` · kind `ECA` · heads `12` · time `14:00` (+ primary/extras/counts the FE renders).
- **Confirm all** ⇒ 200 `{confirmed:4}`.
- **Add teacher from a date** (`fromDate`) ⇒ future rows become **A+B**, earlier rows untouched (03-08/15 = Ek; 03-22/29 = **Ek+Haris**). **Remove** ⇒ 200 `{removed:2}`, back to A.
- **Swap to a busy coach** ⇒ **409 `SLOT_TAKEN`** naming the date (`2027-03-08 14:00 ครูไม่ว่าง — ไม่ได้ย้ายรายการใด`, nothing moved — rolls back).
- **Add a colliding date** ⇒ **409 `DATE_EXISTS`** (`วันที่ 2027-03-15 มีในตารางชุดนี้แล้ว`).
- **Cancel all — the key-58 gate:** a role with `calendar.status` but NOT `calendar.other-cancel-all` ⇒ **403 FORBIDDEN**; add key 58 ⇒ **200 `{cancelled:4}`**. ✅ the destructive door has its own key.

### 🟠 FE / owner-gated (not API-testable this pass)
`Manage plan` toast on create; a row opening the ordinary modal; `Series in range` under the calendar header; the "k attended kept" count on cancel-all (I confirmed all 4 first, so none were attended to keep); the pre-backfill OTHER rows showing Manage plan (your deploy note = 12 keys on 29 rows applied); the ADDED/REMOVED **coach LINE notices** (owner bytes, demo OA — say go and I'll capture on the demo OA like the DUO notices).

### Footprint
Throwaway freelance teacher archived; the ECA series cancel-all'd; the busy-coach test booking cancelled; all test roles deleted, users disabled; 0 live test bookings swept. No `uat`, no code.

**Verdict: REQ-101 + REQ-102 logic/permissions/leak = PASS. Remaining = FE-visual + the owner-gated coach LINE notices (offer to capture on demo OA).**

## 2026-09-22 — Tanya (QA) → @Porter: 📱 **Coach LINE notices on the demo OA — ADDED ✅ + REMOVED ✅ delivered with the right shape; cancel-all notice NOT sent (pending bytes, as you flagged — not failing it).**

Linked a demo TEACHER (Toth) to the demo LINE, then drove an ECA series (QA-ECA-notice, Ek primary, 3 dates 05/12/19-04-2027):
- ✅ **Add teacher ⇒ ADDED delivered** to Toth's LINE: **📅 เพิ่มตารางสอน** · `รายการ: QA-ECA-notice` · `Time: 14:00-15:00` · one line per date **05-04-2027 / 12-04-2027 / 19-04-2027** (DD-MM-YYYY) · "กรุณาตรวจสอบตารางของคุณ". Correct shape; delivery confirmed.
- ✅ **Remove teacher ⇒ REMOVED delivered**: **❌ นำออกจากตารางสอน** · same `รายการ/Time/วันที่` block. Correct shape; delivery confirmed.
- 🟠 **Cancel all ⇒ NO coach notice arrived.** The `cancel-all` itself worked (200, `cancelled:3`), but **no bubble** reached Toth's demo LINE (chat ends at the re-add ADDED). This matches your note — **the cancel-all notice bytes aren't baked yet** (Sober pending). Per your instruction I'm **noting it, not failing it.** When the final bytes land, ping me and I'll re-capture the `❌ ยกเลิกตารางทั้งชุด / SCHEDULE CANCELLED` notice.

🔑 Delivery + shape confirmed for ADDED/REMOVED (they didn't take a silent wrong-text path — the real bubbles arrived with title/time/dates). The exact wording/bilingual bytes stay your/owner's read.

### Footprint
Toth's demo LINE **unlinked** (free again); the ECA series cancel-all'd; 0 live ECA rows; 0 pending link requests; device authorized. No `uat`, no code.

**Net: coach ADDED + REMOVED notices are GREEN (delivered, correct shape). Cancel-all notice is the only open coach-notice — pending Sober's bytes, not a defect. Once that lands, we're clear for the single uat deploy.**

## 2026-09-22 — @Sober → @Porter: three answers
1. **Cancel-all notice — found, TASK-430 (BE, S) cut.** The notice was wired but gated to CONFIRMED rows (it mirrored the per-row cancel's rule); Tanya's series was still PENDING ⇒ nothing enqueued. My contract gap, not a wrong path. Fix: the notice covers every live row cancelled (ADDED already told the coach about pending dates, so their removal must be told too); the §5 bytes baked, placeholder gone. The per-row cancel keeps its CONFIRMED-only rule.
2. **REQ-102 §6 coach-rate key — size BE S · FE S, fold it in BEFORE `uat`.** Key 59 `action:bookings.coach-rate` (`ดูและแก้ค่าสอน`), view ⇔ edit as one: without it `rate`/`classRateMinor` come null and any body carrying `classRateMinor` is 403; the Move-session box, the course-card line and the DUO create field are ABSENT without it. Independent of `teachers.budget-view` (§7) — pinned both ways. TASK-431 (BE, contract first) + TASK-432 (FE) cut. Timing: the cancel-all fix already forces one more `sid` round; this rides the same round, so it delays `uat` by one BE+FE cycle (today), not by a second deploy later. **Scope note for the owner:** I keep the OTHER/GROUP per-teacher rates (`other.teacherRates`, Stage-1 ECA/Group rates) OUT of key 59 — the customer named courses/DUO/Move-session. Say if she means those too.
3. Tanya's next round (after Jason+Fern): re-capture the cancel-all bubble on a PENDING series (and on a confirmed one); key 59: a role without it sees no rate box on a course session / no default line on the card / no rate field on DUO create, and `PATCH /bookings/:id {classRateMinor}` ⇒ 403; with 59 and not 57: rate visible, budget `—`; the reverse.

## 2026-09-22 — @Sober → @Porter: 📥 **§13.4a (subject TYPE for DUO) noted and QUEUED** behind Jason's current work (TASK-430 fix → TASK-431 contract), per the owner. Size + fold-vs-follow follows when those land; first read: a `subjects.kind`-style column (`0050`), two seeds tagged, one filter on the DUO dropdown — likely S/S, backoffice-clean by being a column not a name rule.

## 2026-09-22 — @Sober → @Porter: ✅ TASK-430 DONE (cancel-all notice fires on every live row, §5 bytes baked — 2691/0). Tanya can re-capture once the human restarts `sid` — on a PENDING series AND a confirmed one. TASK-431 (key 59) confirmed and building; TASK-432 (FE) cut.
**One consequence for the owner (Jason's finding):** a DUO course REQUIRES the coach rate at create, so with "no see-only state" a user WITHOUT key 59 cannot create a DUO course at all (the create door is hidden). If she wants a front-desk role to create DUOs without seeing the rate, the rate becomes optional at create (a one-line follow) — her call; built as-is meanwhile.

## 2026-09-22 — @Sober → @Porter: 📥 **REQ-103 (whole-voucher cancel) noted and QUEUED** behind §13.4a, both behind Jason's TASK-431. Analysis (no build) follows in order; first read: the course's end/drop path (`course_dropped_teacher`, `END_REASONS`) is the precedent — the voucher side is what I'll check.

## 2026-09-22 — @Sober → @Porter: 📥 **The Manage-plan 502 on a backfilled series is noted and put at the FRONT of the queue** (a defect, not a request — it goes before §13.4a and REQ-103). Dispatch on the next nudge: BE first (`GET /other-series/:key` on the backfilled key — most likely a null the header read assumes on rows the creator never wrote), the FE SSR only if the API is 200. Jason's TASK-431 is also in for review.

## 2026-09-22 — @Sober → @Porter: ✅ TASK-431 (key 59) DONE 2700/0; Fern wiring TASK-432. 🔴 **The 502 — TASK-433 cut to Jason** (reproduce on backfill-shaped data; the page fetches client-side, so the API is the first suspect). **📋 DATA REQUEST for the owner (settles which side in one line):** on `sid`, (1) the FE server's log around the 502 (`pm2 logs <front> --lines 100` at the time of the open) and (2) the raw API result: `curl -s -o /dev/null -w "%{http_code} %{time_total}\n" -H "Authorization: Bearer <token>" https://<api>/api/other-series/d2b80ca9-7378-40a4-b988-ee03e95ea703` — the status + seconds. Paste both to me via you. Queue behind: §13.4a → REQ-103.

## 2026-09-22 — @Sober → @Porter: 🔎 **502 — the API is CLEARED** (Jason reproduced both of the page's calls on every backfill-shaped row: 200; 2704/0). I read the FE route too: same client/server shape as the calendar page; nothing in code explains one key 502-ing. **The distinguishing fact is on `sid` — for the owner, in this order:**
1. **One-minute check:** paste the CREATED series' Manage-plan URL (Tanya's `QA-ECA-plan`) into a FRESH tab (a hard load, as the backfilled one was opened). 502 too ⇒ it is the Next server rendering that route on ANY key (then Fern, with the log); 200 ⇒ the backfilled key alone, and the log decides.
2. The FE server's log at the time of the 502 (`pm2 logs <front> --lines 100`) and 3. the curl of `GET /api/other-series/d2b80ca9-…` (status + seconds) — as requested.
Also worth one look: is the `sid` FRONT build the one with the route (`ƒ /scheduler/other/[key]` in its build output)? A stale standalone build behind nginx can answer a new route badly.
Meanwhile Fern wires TASK-432 (key 59). Queue: §13.4a → REQ-103.

## 2026-09-22 — @Sober → @Porter: ✅ REQ-102 §8 folded. **Added size: BE S (TASK-434) · FE S (TASK-432 amended, still in Fern's hands).** The mask grows from two keys to three (`teacherRates`), every rate-carrying writer refuses the field without 59, `duo` alone no longer trips the check, the DUO validator's rate goes optional. No migration, no new key. It holds `uat` by nothing beyond the round already owed (Tanya's re-capture + the 502 fact). Tanya's adds: without 59 — a DUO create succeeds with no rate field; an ECA row/series/group reads no rates; `PATCH …/other {teacherRates}` ⇒ 403; with 59 everything as before. The 502 still waits on the owner's hard-load check + log.

## 2026-09-22 — @Sober → @Porter: ✅ TASK-434 DONE (2711/0) — the BE side of the held batch is complete (REQ-094…099, 101, 102 incl. §6–§8; migrations 50 = 50; keys 59; kinds 23). Fern finishes TASK-432 (the last FE piece). Still open on the owner: the 502 hard-load check + log; Tanya's re-capture round after `sid` restarts with the whole batch.

## 2026-09-22 — @Sober → @Porter: ✅ **THE HELD BATCH IS COMPLETE BOTH SIDES.** BE **2711/0** (migrations **50 = 50**, `0042 → 0049` for `uat`; 59 keys; 23 kinds) · FE **533/0** (snapshot 59). Contents: REQ-094…099, 101 (+ cancel-all notice fix), 102 (§4 narrowed + §6/§7/§8 coach-rate key). Nothing further in the chain.
**Waiting on the owner:** (1) `sid` restart with this build; (2) the 502 hard-load check + FE server log + curl (TASK-433 — a stale front build is my first guess; this build includes the route); (3) Tanya's round: cancel-all re-capture on a PENDING + a confirmed series; key 59 checks (Fern's list in TASK-432 §2); the Manage-plan PAGE on a backfilled series; REQ-102 57⇄59 independence.
**Then the single `uat` deploy:** `db:migrate` → verify **50** → `ensure-items` (+8) → `bun scripts/backfill-other-series.ts` dry-run → `--apply` → grants: Teacher role · `budget-view` (57) · `other-cancel-all` (58) · `coach-rate` (59) · `camp_reminder_enabled` off.
Queue after: §13.4a sizing → REQ-103 analysis (I start §13.4a on the next nudge unless the 502 fact comes first).

## 2026-09-22 — @Sober → @Porter: ⚠️ You are right — my "batch complete" was written before I read your three messages (the modal re-spec, the owner gate, the regression); I read only your last block that turn. Corrected now; the batch is NOT ready.
1. **Modal re-spec — confirmed, building: TASK-435 (Fern, S–M).** The page, `seriesHref` and the `ROUTE_ALIASES` entry are retired; one modal from two entry points; no API change. **The 502 chase is withdrawn** — TASK-433 stays closed as a read (the API was cleared, +4 pins kept); the DATA REQUEST for the hard-load check/log is CANCELLED — the owner need not touch it.
2. **The moved-out notice regression — TASK-436 (Jason, debug first, S).** What I read myself: the only producer of `teacher_unassigned` (`applyPlanChange`, the plan editor's teacher change) is intact in code and git shows NO enqueue removed since 09-19; the Move-session popup (`PATCH /bookings/:id`) has NEVER sent it (Jason's Finding A). So the suspects in order: (a) delivery — TASK-425's notice-builder fold makes the worker's render throw for this kind ⇒ the outbox row fails and the coach hears nothing (my prime suspect, and it would be a hotfix); (b) the notice-days gate; (c) the door — the customer now reassigns from the Move-session popup rather than the plan editor (then it is Finding A, fixed by giving `moveBooking` the same old/new coach pair — rides the batch). **📋 DATA REQUEST (owner, `uat`, read-only) that settles (a) in one line:** `SELECT id, created_at, status, error, payload->>'kind' FROM line_outbox WHERE booking_id = '<the 27-09-2026 11:00 Private SURFSKATE booking id>' ORDER BY created_at;` — plus: which screen did Khwan use for that reassignment (the course plan editor or the session's Move popup)? Hotfix-vs-batch recommendation follows the cause.
3. **The owner gate — noted:** ONE batch = REQ-101 (modal + cancel-all + notices + add-teacher) · REQ-102 (+ key 59 §6–§8) · REQ-095 §13.4/§13.4a · REQ-103 · (pending) REQ-077/REQ-051. I start §13.4a sizing and the REQ-103 analysis as soon as 435/436 are dispatched (they are) — next nudge.

## 2026-09-22 — @Sober → @Porter: ✅ **Regression cause found and fixed (TASK-436, 2718/0).** Not a broken notice: delivery and the gate are clean by value (the worker renders the customer's exact row byte-equal to her old screenshot). **The cause is the DOOR** — the Move-session popup (`PATCH /bookings/:id`) has never told the coach; only the course plan editor did. Since 09-21 the popup carries the rate box (TASK-424) and became the everyday way to reassign, so the notice "stopped". Fix: ONE `sendTeacherReassigned` pair (old ⇒ moved-out, new ⇒ assigned) from BOTH doors; a date/time-only move stays silent (§5). **The DATA REQUEST for `line_outbox` rows is withdrawn** — they would show absence, not failure.
**Hotfix vs batch — my recommendation: HOTFIX.** Real coaches are missing reassignments today; the change is one file + its test, no migration/key/FE, and applies on `uat`'s deployed base. It cuts across the owner's "no piecemeal" gate, so it is his call: (a) hotfix now = the human cherry-picks that one change onto the `uat` base (git is his; Jason's change is uncommitted in the working tree alongside TASK-434 — he needs to separate it), or (b) it rides the batch. Please ask.
**Owner's list (new):** the popup has never enforced the `teacher_change_notice_days` rule the plan editor has (a reassignment 1 day out passes from the popup, 409s from the plan editor) — fold the same gate into the popup, or leave it? Not built.
Next from me: §13.4a sizing + REQ-103 analysis (next nudge). Fern on the modal (TASK-435).

## 2026-09-22 — @Sober → @Porter: ✅ A (no hotfix, rides the batch) and B (the popup stays un-gated) noted; TASK-436 stays as built. **§13.4a + REQ-103 analysed — `specs/SPEC-089-subject-type-and-whole-voucher-cancel.md`.**
- **§13.4a — BE S · FE S, one migration (`0050`).** Finding: no name-prefix filter exists in either repo today — the DUO dropdown lists every subject. Design: `subjects.kind` (`PRIVATE` default | `DUO`, a closed set) separate from `price_group` (what it costs vs what it is — backoffice-clean, one column their editor sets); a seed script for the two DUO subjects; the FE filters by `kind`; the server refuses a DUO course on a non-DUO subject. **Owner:** (1) hide DUO subjects from the Private dropdown too? (rec. yes) (2) server refusal? (rec. yes).
- **REQ-103 — BE S–M · FE S, one migration (`0051`).** Finding: a voucher has no lifecycle at all today (no status/end columns, no cancel route; state derived from `used_hours` + expiry; hours consumed only at day-end, so future draws have consumed nothing). Design = the whole-course cancel shape on the voucher: `ended_at/by/reason` (the same closed `END_REASONS`), `POST /vouchers/:id/cancel` + `/preview`, every FUTURE live draw ⇒ CANCELLED, past draws and `used_hours` untouched, the remaining balance FROZEN (readable, for their conversion by hand), nothing may draw from an ENDED voucher, one course-end-style coach notice per coach. **Owner:** (1) reuse key `bookings.course-cancel` (rec.) or a 60th key; (2) coach notice = the course-end message with a voucher line (rec., zero new bytes) or new words; (3) family silent as the course end (rec.) or a notice; (4) the card reads `ENDED · 7h left` (rec.).
Both fold into the gate as one migration each (50 → 52). Contract-first tasks cut on the rulings. Fern still on TASK-435 (modal).

## 2026-09-22 — @Sober → @Porter: ✅ SPEC-089 rulings received; **four tasks cut** — TASK-437 (BE `subjects.kind`, `0050`) → TASK-438 (FE filter) · TASK-439 (BE whole-voucher cancel, `0051`) → TASK-440 (FE door + chip). Contract-first on both BE tasks; Fern finishes the modal (435) first. One note for the owner's deploy list: after `db:migrate` (verify **52**) also `bun scripts/ensure-subjects.ts` (dry-run → `--apply`) to tag the two DUO subjects. I ping you when 435/437–440 land green.

## 2026-09-22 — @Sober → @Porter: TASK-437 contract confirmed (building). Two facts for the owner: (1) the frontoffice has no subject editor — the two DUO subjects get `kind = DUO` by the seed script (`ensure-subjects`, an upsert; the exact names `Duo INLINE SKATE` / `Duo SURFSKATE` are assumed — **please confirm the bytes as they are on `uat`**); a future third DUO program is added with `subjects:add --kind DUO` until the backoffice owns it. (2) The rule is about COURSES: a walk-in single hour or a trial on a DUO subject stays sellable (the DUO card's 1-hour price, TASK-399) — say if the owner meant DUO subjects are courses only.

## 2026-09-22 — @Sober → @Porter: ✅ TASK-437 DONE (2729/0, `0050`, 51 = 51). TASK-439 contract confirmed, building (`0051` ⇒ 52). Two notes for the owner: (1) the coach's voucher-end notice reads `<subject> 10 HR` on its Program line with ZERO new bytes (the course-end title; a 10-hour entitlement ended) — if she wants the word "voucher" there, that is one new phrase (📖), say so; (2) Jason found the whole-COURSE cancel never released the cancelled sessions' slot holds (a pre-existing gap) — fixed in this task as its own line; Tanya's check: after a whole-course cancel, the future slots are bookable again.

## 2026-09-22 — @Sober → @Porter: ✅ **BE side of the locked batch COMPLETE** — TASK-439 DONE (2749/0, `0051`, **52 = 52**). Everything BE-side is in: REQ-101 (series + cancel-all notice + reassign notice), REQ-102 (§4–§8), §13.4a, REQ-103. Fern has three FE tasks left (435 modal → 438 DUO filter → 440 voucher cancel); I ping you when all three land green. Deploy list for `uat` (final): `db:migrate` → verify **52** → `ensure-items` → `ensure-subjects` (dry-run → `--apply`) → `backfill-other-series` (dry-run → `--apply`) → grants 57/58/59 + Teacher role → `camp_reminder_enabled` off. One small owner note: the voucher cancel's optional note is carried on the coach notice, not stored on the voucher (the reason code is stored) — say if she wants it kept.

## 2026-09-22 — @Sober → @Porter: ✅ **THE LOCKED BATCH IS COMPLETE BOTH SIDES — re-run green by me on the final trees.** BE **2749/0** (migrations **52 = 52**, `0042 → 0051` for `uat`; 59 keys; 23 kinds) · FE **543/0** (snapshot 59; the `/scheduler/other/[key]` route GONE — the 502 cannot recur). Contents: REQ-101 (series key · Manage-plan MODAL · confirm/cancel-all · add/remove/swap · notices incl. the cancel-all fix · the reassign notice from both doors) · REQ-102 (§4 budget key 57 · §6–§8 coach-rate key 59) · §13.4a (`subjects.kind`) · REQ-103 (whole-voucher cancel). **Ready for `sid`.**
**Tanya's big round — the checklists are in each TASK's §2 (Fern) / report (Jason); the headlines:** the Manage-plan MODAL from both entry points on a created AND a backfilled series (the old 502 case); cancel-all bubble on a PENDING series + a confirmed one; a reassignment from the Move popup ⇒ old coach's 📤 + new coach's notice; key 57/58/59 with/without; a DUO create without key 59 (no rate field, 201); the DUO dropdown by kind after `ensure-subjects`; a whole-voucher cancel (preview → future draws gone, past kept, `Ended · Nh left`, a draw refused); a whole-COURSE cancel now frees its future slots (Jason's holds fix). **LINE (owner):** ADDED/REMOVED/CANCELLED coach notices; the reassignment pair.
**Deploy for `uat` (the owner's, after Tanya):** `db:migrate` → verify **52** → `ensure-items` → `ensure-subjects` (dry-run → `--apply`; confirm the two names first) → `backfill-other-series` (dry-run → `--apply`) → grants 57/58/59 + Teacher role → `camp_reminder_enabled` off.
Open owner notes on my side (none blocking): the "voucher" word on the voucher-end notice; the voucher note not persisted; Porter's Thai word for voucher (บัตรชั่วโมง vs วอยเชอร์) on the new FE copy.

## 2026-09-22 — Tanya (QA) → @Porter: ✅ **Big round — ALL API items PASS (§13.4a · key 59 · REQ-103 · course-slot-free). 🔴 The demo-OA LINE captures (cancel-all + reassignment) are BLOCKED — the phone screen locked; needs the owner to unlock + reopen the demo OA.**

### ✅ §13.4a — DUO ⇔ DUO-subject pin
- DUO course on a **non-DUO** subject ⇒ **400 `NOT_A_DUO_SUBJECT`** ("คอร์ส DUO ต้องใช้โปรแกรม DUO").
- DUO course on a **DUO** subject (`Duo INLINE SKATE`/`Duo SURFSKATE`) ⇒ **201, kind DUO**.
- **Private** course on a DUO subject ⇒ **400 `DUO_SUBJECT`** ("โปรแกรม DUO ขายได้เฉพาะคอร์ส DUO"). Both directions pinned.

### ✅ key 59 (`bookings.coach-rate`) — VIEW⇔EDIT coupled (isolated with an all-keys-except-59 role)
- **Without 59:** booking `rate` DTO ⇒ **null** (masked); `PATCH {classRateMinor}` ⇒ **403**; a **DUO create with NO rate ⇒ 201** (duo alone isn't an edit); a **DUO create WITH a rate ⇒ 403 FORBIDDEN**.
- **With 59:** `rate` ⇒ `{effectiveMinor:50000, overrideMinor:null, defaultMinor:50000}` (visible); `PATCH {classRateMinor}` ⇒ **200**.
- (Mask also nulls `teacherRates` by the same rule — ECA/Group rates masked without 59.)

### ✅ REQ-103 — whole-voucher cancel
Preview ⇒ `removedSessions:1` (future confirmed draw); **cancel ⇒ 200**, the future draw **CANCELLED**, **used hours KEPT** (used 1), **balance FROZEN** (remaining 4), voucher **ENDED**; a **new draw on the ENDED voucher ⇒ 409 `VOUCHER_ENDED`** ("วอยเชอร์นี้ถูกยกเลิกแล้ว"). (A one-off 500 on confirming a future voucher session did NOT reproduce — 200 on retry, not a finding. The card "Ended · Nh left" label is FE.)

### ✅ Whole-COURSE cancel now FREES the future slots
Confirmed a course session (slot held) → cancel the whole course → **re-booked another session at the exact same teacher/date/time ⇒ 201** (slot freed). Jason's holds-fix works.

### ✅ Already green (Round 57): key 57 budget mask + leak · key 58 cancel-all gate · Manage-plan read 200 (no 502).

### 🔴 BLOCKED — demo-OA LINE captures (cancel-all bubble + reassignment pair)
The demo phone **screen locked** (timed out); waking it showed the **lock screen with personal notifications — NOT the demo OA**, which I discarded without reading. I won't enter a PIN or swipe into the phone. 👉 **Owner: unlock `CPH2735` and reopen the SOM-Balance-Demo LINE chat**, then I'll capture: the **cancel-all bubble** (`❌ ยกเลิกตารางทั้งชุด`) on a PENDING and a confirmed series (the Round-58 gap), and the **reassignment pair** (OLD coach `📤` moved-out + NEW coach assigned from a Move-session teacher change; a date/time-only move stays silent). (ADDED/REMOVED were already captured green in Round 58.)

### Footprint
0 live test bookings; test teachers archived, roles deleted, users disabled, cancelled vouchers/courses remain (not deletable). No `uat`, no code.

**Verdict: the whole batch is API-PASS. Only the demo-OA LINE captures remain, blocked on the phone being unlocked — ping me once it's open.**

## 2026-09-22 — Tanya (QA) → @Porter: 🟠 **Demo-OA retry — the phone unlocked (thanks) and the demo OA is up, but the LINE PUSH notices aren't rendering (DND is ON). Bot REPLIES to my typed input arrive instantly; the approve-notice + cancel-all bubble do NOT appear. API cancel-all works. I can't clear this without a forbidden action.**

### What happened
- Re-linked Toth (register→teacher→Toth on the demo OA; the bot replied promptly each step and "link request sent ✅"). Approved via API (200, linked). Created + confirmed an ECA (Toth primary), then **cancel-all ⇒ 200 `{cancelled:3}`** (API side works).
- 🔴 **On the phone: no bubbles rendered** for the **link-approved** notice NOR the **cancel-all** notice — the chat's last message stays "link request sent" through ~1 min of waiting/scrolling.
- 🔑 The tell: the bot's **replies to my typed messages** (Next-prompt, nickname-prompt, link-sent) appear **instantly**, but **server-initiated PUSH** notices don't. In Round 58 the same approve-notice + ADDED/REMOVED pushes DID render — so this is a **phone-side push/DND issue right now** (the moon 🌙 "โหมดห้ามรบกวน" is active in the status bar), not a backend gap.
- I did **not** toggle DND (a phone setting) or exit to the chat list (would show personal chats) — both are off-limits for me.

### So the two LINE captures are still OPEN, blocked on push delivery:
- **cancel-all bubble** (`❌ ยกเลิกตารางทั้งชุด`) — API fired; the on-phone bubble didn't render.
- **reassignment pair** (📤 moved-out + assigned) — not attempted, same push path would be blocked.

👉 **Owner: on `CPH2735`, turn OFF Do-Not-Disturb (or confirm the SOM-Balance-Demo pushes are flowing), then ping me** — I'll re-fire the cancel-all + do the reassignment and capture both. (ADDED/REMOVED are already green from Round 58; all REQ-101/102/103/§13.4a API items are PASS from earlier this round.)

### Footprint
Toth unlinked (demo LINE free); the ECA cancel-all'd; 0 live; 0 pending link requests. No `uat`, no code.

## 2026-09-22 — Tanya (QA) → @Porter: 🔴 **CEILING: the demo phone won't render server-PUSH LINE notices (3 tries, DND off). Bot REPLIES to typed input work instantly; the approve-notice + cancel-all bubble never appear. cancel-all API works (200, cancelled:3). The visual confirm of cancel-all + reassignment is the owner's on-phone check now — I can't clear this without a forbidden phone-setting change.**

### What I did (phone unlocked + on the demo OA, thanks)
- Re-linked Toth cleanly (register→teacher→Toth; bot replied to each typed step, "link request sent ✅" at 09:27). Approved via API (200, **linked**). Confirmed an ECA, **cancel-all ⇒ 200 `{cancelled:3}`**.
- 🔴 On the phone across **~2 min + DND off**: **no bubble** for the **link-approved** notice NOR the **cancel-all** — the chat's last line stays "link request sent". Same as the previous attempt.

### The diagnosis (consistent, 3 attempts)
- **Bot REPLIES** (answers to my typed `register`/`teacher`/`Toth`) render **instantly** ⇒ LINE connectivity is fine.
- **Server-initiated PUSH** notices (link-approved, cancel-all) **do not render** ⇒ the push path to this demo OA/device is blocked or throttled right now. (In Round 58 the same pushes DID render — so it's an environment/state issue, not a code gap.) Likely LINE per-OA push throttle after many test rounds, or a LINE-app notification block for this OA — either way a **phone/LINE-settings** thing I may not touch.

### So these two stay OWNER-confirmed on the phone (his LINE-text rule applies anyway):
- **cancel-all bubble** (`❌ ยกเลิกตารางทั้งชุด`) — API fired (200, cancelled:3); bytes baked (Sober). 👉 **Owner: glance at the demo OA — did a cancel-all notice arrive?** (I fired one at ~09:27.)
- **reassignment pair** (📤 moved-out + assigned) — not capturable while pushes don't render.

### What's already SOLID
ADDED/REMOVED coach notices ✅ (Round 58, real bubbles). All REQ-101/102(keys 57/58/59)/103/§13.4a/course-slot-free API items ✅ this round. The whole batch is API-PASS; only the on-phone cancel-all + reassignment *visual* remain, and those sit with the owner given the push-render ceiling.

### Footprint
Toth unlinked (demo LINE free); ECA cancel-all'd; 0 live; 0 pending link requests. No `uat`, no code.

**My rec: don't hold uat on the on-phone cancel-all/reassignment visuals — the API + bytes are in, ADDED/REMOVED proved the builder, and the owner can eyeball the cancel-all bubble on the demo phone in one look. If you want me to retry, it needs the LINE push path to this OA flowing again (owner's side).**

## 2026-09-22 — @Sober → @Porter: 🔬 **REQ-104 gap-check — `specs/SPEC-090-req104-gap-check-group-camp.md`. The true picture:**
1. **GROUP — partly done.** The standing schedule ✅ (the GROUP series, weekly rows, coach swap from a date). "Confirm whole course" ✅ per CHILD (`POST /courses/:id/confirm` on a course sold into the group — the GROUP row itself is born CONFIRMED, nothing to confirm). ❌ "Weekly confirm for the coach" — no weekly coach message exists for anything (the coach gets the DAILY 08:15 reminder + per-row confirms); a Monday weekly digest = BE S + owner words. ❌ A GROUP manage surface (cancel-all incl. every child's seat with each family told · add/remove extra coach · add dates · edit name/cap/rates) — the REQ-101 modal generalised to `group_key`: BE S–M · FE S.
2. **CAMP coach rate per day — ❌ entirely a gap:** no camp rate exists anywhere today (not per week, not per day; the derived camp rows carry none). Design: a per-coach-per-day rate on the week's day editor, behind key 59, copied onto the derived rows; BE S–M (one migration) · FE S.
3. **CAMP check-in — ❌ both halves:** the scan consumes the credit but shows nothing and tells nobody. Note Private's "แจ้งแม่" is NOT at the scan either — it is the DAY-END deduction message (`Remaining 14/15`); camp has no deduction message at all. Gaps: the scan page shows the remaining credit (BE S · FE S — Private gets `Remaining n/N` there for free), and a `CAMP DEDUCTION` family notice at day-end with the owner's words (BE S–M).
4. **CAMP redeem — ✅ done:** the picker lists only packages with `credit > 0`; the server refuses beyond the credit.
**Next-slice size if all taken: BE M+ · FE M, one migration, two new notice kinds (owner words).** Owner decisions in the SPEC (group confirm per child vs one button; the weekly digest; cancel-all cascades to seats + families; camp rate default for a new day; deduction at day-end (parity, rec.) vs at scan; the words). The `sid` batch is unaffected.
...
## 2026-09-22 — @Sober → @Porter: TASK-441 contract confirmed (building). **Deploy list gains one line (the human's):** the weekly coach digest is a Windows Task Scheduler job like the others — register `scripts/weekly-teacher-digest.ts` (via the `package.json` script) for **Monday 08:15** on the box; the job writes `job_runs` so a missed registration shows. No on/off setting for it (the words are the owner's). Rows in the digest = CONFIRMED classes only (what the coach has been told is his), a GROUP row one line with the group's name.

## 2026-09-22 — @Sober → @Porter: TASK-441 DONE (2766/0); TASK-443 contract confirmed (building, `0052` ⇒ 53). One fact for the owner from Jason's read, no action needed unless she objects: Private's `COURSE DEDUCTION` actually fires at the moment of attendance (scan or manual attend) AND at the 17:30 cut for never-attended rows — so "parity, at day-end not at scan" for camp is a small difference from Private: the camp `CAMP CREDIT USED` message arrives at 17:30 for every consumed day (scanned or no-show), never at the scan. Built as she ruled (day-end). A no-show camp day is charged today (§8) and therefore also gets the message.

## 2026-09-22 — @Sober → @Porter: ✅ **BE side of the WHOLE batch complete** — TASK-443 DONE (2776/0, `0052`, **53 = 53**, 25 kinds). Everything BE-side for REQ-094…104 is in. Fern has 442 (group modal face) + 444 (camp rate box + `Remaining` line) left; I ping when both land green. **Final `uat` deploy list (the human's, after `sid` + Khwan's customer-UAT):** `db:migrate` → verify **53** → `ensure-items` → `ensure-subjects` (dry-run → `--apply`) → `backfill-other-series` (dry-run → `--apply`) → register the Task Scheduler job `weekly-teacher-digest` Monday 08:15 → grants 57/58/59 + Teacher role → `camp_reminder_enabled` off.

## 2026-09-22 — @Sober → @Porter: ✅ **THE WHOLE BATCH (REQ-094…104) IS COMPLETE BOTH SIDES — re-run green by me on the final trees.** BE **2776/0** (migrations **53 = 53**, `0042 → 0052` for `uat`; 59 keys; 25 outbox kinds) · FE **555/0** (snapshot 59). **Ready for ONE `sid` deploy → Tanya → Khwan's customer-UAT → the single `uat` deploy.**
**Tanya's headlines for the REQ-104 slice (details in TASK-441/443 reports and TASK-442/444 §2):** the series modal from a GROUP row (seats per row, greyed cancelled); `Confirm whole group` ⇒ one `course_confirmed` per child's course; group `Cancel all` (key 58) ⇒ every seat cancelled, each family's `class_cancelled_parent`, the coach once, the toast's `familiesTold`; the weekly digest — run `bun scripts/weekly-teacher-digest.ts` on `sid` once ⇒ one English-only message per coach with CONFIRMED rows Mon–Sun, a second run sends nothing; the camp day editor's `Rate per coach (฿)` column with key 59 only, the derived calendar rows carry it under the same mask, `PATCH` with `teacherRates` and no key ⇒ 403; a camp scan ⇒ `Remaining : 3.5/10 days`, a Private scan ⇒ `Remaining : 7/10 sessions`, a trial ⇒ no line; the 17:30 cut ⇒ `🏕️ CAMP CREDIT USED` to each family account for every consumed camp day (scanned or no-show), never twice. **LINE (owner):** the weekly digest and the camp deduction bubbles — English only under both languages.
**Deploy list (final, the human's):** `db:migrate` → verify **53** → `ensure-items` → `ensure-subjects` (dry-run → `--apply`; the two names confirmed?) → `backfill-other-series` (dry-run → `--apply`) → register Task Scheduler `weekly-teacher-digest` Monday 08:15 → grants 57/58/59 + Teacher role → `camp_reminder_enabled` off.

## 2026-09-22 — Tanya (QA) → @Porter: 🟡 **REQ-104 gap — GROUP confirm/cancel-all ✅, CAMP key-59 rate mask ✅. 🔴 A finding: camp-week-create INCLUDING TODAY ⇒ 500 (future week ⇒ 201). Scan-remaining + the job/LINE items are blocked (that 500 + the push ceiling).**

### ✅ GROUP (TASK-441/442) — API PASS
- **Confirm whole group** ⇒ `POST /group-series/:key/confirm-all` = **200 `{confirmed:6}`**; the child course's 6 sessions all CONFIRMED (one `course_confirmed` per child).
- **Group Cancel all (key 58)** ⇒ **200 `{cancelled:6, seatsCancelled:6, familiesTold:6}`** — seats cancelled + the toast's `familiesTold` present.
  - 🔎 Note: `familiesTold:6` with only ONE family in the group (my 2nd-student setup failed) — so it looks like it counts **per-seat**, not per distinct family. Please have Sober confirm the intended semantic (6 seats × 1 family vs. distinct families).

### ✅ CAMP day rate — key 59 mask (TASK-443) — PASS
- Super-admin `PATCH /camp/weeks/:id/days/:date {teacherRates}` ⇒ **200**.
- Role WITHOUT key 59 ⇒ the day's `teacherRates` reads **null** (masked), and `PATCH {teacherRates}` ⇒ **403**. The mask + write-check both hold on the camp surface.

### 🔴 FINDING — camp week create INCLUDING today ⇒ 500 INTERNAL
- A **future** Mon–Fri camp week ⇒ **201** (fine).
- A week whose range **includes today** (today-only AND today..tomorrow) ⇒ **500 `{code:INTERNAL}`**.
- Likely an unhandled **clash** — the derived camp block (10:00–15:00) collides with EK's existing near-term sessions and the create **throws 500 instead of a clean 409** (§11 clashes refuse cleanly; this path doesn't). Could be aggravated by my leftover today test bookings for EK, but a **500 on create is still wrong** — it should refuse gracefully. Please have Sober/Jason check the camp-week-create clash path.

### 🟠 Blocked / at the ceiling (not failures)
- **Camp scan `Remaining : X/10 days`** — needs a **today** camp day; the today-week create 500s (above), so I couldn't stage it. (The scan is correctly **day-restricted**: a future-day scan ⇒ 409 `CAMP_DAY_NOT_TODAY`.)
- **Weekly digest** (`weekly-teacher-digest.ts`), **17:30 `🏕️ CAMP CREDIT USED`**, and every **family/coach LINE delivery** — job-runs + the LINE **push-render ceiling** I hit earlier today (server pushes don't render on the demo phone right now). These are the owner's on-phone / a sanctioned job-run.

### Footprint
Group bookings swept (13); camp weeks left CLOSED (test data); roles deleted, users disabled. No `uat`, no code.

**Net: the GROUP + CAMP-rate-key API gaps PASS. One real finding (camp-week-create 500 on today). The scan-remaining + all LINE/job items are blocked on that 500 or the push ceiling — I'd fix/verify the 500 before inviting Khwan, since a customer creating "this week's" camp would hit it.**

## 2026-09-22 — @Sober → @Porter: both from Tanya's gap round ⇒ **TASK-445 (Jason, S)**.
1. **The 500** — the camp sync catches only its own `SLOT_TAKEN`; on a today-week the clash reaches it as something else (Jason finds which — a raw DB unique-index error or past dates in the week's range are my reads) ⇒ fixed to a 409 naming date · hour · coach, nothing written.
2. **`familiesTold` — answered:** it counted ROW × household, and Tanya's 6 = one child on six rows ⇒ six dated "class cancelled" bubbles to one family (each a true fact; the family message has no series form). Two changes: siblings on the SAME row were told twice — now once per LINE account per row (a real dup, fixed); the response becomes `familyNotices` (bubbles) + `householdsTold` (distinct families) so the toast is honest. **Owner note (not built):** if she wants ONE summary bubble per family for a whole-group cancel (like the coach's), that is a new family notice with new words — her call for a later slice.
Both green → `sid` → Khwan.

## 2026-09-22 — @Sober → @Porter: ✅ TASK-445 DONE (2783/0). **The 500's real cause:** not a "today" rule — on ANY coach clash the camp create built its 409 correctly, then read the coach's name on the already-aborted transaction ⇒ a raw DB error ⇒ 500. Fixed (names read before the loop) and recorded as a workspace rule. The family de-dupe + the honest counts are in. Fern's one-line toast follow is the last piece; then `sid` → Khwan. Tanya's re-check: a today-week over a busy coach ⇒ 409 naming date · hour · coach, no week created; siblings on one group row ⇒ one bubble per family; six rows one child ⇒ `householdsTold: 1`.

## 2026-09-22 — @Sober → @Porter: ✅ **Tanya's two items closed; the batch is whole again — BE 2783/0 (53 = 53) · FE 555/0.** Ready for `sid` → Tanya's re-check (a today-week over a busy coach ⇒ 409; siblings once per row; `householdsTold: 1` for one child on six rows; the toast wording) → Khwan's customer-UAT → the single `uat` deploy (the final list in my message above).

## 2026-09-22 — Tanya (QA) → @Porter: ✅ **TASK-445 re-check ALL PASS — the camp 500 is a clean 409, the cancel-all de-dupes to households, and the camp scan works. sid is CLEAR for Khwan's customer-UAT (LINE/job visuals aside — they sit on the push-ceiling per your note).**

1. ✅ **Camp-week over a busy coach ⇒ 409, no week created** (was 500). A today-week with HARIS (who had an 11:00 session) ⇒ **409 `SLOT_TAKEN` "วันที่ 2026-09-22 11:00 ครูHaris มีคาบแล้ว — ไม่ได้บันทึกอะไร"** (names date · hour · coach, nothing saved). And a today-week with a **free** coach ⇒ **201** (the create path is healthy again).
2. ✅ **Group cancel-all de-dupe** — one child on six rows ⇒ **`householdsTold: 1`** (distinct family), with `familyNotices: 6` as the honest raw count (`cancelled:6, seatsCancelled:6`). My earlier "familiesTold:6" is resolved — the toast reads the **distinct-households** number now. (Siblings-on-one-row = 1 household by the same rule; the de-dupe mechanism is proven on the one-child/six-rows case.)
3. ✅ **Camp scan `Remaining : X/10 days`** — a today camp day scan ⇒ **200 `{status:ATTENDED, credit:{remainingDays:9, totalDays:10}}`** = "Remaining : 9/10 days"; a **2nd scan ⇒ `{already:true}`** (already-checked). A FULL day drew 1 of 10.

### Still on the push-ceiling (your note: not my gate)
Weekly digest, the 17:30 `🏕️ CAMP CREDIT USED`, and the cancel/reassignment LINE **bubbles** — the demo-OA server-push path hasn't been rendering all afternoon (4 tries; bot replies work, pushes don't; likely a LINE per-OA throttle after today's many rounds). ADDED/REMOVED are green (R58); the APIs + bytes are in. Owner eyeballs the demo phone / a sanctioned job-run.

### Footprint
Camp weeks CLOSED, test teachers archived, group series cancel-all'd, 0 live. No `uat`, no code.

**Verdict: TASK-445 fix confirmed — the camp-500 finding is fixed, cancel-all counts households, camp scan works. From the API/logic side, `sid` is CLEAN for Khwan's customer-UAT.**
