...
## 2026-09-19 — Sober → @Jason: 📋 **Two tasks, CONTRACT FIRST, in this order.** (1) **TASK-399 — Stage 2b (S+S, no migration):** the `balance-duo` price group + four items via `sale:ensure-items`, `resolvePriceGroup(subjectId, groupKind?)` (DUO ⇒ duo, GROUP ⇒ group, solo ⇒ subject — every caller by source), the walk-in seat (`SINGLE_SESSION` + `groupId`, cap via the same `seatOnGroup`, 🚫 no extend for a walk-in). Verify §1 (four reads), confirm §2 via me, then build. (2) **TASK-401 — Stage 3a Camp (L, migration `0042_camp`, 43 = 43):** read `SPEC-082` §1–§2 and the owner's rulings in the TASK §0 (half-day units, informational on slots, no-show consumed, the four prices, early bird = a discount). Verify §1, confirm §2 via me — AFTER 399's contract is confirmed; do not start 401's code before 399 is in. Both `sid`; a later `uat` deploy; 🚫 no deploy request.

## 2026-09-19 — Sober → @Jason: ✅ **TASK-399 CONTRACT CONFIRMED — GO.** Four notes in the TASK: the sweep as the fifth caller (yes — pin by value that a DUO walk-in posts `session-balance-duo` at day-end); a real `--dry-run` flag on `ensure-sale-items` (the human's deploy question); drop `GROUP_NO_DATE`; the group DTO gains `priceGroup` from the resolver's own mapping so the FE never maps kind → group. Then TASK-401's contract.

## 2026-09-19 — Sober → @Jason: ✅ **TASK-399 REVIEWED and IN** (2480/0, 42 = 42, re-run by me; the five call sites + `--dry-run` seen). ✅ **TASK-401 CONTRACT CONFIRMED — GO**: FULL_WEEK = 5 · nav after badges · no early-bird date rule (the reason text is the rule) · the Camp card reads `GET /camp/packages?studentId` · own `GET /camp/prices` · 3a transitions from PLANNED + ATTENDED ↔ ABSENT same day (undo to PLANNED is 3b) · four discount columns · two statuses one effect. Migration `0042_camp`, verify 43; `sale:ensure-items` +4 with the dry-run in the report.

## 2026-09-19 — Sober → @Jason: 📋 **TASK-403 — Camp 3b + REQ-096 (M), CONTRACT FIRST.** Four things in one round: (a) **REQ-096, live-facing:** `REMINDABLE` at `daily-reminder.ts:77` carries PENDING since the job was born — the owner wants CONFIRMED-only; I propose dropping EXTENDED too (REQ-094 treats it as not-yet-confirmed) — say if you read it otherwise; one constant, both audiences, the GROUP seats; (b) UNDO an attended/absent camp day → PLANNED, units back, reason required, no money; (c) the camp check-in QR on the session's pattern (say if the token needs columns ⇒ `0043`); (d) the camp-day reminder rows with PLACEHOLDER labels behind a settings flag default OFF — the copy is gated to the owner. Verify §1, confirm §2 via me, then build. `sid`.

## 2026-09-19 — Sober → @Jason: ✅ **TASK-403 CONTRACT CONFIRMED — GO.** All four yes (the `undo_reason` column; lazy token + 23:59:59 expiry + `CAMP_DAY_NOT_TODAY`; 410 for the camp only, said in the header; no CRM points). `0043_camp_checkin_token` ⇒ 44 = 44. Placeholder labels by form, flag default off.

## 2026-09-19 — Sober → @Jason: ▶️ **TASK-405 (XS) — the owner's final camp-reminder copy replaces the `cp_*` placeholders.** 🔑 The label is **`Students`, not `Kids`** (the app's term; pin `Kids` absent); the teacher block lists the children's names beneath the count (≤12, then `+n` — the group `Seats` shape); the parent block `Student / Camp / Date / Time` with the TH labels in the TASK §0. Pins go from "by form" to "by value". Nothing else moves; the flag stays off by default. Report here + `inbox/SA.md` + log.

## 2026-09-19 — Sober → @Jason: 📋 **TASK-406 — REQ-097 teacher login / own calendar, C-1 + C-2 (M+S), CONTRACT FIRST.** Read `SPEC-083` §1. Verify §1 — above all: (a) EVERY read that returns booking rows, and the one place a predicate can be injected without a second copy; (b) which notice tells the FAMILY on a cancel (the leave must send that and NOT the coach one); (c) where a linked user's allowed-route set fails closed in the guard. Then confirm/correct §2 via me. Migration `0044` (45 = 45), key 55, `TEACHER_LEAVE` as the 4th reason code. `sid`; 🚫 no deploy request.

## 2026-09-19 — Sober → @Jason: ✅ **TASK-406 CONTRACT CONFIRMED — GO (M+M).** All five yes: the family notice as a NEW placeholder kind on the leave path only (the admin's cancel stays coach-only — the owner's call, flagged); my column only; the other teachers get the coach notice; `TEACHER_ALLOWED` from Fern's calendar-page calls, listed in your report, nothing that returns people/courses; M+M. Your §1.4 corrected my SPEC (§5 added). `0044` ⇒ 45.

## 2026-09-19 — Sober → @Jason: ▶️ **TASK-408 (XS) — two closes on REQ-097.** (1) `auth.ts:122`'s early exit for `/users|roles` runs BEFORE your `isScoped` line ⇒ a LINKED super admin (scoped by your rule) still reaches user/role admin — move the scope refusal above that exit for those two prefixes, keep `/auth`, `/me`, `/permissions` exempt; pin through the mini app. (2) The login body carries `teacherId` beside `menus`/`actions` (Fern's first-paint flash). Nothing else. Report here + `inbox/SA.md` + log.

## 2026-09-19 — Sober → @Jason: 🔴 **TASK-410 (S, live-blocking) — the teacher-leave 500 is NOT the notice path: `drizzle/0025_booking_cancel_reason.sql:25` has a CHECK limiting `cancel_reason` to the three old codes; `reportOwnLeave` writes `TEACHER_LEAVE` ⇒ `23514` ⇒ 500, tx rolls back, every type.** A third copy of the closed set nobody listed. Do: `0045` redefines the CHECK (I lean `NOT VALID` + `VALIDATE CONSTRAINT` — `bookings` is hot; say the locks in the header), a pin that the CHECK's list ⇔ `END_REASONS`; + the family-notice FINAL copy (in the TASK §1.3, reason line for `TEACHER_LEAVE` only); + §3.7 YES — the admin's cancel enqueues the family kind too (your one call site). Report here + `inbox/SA.md` + log.

## 2026-09-19 — Sober → @Jason: 📋 **TASK-411 — REQ-098 archive a parent (S+), CONTRACT FIRST, AFTER TASK-410.** Read `SPEC-084`; the owner ruled every §3 as recommended (keep the phone, clear the LINE id with an audit copy, cascade to students, coexist with suspend). Verify §1 (every parent read incl. the LINE lookups and `familyLineUserIdsBulk`; REQ-093's reusable parts; what the LINE user sees after the clear), confirm/correct §2 via me. `0046` ⇒ 47, key 56.

## 2026-09-19 — Sober → @Jason: ✅ **TASK-410 REVIEWED and IN** (2540/0, 46 = 46, re-run by me — the CHECK ⇔ `END_REASONS` pin is exactly the guard this needed). ✅ **TASK-411 CONTRACT CONFIRMED — GO**: all five yes (the `text[]` audit list via `clearFamilyLine` in the tx; the where term; Finding B's two refusals with a placeholder LINE reply, never a silent restore; the SOM report drops archived households; the partial index as witness). `0046` ⇒ 47, key 56.

## 2026-09-20 — Sober → @Jason: ▶️ **TASK-413 (XS) — `verify_parent_archived` gets the owner's bytes:** TH `เบอร์นี้เคยลงทะเบียนไว้แล้ว กรุณาติดต่อร้านเพื่อคืนสถานะ` · EN `This number was registered before — please contact the shop to restore it.` Placeholder comment gone; the pin by value both languages. Nothing else. Report here + `inbox/SA.md` + log.

## 2026-09-20 — Sober → @Jason: ▶️ **TASK-414 — REQ-099 birthday filter BE (XS–S, no migration, no key).** The contract is in the TASK §1 from my own read of `searchStudents` (no unknowns left to verify — build; say if anything differs): `birthMonthFrom/To` both-or-neither, `noDob` exclusive with the range (two `400`s), `birthDate` in the select, a pure `birthMonthWhere` with the wrap-around (month table by value) and the wrap sort key, composed WITH the existing suspended/archived terms. Report here + `inbox/SA.md` + log.

## 2026-09-20 — Sober → @Jason: ▶️ **TASK-416 (XS) — REQ-099 gains optional years:** `birthYearFrom/To` both-or-neither, only beside their months, `from <= to`; with years ⇒ a real `birth_date` range (the whole To month), no wrap, date order; without ⇒ TASK-414 byte-unchanged. One helper, two branches, both by value + a date table. Report here + `inbox/SA.md` + log.

## 2026-09-20 — Sober → @Jason: 📋 **TASK-418 — camp ON the grid (M+S), CONTRACT FIRST.** Read `SPEC-085` §1; the owner ruled all four as recommended (hard block via real rows; one window per day, default 10:00–15:00, editable per week; default teachers = the week's; a clash ⇒ refuse naming it). Verify §1 (the OTHER-row shape for a CAMP hour; the week lifecycle's cascade points; what `swapGroupTeacher` lends the per-day swap; the reminder collapse point; the grid payload's marker), then confirm/correct §2 — esp. my two ❓ inside it: hard-DELETE the derived rows vs CANCEL them; whether a week-level teacher change re-derives days already edited by hand. `0047` ⇒ 48.

## 2026-09-20 — Sober → @Jason: ✅ **TASK-418 CONTRACT CONFIRMED — GO.** All five yes: CONFIRMED at birth + the off-day refusal; immutable dates; hard delete of the derived rows; `edited_at` re-derives un-edited days only; the reminder fold through the existing OTHER block. `HUMAN_OTHER_KINDS` — good. `0047` ⇒ 48; say the index's SHARE lock in the header.

## 2026-09-21 — Sober → @Jason: 📋 **TASK-420 — DUO = one course, two kids (L, `0048` ⇒ 49), CONTRACT FIRST.** The customer locked one shared pool. Read `SPEC-087`; verify §1 — above all the TABLE of every read/notice that resolves a session's or course's student (with line numbers; which take a `studentId` argument, which read the row) and the ONE chokepoint through which every session writer must carry `co_student_id` (the REQ-091 rental lesson). Then confirm/correct §2 incl. my three ❓. Group (3–12) byte-unchanged; the GROUP-row DUO retired for creation only.

## 2026-09-21 — Sober → @Jason: ✅ **TASK-420 CONTRACT CONFIRMED — GO.** All four yes: the chokepoint = `insertBooking` + the two clones (pin the `insert(bookings)` census); Finding B in scope (the LIFF leave for child 2's family); the fourth family-accounts copy retired into `householdLineUserIds` (pin no private read remains); `duo` + `groupKey` ⇒ 400. `0048` ⇒ 49.

## 2026-09-21 — Sober → @Jason: ▶️ **TASK-422 (XS) — `getEntitlementPlan` (`:2246`) returns `student` only; a DUO plan page shows one name.** Add `coStudent` / `classRateMinor` / `courseKind` through the same mapper path, pin by value, and add the plan reader to TASK-420's census. Fern's plan title already falls back to one name and reads `coStudent` when present — no FE change after this. Report here + `inbox/SA.md` + log.

## 2026-09-21 — Sober → @Jason: 📋 **TASK-423 — coach rate PER SESSION (§13.3), CONTRACT FIRST, S–M; the `uat` deploy waits for it.** The customer's rule: the course = ครู A @ 7, the substitute week = 3 for THAT session only. Today `PATCH /bookings/:id { classRateMinor }` rewrites the course (`:3729`). Wanted: course DEFAULT + per-session OVERRIDE, `effective = override ?? default` in ONE pure function. Verify §1 — above all whether `bookings.teacher_rate_minor` (Stage 1's per-row rate) can hold the override without confusing its OTHER/GROUP readers (then no migration), else a new column (`0049`). Also fold Tanya's note: `getBookings` rows lack `coStudent` — carry it + `displayName = A & B` through the calendar's rule. Confirm/correct §2 via me.

## 2026-09-21 — Sober → @Jason: ✅ **TASK-423 CONTRACT CONFIRMED — GO.** Reuse the column (no migration); Correction 2 yes; ONE `displayNameOf` for the three copies (pin no `otherTitle ??` chain remains); the `q` widening yes; the make-up-born-with-default reading yes. Body key `classRateMinor` on both routes.

## 2026-09-21 — Sober → @Jason: 🔴 **TASK-425 (S) — the LINE notices still show one child on a DUO.** The outbox WORKER's `bookingContext` (`outbox.service.ts:20–46`) loads `student` only and sets `studentName: b.student?.name`; the renderers prefer `ctx.studentName` over the payload — a fifth name path outside TASK-420's table. Fix: `bookingContext` loads `coStudent` and uses `displayNameOf`; the two payload writers (`:2861`, `:4264`) likewise; a scan pin that every `studentName:` assignment goes through the one rule; the pair in stable course order on every copy (the row does not know its household — say if you see a cheap way). Re-run the six renderers by value with a DUO context. The `uat` gate is held on this.

## 2026-09-21 — Sober → @Jason: 📋 **TASK-426 — REQ-102 narrowed to the freelance ceiling (S–M, no migration, key 57), CONTRACT FIRST.** Verify §1 — every reader that emits a budget figure (the attach at `:330–363`, the teacher DTO, the routes, any report line), and which callers need the BOOLEANS (`overLimit`, `setupIncomplete` — the booking rule must keep working for a masked user). Then confirm/correct §2: ONE mask at the ONE builder; the two writes double-gated; a linked account always masked (the leak). 🚫 Nothing else masked. **REQ-101 (the ECA series + Manage-plan, `0049`) follows this — its task comes when 426 is in.**

## 2026-09-21 — Sober → @Jason: ✅ **TASK-426 CONTRACT CONFIRMED — GO.** The required `viewer` parameter (null ⇒ masked) — yes, explicit, no request context; the dashboard's near-cap label drops the number without the key, the LINE digest unchanged; `hourlyRate` masked with the same key. Key 57, no migration.

## 2026-09-21 — Sober → @Jason: ✅ **TASK-426 REVIEWED and IN** (2670/0, re-run by me). 📋 **TASK-428 — REQ-101 ECA manage-plan BE (M+, `0049` ⇒ 50, key 58, two notice kinds), CONTRACT FIRST.** Read `SPEC-088` Part A; the owner ruled all four as recommended (the notice bytes = REQ-101 §4's draft, by value — not placeholders). Verify §1 (the creator's stamp; the N-row add/remove/swap in one tx; what the coach is told on move/confirm/cancel today; the backfill's grouping), then confirm/correct §2 incl. my three ❓ (cancel-all's coach message shape; `startTime` in the header PATCH — I lean no; the backfill's grouping key).

From @Sober 2026-09-21: ✅ **TASK-428 CONFIRMED — build.** ❓1 ONE `other_series_cancelled` summary per teacher (23rd kind, placeholder words — the owner's gate; a cancel-all enqueues NO per-row notice, pinned). ❓2 `fromDate` on remove + swap too, one `seriesRowsFrom`, default today. ❓3 Finding A SEPARATE — on the owner's list via Porter, nothing here. 🔴 One correction of mine: the date line is **`DD-MM-YYYY`** (REQ-085 §16d / REQ-087, the customer's), not `DD/MM/YYYY` — pin the bytes. Add `otherSeriesKey` to the booking DTO explicitly. Everything else as you wrote (50 = 50, key 58, `ALREADY_ON_ROW` both ways, no `startTime`, backfill keyed with `teacher_id`). Fern's lines final; TASK-429 cut against them.

From @Sober 2026-09-21: ✅ **TASK-428 REVIEWED — DONE.** 2690/0, tsc 0, 50 = 50, key 58, kinds 23 — re-run by me. Nothing waits on you; the next is Fern's wiring.

From @Sober 2026-09-22: 🔴 **TASK-430 — cancel-all coach notice does NOT fire on `sid` (S, defect, build now, no contract round).** `other-series.service.ts:124` notifies on the CONFIRMED subset — Tanya's rows were PENDING ⇒ zero ⇒ no bubble. My gap: the series notices are their own family (ADDED told the coach about pending dates) ⇒ the cancel notice covers EVERY live row cancelled. Plus the owner-accepted bytes (REQ-101 §5) replace the 📖 placeholder. `tasks/TASK-430-cancel-all-notice-fires-be.md`. **Then TASK-431 (contract first, S):** key 59 `action:bookings.coach-rate` — view ⇔ edit of the §13.3 rate, independent of 57; `tasks/TASK-431-coach-rate-key-be.md`. Report 430 first.

From @Sober 2026-09-22: ✅ **TASK-430 REVIEWED — DONE** (2691/0 re-run). ✅ **TASK-431 CONFIRMED — build.** Your seam (ONE `/api/*` response mask on exactly `rate`/`classRateMinor`, exact keys, `teacherRates`/`hourlyRate` untouched) + the scan pin that only the two mappers produce those keys; `assertMayEditCoachRate` at the three routes; a linked account masked regardless; super admin skipped; the calendar payload pinned masked in one walk, `/api/me`+`/api/permissions` byte-identical; independence 57⇄59 pinned. DUO create as-is (no key ⇒ no DUO) — the owner is asked. Fern's lines final in the TASK.

From @Sober 2026-09-22: ✅ **TASK-431 REVIEWED — DONE** (2700/0 re-run). 🔴 **TASK-433 — the Manage-plan page 502s on a BACKFILLED series on `sid`** (a created one is 200). The page fetches client-side, so the API on backfilled data is the first suspect: reproduce on a fixture shaped like the backfill's output (old rows: null heads/rates/end_time, an all-ended series, an archived extra, a long span through `GET /bookings … limit 200`), find the throw or the slow query, fix + pin. `tasks/TASK-433-manage-plan-502-backfilled-series-be.md`. The owner is asked for the server log in parallel.

From @Sober 2026-09-22: ✅ **TASK-433 DONE (read)** — the API cleared; the owner's check + log decide the rest. Nothing waits on you.

From @Sober 2026-09-22: 📋 **TASK-434 (S, light contract: census then build)** — REQ-102 §8: key 59 on EVERY rate surface incl. the ECA/Group `teacherRates` (mask + every writer refused when the field is present; `duo` alone no longer trips the check) and the rate OPTIONAL at DUO create. `tasks/TASK-434-coach-rate-key-everywhere-optional-create-be.md`. Write the census in the TASK, then build; ping only if a producer surprises you.

From @Sober 2026-09-22: ✅ **TASK-434 REVIEWED — DONE** (2711/0 re-run). BE side of the held batch complete: 2711/0, 50 = 50, keys 59, kinds 23. Nothing waits on you.

From @Sober 2026-09-22: 🔴🔴 **TASK-436 — REGRESSION on `uat`: the coach's `teacher_unassigned` (`📤 คาบสอนนี้ถูกย้ายออกจากตาราง…`) stopped arriving on a reassignment (customer screenshot).** I read: the only producer (`applyPlanChange :3231`) is intact and git shows no enqueue removed since 09-19 — so suspect DELIVERY through TASK-425's `bookingContext` fold first (a thrown render ⇒ FAILED row, silent coach), then the notice-days gate, then the DOOR (the Move-session popup never told the coach — Finding A; if 1–2 are clean, build that pair into `moveBooking` here). Cause with evidence + hotfix-vs-batch. `tasks/TASK-436-moved-out-coach-notice-regression-be.md`. First priority.

From @Sober 2026-09-22: ✅ **TASK-436 REVIEWED — DONE** (2718/0 re-run). The notice-days gate on the popup ⇒ filed to the owner, not folded. Nothing waits on you.

From @Sober 2026-09-22: 📋 **Two contract-first tasks, in order.** **TASK-437** (S): `subjects.kind` (`0050` ⇒ 51, CHECK ⇔ `SUBJECT_KINDS`), the DTO field, `scripts/ensure-subjects.ts` for the two DUO subjects, the server rule (a DUO course needs a DUO subject; ❓ the reverse refusal too). **TASK-439** (S–M): whole-voucher cancel on the course-end shape (`0051` ⇒ 52: `vouchers.ended_at/by/reason` + CHECK ⇔ `END_REASONS`; `POST /vouchers/:id/cancel` + `/preview` under `bookings.course-cancel`; future live draws ⇒ CANCELLED, past + `used_hours` untouched, balance frozen; `voucherStatus` with ENDED first; ENDED cannot draw; one `course_dropped_teacher` per coach with a voucher line, no new bytes; no family notice). Both in `tasks/`. Contract 437 first.

From @Sober 2026-09-22: ✅ **TASK-437 CONFIRMED — build.** Reverse refusal yes · `add-subject --kind` yes · the seed as an upsert yes (planner pinned) · single/trial on a DUO subject left as designed. Then TASK-439's contract.

From @Sober 2026-09-22: ✅ **TASK-437 REVIEWED — DONE** (2729/0, 51 = 51 re-run). ✅ **TASK-439 CONFIRMED — build:** the split as you wrote (ONE sender generalised, shared `cancelDoomedRows`); `<subject> 10 HR` no new bytes; undo allowed; both corrections (`sessions[]` shape + `remaining`; body `reason`); `cancel_reason` on the voucher rows; **take the course end's holds line** as its own bullet + pin. Fern's lines final.

From @Sober 2026-09-22: ✅ **TASK-439 REVIEWED — DONE** (2749/0, 52 = 52 re-run). The note-not-persisted is accepted. BE side of the locked batch complete — nothing waits on you.

From @Sober 2026-09-22: 📋 **REQ-104 gap build — two contract-first tasks, in order.** **TASK-441** (M): GROUP parity — the series service generalised to `group_key` (cancel-all CASCADES to every seat, each family `class_cancelled_parent`, the coach once; add/remove extra; add dates; header PATCH), one-button confirm-whole-group (`confirmCourse` per seated course), the Monday weekly coach digest `weekly_schedule_teacher` — **ENGLISH ONLY** bytes in the TASK. **TASK-443** (M): camp per-coach-per-day rate (`0052` ⇒ 53; behind 59 by the existing body check + mask; copied onto the derived rows; default 0), the scan responses carry the remaining credit (camp + Private), `camp_deduction` at the day-end cut — **ENGLISH ONLY**. Contract 441 first.

From @Sober 2026-09-22: ✅ **TASK-441 CONFIRMED — build.** One module with `SeriesKey`; the group swap delegates; the existing seat path; `confirmCourse` no-op caught as skipped; NO flag; CONFIRMED-only rows; the bytes/separators as written; the Monday task registration is the human's line. Fern's lines final. Then TASK-443's contract.

From @Sober 2026-09-22: ✅ **TASK-441 REVIEWED — DONE** (2766/0 re-run). TASK-443's contract next.

From @Sober 2026-09-22: ✅ **TASK-443 CONFIRMED — build.** (b) day-end only with the stamp; ABSENT notifies the same; `(total − used) / 2`; both simplifications; the one route line; the pins move. Fern's lines final.

From @Sober 2026-09-22: ✅ **TASK-443 REVIEWED — DONE** (2776/0, 53 = 53 re-run). BE side of the whole batch complete — nothing waits on you.

From @Sober 2026-09-22: 🔴 **TASK-445 (S, before the customer-UAT):** (1) camp-week create INCLUDING today ⇒ 500 on a coach clash — find the actual exception (my reads in the TASK: a raw 23505 the sync's catch misses, or past dates in the week's range) ⇒ 409 `SLOT_TAKEN` naming the clash, nothing written; (2) group cancel-all family notices: union the row's seat households and tell each LINE account ONCE per row (siblings dup today); response gains `familyNotices` + `householdsTold` (distinct accounts). `tasks/TASK-445-camp-week-500-and-family-dedupe-be.md`.

From @Sober 2026-09-22: ✅ **TASK-445 REVIEWED — DONE** (2783/0 re-run). The 25P02 lesson recorded in SYSTEM-FACTS. Past rows on re-sync: leave. Nothing waits on you.
