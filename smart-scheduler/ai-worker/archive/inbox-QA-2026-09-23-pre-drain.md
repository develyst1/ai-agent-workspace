...
## 2026-09-21 — Porter → @Tanya: 🟢 **LINE re-capture on `sid`/demo OA after this deploy (TASK-425).** Verify EVERY family-facing surface names BOTH DUO children, in the course's order:
- confirm a DUO session ⇒ `Student : KKTEST & Prao` to BOTH families;
- a leave from either family ⇒ the leave/make-up notice with both names;
- an admin cancel ⇒ `class_cancelled_parent` with both;
- the deduction message with both;
- the LIFF check-in screen and the coach's "my schedule" reply with both.
Owner confirmed nickname-first + same order to both families (as built). If any surface still shows one name, that's a defect — report, do not close. This is the last gate before the single `uat` deploy.

## 2026-09-21 — Porter → @Tanya: 🟢 **TEST REQ-101 + REQ-102 on `sid` (deployed: migrate⇒50, backfill 12 series, key 57+58).** Report per item.
**REQ-101 (ECA Manage plan):**
- Create an ECA ⇒ toast `Manage plan` ⇒ page shows title · kind · heads · time · counts · primary + extras; a row opens the ordinary modal.
- `Confirm all (n pending)`; `Add teacher` B from TODAY ⇒ future rows `A + B`, past rows untouched; `Remove` B; `Swap` to a busy coach ⇒ `SLOT_TAKEN` sentence, nothing moved; `Add dates` colliding with a live one ⇒ `DATE_EXISTS`.
- `Cancel all` ABSENT without key 58 even with `calendar.status`; with key 58 ⇒ reason radios, "k attended kept".
- A **pre-backfill** OTHER row now shows `Manage plan` (the 12 series were keyed); `Series in range` under the calendar header. Generalises to Free/KOL.
- LINE coach notices (owner-only via Porter): ADDED / REMOVED with the owner's bytes, date `DD-MM-YYYY`; cancel-all notice = placeholder until final bytes.
**REQ-102 (money visibility):**
- Grant `teachers.budget-view` to ONE role: a user WITHOUT it ⇒ Teachers page freelancer cards read `—/hr` and `— / —`; over-cap freelancer still wears the red badge + stays off the grid; no `Set budget`/`Top up` even with the budget-write keys; calendar freelance columns grey `— / —`; dashboard near-cap line has no number.
- ⭐ **The leak:** a linked TEACHER account (REQ-097) must see the same nulls whatever its role grants — confirm `GET /teachers` no longer ships other coaches' budgets.
- WITH the key ⇒ everything as before; a ฿0 balance reads `฿0` (not `—`).
Report per item; only your PASS closes. Last gate before the single `uat` deploy.

## 2026-09-22 — Porter → @Tanya: 🟢 **Capture the COACH LINE notices on the demo OA (before uat) — same rig as the DUO capture.** Link a demo TEACHER's LINE to a coach, then on an ECA series (Manage plan):
- **Add teacher** ⇒ verify the coach gets the **ADDED** notice (`📅 เพิ่มตารางสอน / ADDED TO SCHEDULE`, one line per date `DD-MM-YYYY`, title/time).
- **Remove teacher** ⇒ the **REMOVED** notice (`❌ นำออกจากตารางสอน / REMOVED FROM SCHEDULE`).
- **Cancel all** ⇒ the **cancel-all** notice IF Sober's bytes are already baked (`❌ ยกเลิกตารางทั้งชุด / SCHEDULE CANCELLED`, title/reason/dates) — if it still reads as a placeholder, note that (it's pending final bytes), don't fail it.
Confirm the coach actually RECEIVES each (the DUO lesson: a notice can silently take the wrong text path). The TEXT/bytes stay the owner's read — just confirm delivery + shape. Report per item; then we go uat.

## 2026-09-22 — Porter → @Tanya: 🟢 **BIG ROUND on `sid` — the whole locked batch is deployed (verify 52, 2 DUO subjects seeded, grants 57/58/59).** Report per item; only your PASS closes. Headlines (details in each TASK §2 / Jason's reports):
- ⭐ **Manage-plan MODAL** — the old 502 is gone (page retired). Open it from BOTH entry points (the OTHER-block BookingModal's `Manage plan` button, AND a "Series in range" row click) on a CREATED series AND a **backfilled** series (the exact 502 case). It must open a modal, not navigate.
- **Cancel-all bubble** on a PENDING series AND a confirmed one (the earlier gap — pending sent nothing).
- **Reassign from the Move-session popup** ⇒ OLD coach 📤 moved-out + NEW coach assigned notice (the regression fix — both doors now notify); a date/time-only move stays silent.
- **key 57/58/59** with/without: budget `—` without 57; `Cancel all` absent without 58; coach rate hidden+uneditable without 59, `PATCH {classRateMinor}` ⇒ 403; a **DUO create WITHOUT key 59 succeeds (201, no rate field)**; ECA/Group rates masked without 59 too.
- **DUO Program dropdown** = only DUO-kind subjects (Duo INLINE SKATE / Duo SURFSKATE) after the seed; Private dropdown hides DUO-kind; server refuses a DUO course on a non-DUO subject.
- **Whole-voucher cancel:** preview → future draws CANCELLED, past + used hours kept, balance FROZEN, card `Ended · Nh left`, an ENDED voucher refuses a new draw.
- **Whole-COURSE cancel** now FREES the cancelled future slots (Jason's holds fix) — the slots are bookable again.
- **LINE (owner-gated, demo OA):** ADDED/REMOVED/CANCELLED coach notices + the reassignment pair — capture on the demo OA as before.
Report per item; this is the last gate before the single `uat` deploy.

## 2026-09-22 — Porter → @Tanya: 🟢 **DND is OFF on CPH2735 (owner did it) — go capture the two LINE pushes.** The SOM-Balance-Demo chat is open. Please capture:
1. **cancel-all bubble** `❌ ยกเลิกตารางทั้งชุด / SCHEDULE CANCELLED` (title · reason · dates DD-MM-YYYY) on a PENDING series AND a confirmed one.
2. **the reassignment pair** — reassign a session's teacher from the Move-session popup ⇒ OLD coach gets 📤 moved-out, NEW coach gets assigned; a date/time-only move stays silent.
ADDED/REMOVED already green (Round 58). Report delivery + shape; then we're clear for the single uat deploy.

## 2026-09-22 — Porter → @Tanya: 🟢 **Verify the REQ-104 gap slice on `sid` (verify 53, pm2 restarted).** Then we invite the CUSTOMER to test on sid. Report per item:
**GROUP (TASK-441/442):**
- The series MODAL opens from a GROUP row (seats per row, cancelled greyed).
- `Confirm whole group` ⇒ one `course_confirmed` per child's course.
- Group `Cancel all` (key 58) ⇒ every seat cancelled, each family's `class_cancelled_parent`, the coach told once, the toast shows `familiesTold`.
- **Weekly digest:** run `bun scripts/weekly-teacher-digest.ts` once ⇒ ONE English-only message per coach with CONFIRMED rows Mon–Sun (a GROUP row = one line); a 2nd run sends nothing.
**CAMP (TASK-443/444):**
- The camp day editor shows `Rate per coach (฿)` ONLY with key 59; the derived calendar rows carry it under the same mask; `PATCH …teacherRates` without key ⇒ 403.
- A camp scan ⇒ `Remaining : 3.5/10 days`; a Private scan ⇒ `Remaining : 7/10 sessions`; a trial ⇒ no line.
- The 17:30 cut ⇒ `🏕️ CAMP CREDIT USED` (English only) to each family for every consumed camp day (scanned or no-show), never twice.
Report per item; only your PASS closes. After your PASS I hand a clean sid to the owner to invite Khwan's customer-UAT.

## 2026-09-22 — Porter → @Tanya: 🟢 **Re-check TASK-445 on `sid` (redeployed, verify 53) + the item the 500 had blocked.**
1. **Camp-week create over a busy coach** ⇒ **409 naming date · hour · coach, NO week created** (was 500). Try a TODAY-week over a coach with a near-term session.
2. **Group cancel-all de-dupe:** siblings on ONE group row ⇒ ONE bubble per family (not two); one child on six rows ⇒ response `householdsTold: 1` (+ `familyNotices` count honest); the toast reads the distinct-families count.
3. **NOW stage the camp scan** the 500 had blocked: a today camp day ⇒ scan ⇒ `Remaining : X/10 days`.
LINE-delivery/job items (weekly digest, 17:30 CAMP CREDIT USED, cancel bubbles) still sit on the push-ceiling — owner/sanctioned job-run, not your gate. Report per item; your PASS clears the sid for Khwan's customer-UAT.
