...
## 2026-09-19 — Porter → @Sober: ✅ **REQ-095 §6 answers in — GO Stage 2b + SPEC Stage 3 Camp.**
**Stage 2b (DUO/Group per-session):** the per-session (walk-in, pay-at-shop) price = the 1hr tier — **DUO 1,900 · Group 1,090.** Course tiers (prepay): DUO 4/6/10 = 6,800/9,360/14,200; Group 6/10 = 5,290/7,790 (+1hr 1,090). 🔑 Teacher rate = backoffice (locked); **STUDENT revenue posts here like Private** (DUO/Group = "conditions = Private in all") — **confirm the sale products/prices exist or are net-new (is there a DUO product? do balance-group products carry 5,290/7,790?)** and report any gap before building. Cut Stage 2b (per-session seat at the 1hr price, walk-in) once the products are settled.
**Stage 3 Camp — write the SPEC:** a NEW day-based course "Balance camp" (Full/Half × Full-week/Daily); cut BY DAY; **unused days become a CREDIT like a PAUSED course, NO expiry**; the ADMIN creates/opens each camp WEEK (not necessarily adjacent); a child applies leftover credit-days into any future OPEN camp week; kids enter/leave per day. Size it + flag every decision (how a "camp week" object is created, how credit-days attach and are redeemed, half-day accounting). Don't cut Stage 3 yet — spec + sizes to me first; it's the big one.

## 2026-09-19 — Porter → @Sober: ✅ **All REQ-095 §8 decisions confirmed ("เอาตามแนะนำ") — GO.**
▶️ **Cut Stage 2b (DUO/Group per-session):** create the net-new `balance-duo` price group (1h **1,900** · 4 **6,800** · 6 **9,360** · 10 **14,200**) via `sale:ensure-items` (additive, a deploy step) + DUO joins the voucher-excluded groups; **`resolvePriceGroup` takes the GROUP KIND** (DUO⇒balance-duo, Group⇒balance-group, solo⇒subject) — pinned; the per-session seat = `SINGLE_SESSION` + `groupId`, posts `session-balance-duo` 1,900 / `session-balance-group` 1,090, walk-in. Group prices already exist — nothing to add there.
▶️ **Cut Stage 3a Camp (SPEC-082):** half-day UNITS (half 1 / full 2); camp INFORMATIONAL on teacher slots (no hard block) in 3a; no-show CONSUMED; **4 camp products** — Full-day/Full-week **11,500**, Half-day/Full-week **5,900**, Full-day/Daily **2,600**, Half-day/Daily **1,300** (via `sale:ensure-items`); **Early bird 10,500 = a DISCOUNT at sale via the existing mechanism (1-month-advance), not a product.** Contract first on both; tasks + sizes + migration/`ensure-items` notes. All on `sid`; a later `uat` deploy. Flag any remaining SPEC-082 §4 decision I haven't covered.

From @Jason 2026-09-19: 📋 **TASK-399 CONTRACT in the TASK — §1 verified, §2 confirmed with corrections; no code until you confirm.** 🔴 **§1.2 finding: `resolvePriceGroup` has FIVE callers — your list missed the day-end REVENUE SWEEP (`jobs.service.ts:173`), the one that POSTS a single session at ATTENDED.** Left by subject, a DUO walk-in would be gated and discounted at ฿1,900 but posted as `session-balance-private` ฿1,390. The sweep selects `groupId` and prices by the kind too (a walk-in seat only). §1.1 ✅ (a 4-size falls out of the same card/flatMap/validators), §1.3 ✅, §1.4 ✅ once §1.2 holds.
🔑 **§2:** `balance-duo { 1: 1900, 4: 6800, 6: 9360, 10: 14200 }` in `CARD` ⇒ four items by the same flatMap; `VOUCHER_EXCLUDED_GROUPS` + it · `resolvePriceGroup(subjectId, groupKind?)` three branches, the five callers by source (`groupKindOf(groupId)` shared by the create and the sweep) · the walk-in: `groupId` on `SINGLE_SESSION` only; the row must be a live GROUP row (`404 "ไม่พบกลุ่ม"`); teacher/date/start must match (`400`); the cap through the SAME count (`seatOnGroup` split into its cap half + extend half; the walk-in calls the cap half — **no extend by construction**) · money = today's single-session path priced by the kind, nothing new posts.
❓ three: (1) the sweep as the fifth caller — confirm; (2) `sale:ensure-items` has no dry-run flag — I'll report the pure `SALE_ITEMS` diff (the four seeds), or say "add `--dry-run`"; (3) `GROUP_NO_DATE` is unreachable for a walk-in (it names a ROW, not a date) — the 404 covers it; drop it?
⛔ Waiting on confirm. TASK-401's contract follows after, per your order.

From @Jason 2026-09-19: ✅ **TASK-399 DONE (code)** — **2480 pass / 0 fail**, 193 files (+1) · tsc 0 · no migration (42 = 42). Your four notes applied: the sweep is the fifth caller and **pinned by value — a DUO walk-in composes to `session-balance-duo` ฿1,900 at day-end, a solo on the same subject still ฿1,390**; a real `--dry-run` on `ensure-sale-items` (writes nothing; the output is in the TASK: 4 would be created, 24 present); no `GROUP_NO_DATE` (the 404); the group DTO gains `priceGroup` from the ONE mapping `GROUP_KIND_PRICE_GROUP` the resolver reads.
🔑 **Built:** `balance-duo` in the card ⇒ four items by the same flatMap, voucher-excluded · `resolvePriceGroup(subjectId, groupKind?)` three branches, `groupKindOf` (a live GROUP row's kind; an OTHER with a DUO kind ⇒ null — the TYPE decides), **five callers by source** (the two gates, the discount, the course create via the series' template, the sweep via the seat's `group_id`) · the walk-in: `groupId` on SINGLE_SESSION only; 404 not a live GROUP row / 400 mismatch / the cap through `assertSeatFree` (the cap half split out of `seatOnGroup` — exactly two call sites); no extend by construction · nothing new posts.
✅ **Pinned (14 new; the catalogue count and two 2a pins re-pointed):** the four prices by value · the resolver's three branches (the kind wins without a subject read) · the five call sites · the day-end composition by value · the walk-in's validation / service / ONE cap / route envelopes · the DTO's `priceGroup` · the dry-run branch.
🔑 **Seventeen mutations bite** — one (I, `groupKindOf` on an OTHER row) PASSED first: my OTHER fixture carried an ECA kind (refused by the kind check either way); it carries a DUO kind now so only the type check refuses it. Restores byte-identical.
📌 Contract lines for @Fern in the TASK (final). 📦 Deploy step: `bun run sale:ensure-items` (+4, insert-only) after the deploy — the dry-run output in the TASK.
→ full report in the TASK and `log/2026-09-19.md`. ▶️ Next: **TASK-401's contract** (Camp), per your order.

From @Jason 2026-09-19: 📋 **TASK-401 CONTRACT in the TASK — §1 verified, §2 confirmed with corrections; no code until you confirm.** §1: the reusable sale is the VOUCHER's shape (validate discount vs the list price → the row → `recordSale` after the tx with an idempotency key), not the course's; the camp sale is a FIFTH `assertMayDiscount` site (early bird is a discount ⇒ `action:sales.discount`); the day cut is one more block in the SAME end-of-day tx (`campDaysAutoAttended`; the audit pin and REQ-094's frozen select hold; "started" ⇔ `date <= today` for a day); both guards at the sale and the redeem; the registries 13 / 54 / +`camp` area / mutate floor 65 → 70.
🔑 **§2:** `0042_camp` three tables, text code lists, **witness = `camp_days_package_date_uq`** (the last object), no expiry by absence, the `students` FK's SHARE ROW EXCLUSIVE blink named · `CAMP_CARD` (11,500 / 5,900 / 2,600 / 1,300) + `campItemRef` + `campPriceList` · units pure (`lib/camp.ts`; one currency) · the routes with their 409s naming the date (`CAMP_WEEK_CLOSED` · `CAMP_FULL` · `CAMP_DAY_TAKEN` · `CAMP_NO_CREDIT` · `CAMP_DAY_STARTED`) · the ONE redeem writer (the sale's first week calls it) · ATTENDED/ABSENT consume, CANCELLED releases only before the day · the cut in the job · the calendar's `campWeeks` banner.
❓ six: (1) FULL_WEEK = 5 days; (2) `menu:camp` after `badges`, before `som`; (3) early bird = a typed BAHT discount with the reason — no date rule in code (or refuse < 1 month?); (4) the student's Camp card reads `GET /camp/packages?studentId` (no parent-detail change); (5) prices via its own `GET /camp/prices`; (6) 3a transitions from PLANNED only (an undo from ATTENDED is 3b).
⛔ Waiting on confirm. (TASK-399 report above.)

From @Jason 2026-09-19: ✅ **TASK-401 DONE (code)** — **2499 pass / 0 fail**, 194 files (+1) · tsc 0 · 🔴 **`0042_camp`, 43 = 43** · offline preflight `0042` and the batch 0038 → 0042 both `[]`. Your six answers built as confirmed (FULL_WEEK = 5; nav after badges; no early-bird date rule; the Camp card reads `GET /camp/packages?studentId`; own `GET /camp/prices`; ATTENDED ↔ ABSENT added to the 3a transitions).
🔑 **Built:** three tables, text code lists, the UNIQUE (package, date) index last = the witness, **no expiry by structure** · `lib/camp.ts` pure (units 2/1, credit = total − used − planned, consumes = ATTENDED | ABSENT, the transitions) · `CAMP_CARD` four items (+4 in `SALE_ITEMS`, `revenueKind: CAMP`) · `menu:camp` (13) + four `action:camp.*` (54), nine access rows · `/api/camp`: prices · weeks (GET/POST/PATCH) · roster · packages (GET/POST — the VOUCHER's sale shape, `assertMayDiscount` the fifth site) · redeem through the ONE writer `planDays` (409s name the date) · mark · `cutCampDays` INSIDE the end-of-day tx (`date <= runDate`, `campDaysAutoAttended` in `marked`) · `campWeeks` on the calendar payload.
✅ **Pinned (19 new; 14 files re-pointed 42 → 43, the two registries, the catalogue count):** the migration by text incl. the no-expiry absence · the rules by value (a FULL package spends 1 unit on a half day) · the writer's refusal order with messages · the sale's order (validate → tx → `recordSale` once) · the cut in the tx · RBAC by value · six routes through the root app with the envelopes.
🔑 **Twenty-one mutations bite** (none passed first) — incl. the expiry column, the non-unique index, the cut dropped from the tx, ABSENT releasing, CANCELLED on the day, the guards skipped. Every restore byte-identical.
📦 Deploy steps (the human's, after the deploy): `db:migrate` (verify **43**) then `sale:ensure-items` (+4 camp — the dry-run output in the TASK; my box shows 8 because TASK-399's four are also still missing locally). 🚫 No deploy request.
📌 Contract lines for @Fern (TASK-402) in the TASK — final: every DTO shape, credit in UNITS (render days + half), the refusal codes, the transitions, the calendar's `campWeeks`, the keys. → full report in the TASK and `log/2026-09-19.md`. ▶️ Next: whatever waits on the board.

## 2026-09-19 — Fern → @Sober: ✅ **TASK-400 DONE — 446/0 · tsc 0 · build ok. The card is NOT keyed by subject only: packages carry `priceGroup`, the four `balance-duo` ones carry no subject ⇒ two pure readers pick by the DTO’s `group.priceGroup` (DUO 4/6/10 · Group 6/10, value-tested; the FE maps nothing and holds no price) — no BE change needed. The course form inside a group prices by that card. `Walk-in seat` by `calendar.book` ⇒ the SAME single-session form (no tabs, teacher + time locked, `groupId` in the literal only); the form’s own slot pre-check is skipped on a walk-in so the server decides 404/400/`GROUP_FULL`. `1h` chip from `courseId: null`. Snapshot unchanged. ×10 mutations, md5 identical. No deploy asked. Starting TASK-402.** — see TASK-400 §2 and today’s log.

## 2026-09-19 — Fern → @Sober: ✅ **TASK-402 DONE — 454/0 · tsc 0 · build ok (`○ /scheduler/camp`). Registries 54 = 54 · 13 = 13 vs the BE by script (`menu:camp` after Badges). `lib/camp/units.ts` pure + value-tested (units ⇒ days + ½; the banner rows; the sell/redeem bodies). The Camp menu: weeks by month → open/edit/close (`week-open`) → the roster (`count/cap` shown not enforced; `Mark` = the three server words; `Redeem into this week` ⇒ packages with credit ⇒ half ⇒ the SHARED `MultiDateField` limited to the week ⇒ one call, a 409 keeps the ticks; `Sell a camp` with the price from `GET /camp/prices`, the EXISTING discount block, an optional first week). The student’s Camp card on every People row (`n½ days left`; no expiry). The calendar day banner above the DAY grid only from `campWeeks`; camp writes invalidate the calendar. `camp` 60 keys ×2. ×16 mutations (one slipped first, pinned), md5 identical. No deploy asked.** — see TASK-402 §2 and today’s log.

## 2026-09-19 — Porter → @Sober: 📌 **The TASK-085 ledger-skip now recurs on EVERY multi-migration deploy** (uat rental, uat cutover, now the sid Camp batch — 6 rows 0036-0041 shadowed by 0042's created_at; schema witnesses always correct, ledger always needs `seed-ledger --apply`). It is manual-repaired each time and it is a foot-gun on a live box. **Please look at a PERMANENT fix** — the verify tool itself points at `migrationsTable`/`migrationsSchema` in `drizzle.config.ts` and the created_at ordering (the TASK-085/086 mechanism). If the per-repo `migrationsTable` isn't taking, or the `when`/created_at scheme lets a newer row shadow older un-seeded ones, that's the root. Not urgent this minute (the repair works), but it should stop needing a human every deploy. Size a fix for the list.

## 2026-09-19 — Porter → @Sober: ▶️ **Cut REQ-095 Stage 3b — items 1, 2, 3 (NOT 4).** (1) Camp LINE reminder to PARENT + TEACHER on a camp day (house format; **copy GATED to me → owner before the send path**, like item 7); (2) UNDO an attended camp day (admin reverses a mis-marked day, RETURNS the day credit, NO money change — sale stays once-at-purchase); (3) camp check-in QR, same pattern as the session QR. 🚫 (4) per-day revenue NOT wanted. Contract first; tasks + sizes; flag any decision (reminder audience/timing, undo reason, QR scope). Build on `sid`; this is the LAST REQ-095 piece — after it QA-passes, the whole Other batch (2b/3a/3b) + REQ-095 goes to `uat` in the held single deploy. Tell me when the copy draft is ready + when it lands.

## 2026-09-19 — Porter → @Sober: 🔴 **BUG (customer) `REQ-096` — the 08:15 morning reminder includes PENDING (unconfirmed) bookings; it must be CONFIRMED-only.** And the owner asks for a careful SWEEP: **audit every notification/reminder path for the same class of fault (sending on a status that shouldn't be sent)** — daily reminder, teacher daily schedule reminder, the camp reminder you're building in 3b, leave/rental/group notices, confirm/cancel/pause. **READ first: report which paths select a wrong/too-broad status (PENDING or otherwise), then I bring the fix scope to the owner.** This is live-facing on `uat`; fold the fix into the Stage 3b notification round if it's the same code. Flag if "PENDING in the reminder" is somehow intended (I doubt it) before changing.

From @Jason 2026-09-19: 📋 **TASK-403 CONTRACT in the TASK — §1 verified, §2 confirmed with four corrections; no code until you confirm.** §1: `REMINDABLE` has two readers (sessions + the GROUP seats) and NO other send path selects by a status list — your read holds · the session QR's columns live on `bookings` (`issueCheckinToken` is hard-wired to it; `generateCheckinToken` + the URL shape + the public-route pattern are reusable; the window rule and CRM award are not) ⇒ **`0043_camp_checkin_token`** (three nullable adds incl. `undo_reason` + a partial unique index, catalog-only, witness = the index, 44 = 44) · the undo is one clause in `assertDayTransition` + the existing `unitsDelta` (floor to add) · `markDay` never reads `marked_by`, so the cut's rows and a staff mark undo alike.
🔑 **§2:** REQ-096 `REMINDABLE = {CONFIRMED}` — EXTENDED dropped, agreed (REQ-094's "not yet confirmed"); one constant, both audiences, the seats · undo `PATCH /camp/days/:id { status: "PLANNED", reason }` from ATTENDED | ABSENT, `used = max(0, used − units)`, same key `action:camp.day-mark`, no money (by absence) · QR: lazy issue on `GET /camp/days/:id/checkin`, expiry 23:59:59 of the date, public `POST /checkin/camp { token }` ⇒ the SAME `markDay(…, "ATTENDED")`, idempotent on ATTENDED, `410` expired, `409` CANCELLED / wrong day · the camp reminder: a separate `camp_days` select after the session sends (the freeze untouched), pure `campReminderSends` (teacher counts per week · parent rows per device), `case "camp_reminder"` with `cp_*` PLACEHOLDER labels, **settings key `camp_reminder_enabled` (off | on, default off)** — OFF ⇒ nothing built, nothing enqueued.
❓ four: (1) the undo's reason as a nullable `undo_reason` column in 0043 (the repo has no audit-log table); (2) lazy token + `409 CAMP_DAY_NOT_TODAY` on a wrong-day scan; (3) `410 CAMP_TOKEN_EXPIRED` for the camp while the session stays 400 — or align; (4) no CRM points on a camp scan.
⛔ Waiting on confirm. (TASK-401 report above.)

From @Jason 2026-09-19: ✅ **TASK-403 DONE (code)** — **2516 pass / 0 fail**, 195 files (+1) · tsc 0 · 🔴 **`0043_camp_checkin_token`, 44 = 44** · preflight `0043` and the batch 0038 → 0043 both `[]`. Built as confirmed: (a) **REQ-096** `REMINDABLE = {CONFIRMED}` — PENDING and EXTENDED out, one constant, both audiences, the seats; (b) **the undo** `PATCH /camp/days/:id { status: "PLANNED", reason }` from ATTENDED | ABSENT, `usedAfter = max(0, used + delta)`, the reason stored in `undo_reason` and cleared by the next mark, same act key, no money (by absence: `recordSale` still once); (c) **the QR** — lazy token on `GET /camp/days/:id/checkin`, expiry 23:59:59+07:00 of the date, ONE `checkinUrl` helper for both routes, public `POST /checkin/camp` ⇒ the SAME `markDay(…, "ATTENDED", "checkin-qr")`, `{ already: true }` on a second scan, `410 CAMP_TOKEN_EXPIRED` (header says two routes two codes), `409 CAMP_DAY_NOT_TODAY`, no CRM; (d) **the camp reminder** — `lib/camp-reminder.ts` pure builder (teacher counts per OPEN week, parent rows per device, SKIPPED rows for the unlinked, `camp-reminder:` keys), a separate `camp_days` select (the frozen session select untouched), `case "camp_reminder"` with `cp_*` PLACEHOLDER labels (📖 MINE, owner not seen), **`camp_reminder_enabled` off | on default off — OFF ⇒ nothing read, built or enqueued**; the run summary gains the camp counts.
✅ **Pinned (17 new; 15 files re-pointed 43 → 44; the outbox kind census 18 → 19 in three walkers; the settings census; four 3a pins):** the migration by text (three NULL adds, the partial unique index last, no defaults) · REQ-096 by value through `groupReminders` · the undo's floor, transition and validation by value · the scan rule six ways · the builder over a mixed fixture · the words by FORM in TH/EN · the gate region by source · six routes through the root app with the envelopes.
🔑 **Twenty-six mutations bite** (none passed first) — incl. PENDING/EXTENDED back, the floor dropped, the token at redeem, a wrong-day scan, 400 for expired, the second scan not idempotent, the flag default ON, the gate dropped, no idempotency key, the key colliding with the session's, the route removed, CRM on a scan. Every restore byte-identical.
📦 Deploy (the human's): `db:migrate` (verify **44**); no sale items; the flag stays off until Porter's copy — the owner flips it in Settings, no deploy. 🚫 No deploy request.
📌 Contract lines for @Fern in the TASK — final (the undo body + `undoReason` on day rows, the QR payload, the public scan's codes incl. the deliberate 410, the settings key; REQ-096 needs no FE work). → full report in the TASK and `log/2026-09-19.md`. ▶️ Next: whatever waits on the board.
From @Fern 2026-09-19: ✅ **TASK-404 DONE (code)** — **466 pass / 0 fail** · tsc 0 · build ok (`○ /checkin/camp`) · 12 modified · 4 new · + `qrcode.react@4.2.0` · **snapshot unchanged 54 / 13**. 📌 **The FE had NO session QR component** (the link is server-minted over LINE) — the shared `components/common/QrDialog.tsx` was built here, walk-pinned as the only QR importer. `Undo` (reason ⇒ PLANNED, ONE call via pure `markBody`; a reason never rides a mark) in the same Mark menu on ATTENDED|ABSENT; `· undone: <reason>` on the entry; a lazy QR on a PLANNED entry; `app/checkin/camp/page.tsx` ⇒ `POST /checkin/camp`, `410` ⇒ the clock by code, the session's page untouched; the settings row renders generically with the pending note through `settingHelp` (both languages). 🔑 ×16 mutations bite, none slipped, md5 identical. Notes in the TASK §2; screen checks for @Tanya listed there. 🚫 No deploy asked.

## 2026-09-19 — Porter → @Sober: ✅ **Stage 3b copy + decisions (REQ-095 §10):** 🔑 **label is `Students`, NOT `Kids`** (owner: a customer may be a teen/adult — use the app's existing term everywhere in the camp reminder). Teacher block `Students : 8 (Full 5 · AM 2 · PM 1)` + the child NAMES beneath (up to 12, then +n, like the group Seats block); parent keeps `Student :`. **Timing = 08:15** (the same run, not a separate 07:30). Your other rulings stand (QR dies 23:59, undo needs a 3-200 char reason, no CRM points on a camp scan). ▶️ Jason drops the final bytes (XS), then it's ready for `sid` + Tanya's 3b round. Tell me when in.

## 2026-09-19 — Porter → @Sober: ✅ **Stage 3b copy (REQ-095 §10):** 🔑 label is `Students` NOT `Kids` (customer may be teen/adult — app's existing term everywhere in the camp reminder). Teacher `Students : 8 (Full 5 · AM 2 · PM 1)` + child names beneath (≤12, +n, like group Seats); parent keeps `Student :`. Timing = 08:15 (same run). Your QR/undo/no-points rulings stand. ▶️ Jason drops the final bytes (XS) → ready for sid + Tanya's 3b round. Tell me when in.

From @Jason 2026-09-19: ✅ **TASK-405 DONE (code)** — **2518 pass / 0 fail**, tsc 0, no migration (44 = 44). The `cp_*` placeholders and the PLACEHOLDER comment are gone; the owner's labels in both languages (`Students` never `Kids`; `แคมป์`/`Camp`, `ช่วง`/`Time`, `เต็มวัน`/`Full day`, `ช่วงเช้า`/`Morning (AM)`, `ช่วงบ่าย`/`Afternoon (PM)`; `Student`/`Date` reuse `ob_f_*`). Teacher block per OPEN week under `⏱️TODAY'S SCHEDULE:` — Camp / Date / `Students : n (Full x · AM y · PM z)` / the children ≤12 then `+n` (`CAMP_NAMES_MAX`); parent block per child — Student / Camp / Date / Time. Blocks blank-line separated.
⚠️ Two notes: (1) your §1 "the builder already carries the children" — it did not (TASK-403's teacher row was counts only), so `CampTeacherRow` gained `date` + `names` and `CampParentRow` gained `date`; selection, keys, select unchanged. (2) §0's `DD/MM/YYYY` — the owner's REQ-087 rule (TASK-344's every-date walker, all 19 kinds) is **`DD-MM-YYYY`**, so the camp date renders `05-10-2026` through the one `ddmmyyyy` helper like the class blocks in the same message; say if a slash is really wanted here alone.
✅ Pins from FORM to VALUE: both blocks byte-for-byte in TH and EN, the three halves, 13 ⇒ 12 + `+1`, 12 ⇒ no `+`, two weeks ⇒ two blocks, `Kids` absent (table + renderer), the old keys absent, the builder's `names`/`date`. 🔑 Fourteen mutations bite (byte-identical restores). → report in the TASK and the log. ▶️ Next: whatever waits on the board.

## 2026-09-19 — Porter → @Sober: 🔵 **`REQ-097` (teacher login, own-calendar) — feasibility READ, no build.** The customer wants TEACHERS to log in and see/manage only their OWN calendar. My proposed design extends option C: (1) a user↔teacher LINK so "own" = their assigned bookings; (2) a new grant **`calendar:scope = all | own`** (per user/role); (3) a small teacher self-manage action set. Read: does a user↔teacher link exist or how to add it? can the calendar/people reads filter to "assigned-teacher = me" (primary + a group's swapped teacher + seats)? where does scope=own need to bite? are check-in and teacher-leave reachable as scoped versions of existing actions? Size A (view only) / B (+ check-in own) / C (+ report own leave). The owner is picking A/B/C (I recommend C). No task until his pick + your read.

## 2026-09-19 — Porter → @Sober: ✅ **REQ-097 = option C** (view own + check-in own + report own teacher-leave; create/cancel/move stays admin). Confirmed direction — **deliver your feasibility read + sizes (A/B/C now moot, it's C) and a build plan** (user↔teacher link + `calendar:scope=all|own` + scoped check-in + report-own-leave). I take sizes to the owner, then cut. Flag any decision (e.g. how a teacher account is created + linked; whether report-own-leave reuses the TEACHER_LEAVE cancel reason from REQ-089 §5 era; what a group's swapped teacher sees).

## 2026-09-19 — Porter → @Sober: ✅ **REQ-097 §7 all "เอาตามแนะนำ" — GO C-1 + C-2 (SPEC-083).** Confirmed: admin-created teacher account with a teacher picker; two accounts for an admin-teacher; report-leave = cancel with new `TEACHER_LEAVE` reason (no new status); leave = whole day, per-session un-tick; swapped teacher sees the group from the swap date; NO advance-notice cutoff for a teacher's own leave. Build both stages, one `sid` deploy: `users.teacher_id` link (migration `0044`, fails-closed scope), the `ownScope` predicate (out-of-scope ⇒ 404) on every calendar/bookings read + scoped write (attend only), the report-own-leave act (55th key) via the existing teacher-cancel path + make-ups + the family notice, the new `TEACHER_LEAVE` cancel reason. Contract first; tasks + sizes to me; flag anything new. All on `sid`; later a uat deploy.

From @Jason 2026-09-19: 📋 **TASK-406 CONTRACT in the TASK — §1 verified, ONE premise FALSE, §2 confirmed with four corrections; no code until you confirm.** 🔴 **§1.4: a cancel tells the COACH only** (`class_cancelled_teacher`); NO family notice exists anywhere on a cancel — `enqueueParentCopies` has two callers (the confirm, `confirmCourse`), `cancelSeatsOfGroup` tells nobody, and the `sick_leave` / `leave_teacher` renderers have no producer (dead kinds). The leave cannot reuse what is not there ⇒ I propose a NEW outbox kind `class_cancelled_parent` (20th) on the leave path with PLACEHOLDER labels by form (the TASK-403 precedent), the admin's cancel left coach-only. §1.1: no shared where-builder — the grid/tray (relational callbacks) and `getBookings` (page + its `total` count — two statements) take ONE drizzle condition `ownScopeWhere(me)` (OR-EXISTS on `booking_teachers`) unchanged in both shapes; the by-id reads through `isOwnBooking`; there is no `GET /bookings/:id`. §1.3: the action gate at the route beside `assertMayOverrideLeave`, the row-in-scope 404 inside the service; the public QR is mounted before the auth guard — no scope. §1.5: `END_REASONS` is copied in `validation.ts:371` — I re-point the enum at the set.
🔑 **§2 corrections:** (A) a scoped grid returns only MY column; (B) `status` body's `action ≠ attend` refused at the route (the guard cannot read the body), the set itself in the guard as `TEACHER_ALLOWED` fails closed; (C) the coach notice on a leave goes to the OTHER teachers of the row (a row where I am an extra still has its primary), none when I am alone; the family notice is the new placeholder kind; (D) `REASON_ENUM_REQUIRED` unchanged — the leave writes `TEACHER_LEAVE` on every row it cancels regardless of type.
❓ five: (1) the new family kind, placeholder, admin cancel untouched? (2) my column only? (3) other teachers told? (4) the exact `TEACHER_ALLOWED` read set — I take it from Fern's calendar-page calls; (5) size M + M (the family kind + the 19 → 20 census). ⛔ Waiting on confirm.

## 2026-09-19 — Porter → @Sober: 🔵 **`REQ-098` (archive a PARENT) — read, no build; queued behind REQ-097.** Follow-on to REQ-093: a duplicate leftover PARENT (new phone after a family re-registers) can't be removed; parent SUSPEND still lists it. Proposal: extend REQ-093's archive to parents — `parents.archived_at`, hidden from every parent read (lists, pickers, search, LINE/registration lookup), history+ledger kept, restorable, archivable only when no live future sessions. Read: what does parent suspend do today; is archive feasible reusing REQ-093's shape; should archiving a parent CASCADE-archive its students; every scope bite point. Size it. No task until the owner confirms + your read.

## 2026-09-19 — Porter → @Sober: 🔴 **REQ-094 REOPENED — customer says purple (EXTENDED) STILL not cut at 17:30 on `uat`** (screenshot 5:53 PM, EXTENDED `Private SURFSKATE`). My read: **TASK-389 only made bulk-confirm ACCEPT EXTENDED; the end-of-day job is still CONFIRMED-ONLY**, so a make-up that nobody bulk-confirmed stays EXTENDED and the job skips it — but the customer's intent is auto-cut WITHOUT a manual bulk-confirm. **Two questions:** (1) did today's 17:30 uat run actually use the new build (deploy timing — if the Camp batch/first cutover landed after 17:30, today's run was old and it's moot until tomorrow)? (2) to meet the intent, should the JOB attend EXTENDED directly (start-passed), reversing your earlier "don't put EXTENDED in the job select"? Now that the job is START-based (attends only started classes), is attending a started make-up safe — same as a started CONFIRMED class? Read + recommend; this is a live customer complaint. If it's (2), it's a small job-predicate change. Don't cut yet — tell me which.

From @Jason 2026-09-19: ✅ **TASK-406 DONE (code)** — **2535 pass / 0 fail**, 196 files (+1) · tsc 0 · 🔴 **`0044_user_teacher_link`, 45 = 45** · preflight `0044` and the batch 0038 → 0044 both `[]`. Built as confirmed (all five): `lib/own-scope.ts` — the link is the scope (a linked super admin is scoped too), ONE drizzle OR-EXISTS predicate on the grid · tray · `getBookings` page + count, MY column only, my camp weeks, the by-id reads + the status write through `assertOwnBooking` at the route (404 outside), the public QR unscoped by mount order · **`TEACHER_ALLOWED`** in the guard after menu/action, fails closed — the eight routes from Fern's calendar page (`GET /calendar`, `/bookings`, `/teachers`, `/badges`, `/bookings/:id/checkin`, `/bookings/:id/posted-sale`, `PATCH /bookings/:id/status` attend-only at the route, `POST /teachers/me/leave`); pinned through REAL tokens: a linked user with all 55 keys ⇒ 200 on the eight, 403 SCOPE_TEACHER on the other 95 · users page `teacherId` + `409 TEACHER_LINKED`, DTO `teacherId`/`teacherName`, `/me.teacherId` · **the leave**: `POST /teachers/me/leave` (key 55) — my LIVE rows that date or the named subset, `409 SESSION_DELIVERED` BEFORE the tx, one tx, `CANCELLED` + `TEACHER_LEAVE` + note, seats first, the make-up re-owed, the OTHER teachers' coach notice (minus me), **the FAMILY's NEW `class_cancelled_parent`** (20th kind, `cl_title` PLACEHOLDER by form, ONE producer — the admin's cancel stays coach-only, pinned) · `END_REASONS` 4th + the validator re-pointed at the set.
✅ **Pinned (17 new; 16 files 44 → 45; the kind census 20; the registry 55; the reasons four; the ended-course table; the tray, the family-copies count, the DTO keys, the /me shapes, the DEV user; the Stage-2 exclusion regex ANCHORED — it was unanchored and would have read `/teachers/me/leave` as `/me`).** 🔑 **Thirty mutations bite** (byte-identical restores) — incl. the extras forgotten, 403 instead of 404, the guard line dropped, `POST /bookings`/`GET /students` slipped into the set, each read unscoped, a hand-written predicate, a delivered session cancelled, no tx, the route shadowed, CASCADE, two users one teacher, the admin's cancel telling the family.
📦 Deploy (the human's): `db:migrate` (verify **45**); then the "Teacher" role + the links on the Users page. 🚫 No deploy request. 📌 Contract lines for @Fern (TASK-407) final in the TASK — the allowed set is the page's call list. → full report in the TASK and `log/2026-09-19.md`. ▶️ Next: whatever waits on the board.
From @Fern 2026-09-19: ✅ **TASK-407 DONE (code)** — **478 pass / 0 fail** · tsc 0 · build ok · 16 source modified · 10 tests re-pinned · 3 new · **snapshot 54 → 55** (Jason's slot). ONE gate extended: `can()` under `me.teacherId` passes only `calendar.status` + `teacher-leave` ⇒ every door everywhere hides through the existing gate (value-tested over the whole snapshot). **🔴 The calendar page's load path = Jason's allowed set, walked by the hooks it mounts** (a fifth hook fails; `RentalSection`/`sellable-packages` off under the flag). Scoped calendar = the payload's columns, no pickers; the modal = details + `Check in`; `Report leave` ⇒ ONE call via pure `leaveBody`; Users picker on create + edit (null clears); `TEACHER_LEAVE` labelled on the tray, offered in no dialog (asserted). 📌 **Two assumptions for @Jason via you:** `/me` sits before the scope guard (not in his eight); the login body lacks `teacherId` ⇒ a one-request door flash until `/me` lands. 🔑 ×18 mutations bite, none slipped, md5 identical. Notes + Tanya's screen list in the TASK §2. 🚫 No deploy asked.

From @Jason 2026-09-19: ✅ **TASK-408 DONE (code)** — **2537 pass / 0 fail** (+2), tsc 0, no migration (45 = 45). (1) The hole closed: the `/users*` + `/roles*` scope refusal now sits BEFORE their early exit (`/auth`, `/me`, `/permissions` exempt); pinned through real tokens — a LINKED super admin ⇒ 403 SCOPE_TEACHER on `GET/POST /users`, `GET /users/:id`, `GET/POST /roles`; an unlinked super admin 200; linked accounts still 200 on `/me`, `/permissions`, `/auth/*`. (2) ⚠️ **The login body ALREADY carried `teacherId`** (+ `teacherName`) — `routes/auth.ts` spreads `userDTO(user)`, which gained both in TASK-406 (the auth test's login pin shows them). Pinned by value through the root app and by source (one DTO), nothing re-added; if Fern read "lacks" off a `sid` box, that box predates the 406 deploy. 🔑 Six mutations bite (incl. the line after the exit = dead code, the super-admin exemption, `roles` left open). 📌 One line for @Fern in the TASK (the login fields; hide the users/roles doors when linked). 🚫 No deploy request. ▶️ Next: whatever waits on the board.

## 2026-09-19 — Porter → @Sober: ✅ **REQ-097 complete noted (big one — teacher login).** 📌 **Two items I sent mid-build are NOT yet addressed — pick them up next (reads, no build):**
1. 🔴 **REQ-094 REOPEN** — customer says purple (EXTENDED) STILL not cut at 17:30 on uat. My read: TASK-389 only made bulk-confirm accept EXTENDED; the day-end job is CONFIRMED-only, so an unconfirmed make-up isn't cut — but the customer wants auto-cut with NO manual step. **(a) deploy timing of today's 17:30 uat run; (b) should the job attend EXTENDED directly now it's START-based?** Recommend + size.
2. **REQ-098** — archive a PARENT (leftover duplicate phone); extend REQ-093's archive to parents; read feasibility (what suspend does today, cascade to students, scope bite) + size.
Both queued; the owner is waiting on your read for each. Also still open: send me the REQ-097 family-notice copy DRAFT (gated) for the owner, and I need his §3.7 + reason-line answers.

## 2026-09-19 — Porter → @Sober: 🎯 **REQ-094 REOPEN resolved by the screen — the owner: EXTENDED confirms like PENDING, and the fix is FE, not the job.** Screenshots: a PENDING modal offers `Confirm + LINE`; an EXTENDED modal offers ONLY `Attended` — **no Confirm button**. So "confirm the purple" was impossible on the single modal. ▶️ **Cut the fix: the booking modal for an EXTENDED session offers `Confirm + LINE`** (the BE single-confirm already accepts EXTENDED — Sober's own note; likely FE-only to surface the button). Then day-end (CONFIRMED-only, start-based) cuts it — consistent with REQ-096. 🚫 **CANCEL the "job attends EXTENDED directly" line — do NOT change the job**; the owner ruled a make-up is confirmed first like a new booking. Size (XS FE?), cut, tell me. This closes the customer's "purple not cut" for real.
From @Fern 2026-09-19: ✅ **TASK-409 DONE (code)** — **480 pass / 0 fail** · tsc 0 · build ok. `CONFIRMABLE_STATUSES = ["PENDING", "EXTENDED"]` beside `MOVABLE_STATUSES`, the door at `:717` reads the pure `canOfferConfirm(status)` — value-tested both ways (EXTENDED/PENDING ⇒ door; the other seven ⇒ none); same handler/notice; Attended door untouched. **Also done as you asked (TASK-408 follow-up): the login body's `teacherId` now seeds `/me` (auth → token → session → seed), no flash; pinned.** 🔑 ×6 mutations bite, none slipped, md5 identical. Notes in the TASK §2. 🚫 No deploy asked.

## 2026-09-19 — Porter → @Sober: ✅ **A/B/C all GO.**
**A (REQ-094):** the owner deploys the FE build alone to `uat` now (your call that it's safe stands) — the customer gets the purple Confirm button. Nothing from you.
**B (REQ-097 copy):** APPROVED — ▶️ Jason drops the final bytes (XS). §3.7 = **YES, the shop's own cancels notify the family too** (the one call site); the `Reason` line shows for teacher leave (`ครูลา`) only, hidden for the shop's codes.
**C (REQ-098 archive parent):** ▶️ **GO — build SPEC-084 as recommended:** `parents.archived_at/by` (mig 0045), hidden from every active-parent read incl. LINE lookups + `familyLineUserIdsBulk`, restore view, key 56; KEEP phone; CLEAR the LINE id on archive; CASCADE to students (restore restores only those); coexist with suspend. Contract first; tasks + sizes.
B + C build on `sid` and join the batch; A is the owner's FE-only uat deploy now. Tell me when the 097 bytes + 098 land.

## 2026-09-19 — Porter → @Sober: 🔴 **REQ-097 check 2 (report own leave) = 500 INTERNAL, rolls back — real runtime bug, NOT deploy-lag (Tanya confirmed 55 keys) nor test data.** `POST /api/teachers/me/leave` with ≥1 live own session ⇒ 500, sessions stay CONFIRMED, 0 make-ups (whole tx rolls back). Reproduced 3× fresh tokens; BOTH SINGLE_SESSION and COURSE fail identically; the pre-check works (409 `SESSION_DELIVERED` on an attended day); an admin cancel of a CONFIRMED session ⇒ 200 (outbox infra healthy). Tanya's read: the throw is in `reportOwnLeave`'s tx body (`scheduler.service.ts:2935`), AFTER the CANCELLED update — in the code new to `b72f88b`: `sendClassCancelledToOtherTeachers` and/or the new `class_cancelled_parent` enqueue (not the reconcile make-up branch — both types hit it). **Read the server error and fix — is it a code bug in the new notice path, or a `sid` schema/enum mismatch for the `class_cancelled_parent` outbox kind?** Live-blocking for teacher leave. Also: TASK-405's family-notice bytes (B) are still placeholder — land them with the fix if same area.
📌 **QA housekeeping + a product Q for the owner:** a DISABLED linked teacher account KEEPS its `teacher_id`, so that teacher can't be linked to a new account (Lewis/Kowjoe/Haris now tied to disabled test accounts on `sid`). Is that intended (re-enable is the path) or should disabling free the link? And QA needs 2-3 teachers unlinked to keep testing (re-enable+relink, or a DB unlink).

From @Jason 2026-09-19: ✅ **TASK-410 DONE (code)** — **2540 pass / 0 fail** (+3), tsc 0, 🔴 **`0045_cancel_reason_teacher_leave`, 46 = 46**, preflight `[]` alone + the 0038 → 0045 batch. (1) `0045`: DROP → ADD … `NOT VALID` → `VALIDATE` (the scan under SHARE UPDATE EXCLUSIVE, reads/writes flowing — said in the header); witness = the DEFINITION via a new `constraint-def` probe (`pg_get_constraintdef` contains `TEACHER_LEAVE`) since the name pre-existed. (2) The three copies pinned as ONE: the last migration naming the CHECK has its `IN` list parsed ⇒ `toEqual(END_REASONS)`; the leave's written codes ⇒ `isEndReason`. (3) The owner's family copy by VALUE in TH/EN: `❌ CLASS CANCELLED:`, Student/Program/Date/Time (never the coach), `Reason` ONLY for `TEACHER_LEAVE`, `Note` by shape (course/GROUP ⇒ make-up added; 1-hour/voucher/trial ⇒ hour returned); placeholders gone, no `Kids`. (4) §3.7: ONE exported sender `sendClassCancelledToFamilies` — the leave and the admin's cancel both call it (CONFIRMED-gated, per seat on a GROUP); pinned by value through a fake tx (two devices ⇒ two rows; PENDING ⇒ none; unlinked ⇒ SKIPPED; GROUP ⇒ both families). 🔑 Seventeen mutations bite. 📦 `db:migrate` (verify 46). No shape moved for Fern. ⚠️ SYSTEM-FACTS line for you: the CHECK is the DB copy of `END_REASONS` — a new code = a migration. ▶️ Next: TASK-411's contract.

From @Jason 2026-09-19: 📋 **TASK-411 CONTRACT in the TASK — §1 verified with TWO findings, §2 confirmed with corrections; no code until you confirm.** 🔴 **A:** clearing `parents.line_user_id` alone leaves the ghost resolvable — `familyOfLineUser` reads `family_line_links` FIRST (every account since REQ-079, the primary included). The existing ONE unlinker `clearFamilyLine(parentId, actor, tx)` already does the whole job (link rows + column + rich menu, returns the ids) ⇒ the archive calls it in its tx and stores the list in **`archived_line_user_ids text[]`** (all accounts, audit only); the LINE user then lands on the bot's existing "not linked" path — no new copy there. 🔴 **B:** `parents_phone_uq` — the phone is UNIQUE, so "keep the phone" means: the admin's create with that number ⇒ `409 PARENT_ARCHIVED` (restore instead), and the LINE register flow (find-by-phone → link, else insert) would hit 23505 ⇒ a bot 500 ⇒ needs a new outcome `phone-archived` with a PLACEHOLDER reply (copy via Porter). §1.1: 16 parent-resolving sites listed; ONE `activeParentWhere` at the list/search/count, the phone lookup, the attention + SOM sweeps, `familyLineUserIdsBulk` (a where term — your ❓2), `line-lang`; `assertParentActive` on the by-id writes; the detail 404 unless `?archived=1`. §1.2: REQ-093's count + write lifted into `liveFutureSessionCount(studentIds[])` (one grouped count = the parent's n) + `markStudentArchived(tx)`, shared by `archiveStudent` and the cascade. §1.5: suspend needs no extra term. §2: `0046` three columns + a partial index on `archived_at` (the witness, the restore view's read); archive = count → `clearFamilyLine` → the three columns → every non-archived child `archived_by = "parent:<id>"`; unarchive restores only those, LINE not restored; one key 56 for both doors (the students' precedent). ❓ five: the `text[]` column; the where term; Finding B's two refusals (or a silent restore — advised against); the SOM stops counting archived households; the index as witness. ⛔ Waiting on confirm.

From @Jason 2026-09-19: ✅ **TASK-411 DONE (code)** — **2554 pass / 0 fail**, 197 files (+1) · tsc 0 · 🔴 **`0046_parent_archive`, 47 = 47** · preflight `0046` and the batch 0038 → 0046 both `[]`. Built as confirmed (all five): `lib/parent-archive.ts` — ONE `activeParentWhere` at the list (search + page + count share one where; `?archived=1` = the restore view), the phone lookup, `familyLineUserIdsBulk` (a where term), line-lang, the nag, the SOM; `assertParentActive` at every by-id write (add a student ×2, edit, suspend, clear-link, the LINE claim, a child's own restore); the detail 404 unless asked · REQ-093's count + write lifted (`liveFutureSessionCount` one grouped statement, `markStudentArchived`) and shared · **archive** = 404 · idempotent · the household count ⇒ `409 PARENT_HAS_SESSIONS` before the tx · ONE tx: `clearFamilyLine` (Finding A) → `archived_at/by` + `archived_line_user_ids = the cleared list` → every child `parent:<id>` · **restore** = the two columns cleared (LINE ids NOT restored), only the cascaded children back · **Finding B**: `findArchivedParentByPhone` the one read past the predicate; admin create ⇒ `409 PARENT_ARCHIVED`; find-or-create refuses before the insert; both register doors ⇒ `phone-archived` (chat: `verify_parent_archived` PLACEHOLDER by form; page: `409 PHONE_ARCHIVED`); never a silent restore · key 56 one for both doors.
✅ **Pinned (14 new; 17 files 46 → 47; the registry 56; `actorOf` 16; the ended-course table; REQ-093's three pins re-pointed; ITEM 12's reply census 6):** the migration by text · the predicate by value + by source at every site + no hand-written term (seven files) · the archive/restore by VALUE through a fake tx (refused with nothing written; the unlinker's write first; the audit list; the cascade marks only the active child; idempotent; the restore's selectivity) · Finding B by source + value · the routes through the root app. 🔑 **Twenty-seven mutations bite** (byte-identical restores) — incl. the predicate inverted, page ≠ count, the ghost resolvable, the audit dropped, the cascade dropped/mis-marked, the restore over-restoring or re-linking, every unscoped read, both register doors, a non-partial witness.
📦 `db:migrate` (verify 47). 📌 Contract lines for @Fern (TASK-412) final in the TASK. 🚫 No deploy request. ▶️ Next: whatever waits on the board.
From @Fern 2026-09-19: ✅ **TASK-412 DONE (code)** — **485 pass / 0 fail** · tsc 0 · build ok · 7 source modified · 7 tests re-pinned · 2 new · **snapshot 55 → 56** (`people.parent-archive` after `student-archive`, the BE's slot by position). The family card (the card IS the detail — no separate parent page) gets a red Archive icon by the 56th key ⇒ confirm (students go with it, LINE link cleared) ⇒ ONE `POST /parents/:id/archive`, toast from the server's counts, 409 = the sentence in the dialog. ONE toggle: `Show archived` now also fetches `?archived=1` (only while on, same search) ⇒ dimmed cards + badge + children's names + `Restore` (same key) ⇒ confirm says the LINE link is NOT restored ⇒ ONE `/unarchive`. Nothing reads `archivedAt` on the page (TASK-393's pin holds). 🔑 ×12 mutations bite, none slipped, md5 identical. Notes + Tanya's list in the TASK §2. 🚫 No deploy asked.

## 2026-09-20 — Porter → @Sober: ✅ **phone-archived bot reply APPROVED** — "เบอร์นี้เคยลงทะเบียนไว้แล้ว กรุณาติดต่อร้านเพื่อคืนสถานะ" / "This number was registered before — please contact the shop to restore it." Land the final bytes (drop the placeholder). The `disable-frees-teacher-link` product question stays on the list (not blocking — QA self-serves the unlink on sid). Everything else in the batch is built; awaiting the owner's sid restart, then Tanya's full round (097 check-2 re-run + 098), then the uat single deploy.

## 2026-09-20 — Porter → @Sober: 🔵 **`REQ-099` (People birthday filter) — read + size, no build.** Owner-confirmed: on the People page, filter students by **birth MONTH RANGE** (e.g. born in Sep, for birthday promos — ignore year) AND a separate **"no DOB recorded"** filter (DOB is optional; find the blanks to chase). Read: does the People/students query expose DOB + can it filter by month-of-birth range + is-null server-side; where the control sits among the existing People filters; FE+BE size. Small expected. No task until your read + I confirm scope with the owner if anything's open.

From @Jason 2026-09-20: ✅ **TASK-413 DONE (code)** — **2554 pass / 0 fail**, tsc 0, no migration (47 = 47). `verify_parent_archived` carries the owner's bytes (TH `เบอร์นี้เคยลงทะเบียนไว้แล้ว กรุณาติดต่อร้านเพื่อคืนสถานะ` / EN `This number was registered before — please contact the shop to restore it.`), the PLACEHOLDER comment gone; the pin by VALUE both languages + the old words pinned absent + the webhook's branch. 🔑 Four mutations bite (both placeholders back, EN missing, the branch re-mapped). ⚠️ One unrelated pin repaired: the commit stored `0044_user_teacher_link.sql` with CRLF and the REQ-097 header pin read raw bytes — the read is normalised, the pin unchanged, no product file touched. 🚫 No deploy request. ▶️ Next: whatever waits on the board.

## 2026-09-20 — Porter → @Sober: ✅ **REQ-099 scope confirmed — GO.** Suspended households EXCLUDED (keep the searchStudents rule); archived children EXCLUDED by default. Cut it: BE (`birthMonthFrom/To` 1..12 + `noDob`, `birthDate` in the select, wrap-around helper pinned) + FE (Birthday control beside the People filters + student result list name·nickname·DOB/—·phone, sorted by birth day). No migration, rides `menu:people`. Build on `sid`; it joins the pending batch's next `uat` deploy (along with the TASK-413 phone-archived bytes). Tell me when in.

From @Jason 2026-09-20: ✅ **TASK-414 DONE (code)** — **2561 pass / 0 fail**, 198 files (+1), tsc 0, no migration (47 = 47), no key; nothing differed from your §1. `lib/birth-month.ts` pure (`birthMonthWhere` BETWEEN / `>= OR <=` on the wrap, `noDobWhere`, `wrapSortKey` + `birthMonthOrder`, `withBirthdayFilter` composing WITH the base) · `studentsQuery` + the three fields with the two `400`s · `searchStudents` + `birthDate` in the select/DTO, the filter OVER search ∧ archived ∧ suspended, the wrap order only on a range. ✅ Pinned (7): the three SQL shapes by `toSQL` with params, the month table, the wrap key + ORDER BY text, the composition, the validator both ways, the source (REQ-093/TASK-058 terms untouched and under the filter), the root app (the parsed args reach the service; the two 400s before it). 🔑 Thirteen mutations bite (the wrap gone/AND, the key unwrapped, the base replaced, noDob/range precedence, the select, the order, the validator's three). 📌 Contract lines for @Fern (TASK-415) final in the TASK — none beyond §1. 🚫 No deploy request. ▶️ Next: whatever waits on the board.
From @Fern 2026-09-20: ✅ **TASK-415 DONE (code)** — **491 pass / 0 fail** · tsc 0 · build ok · 5 source modified · 3 new · snapshot unchanged (56, no key). Pure `birthdayQuery` value-tested (both months OR `noDob: "true"`, never both; `q` carried; a half range ⇒ null); `formatDob` ⇒ `DD-MM-YYYY` / `—`. The `Birthday` popover beside search + `Show archived` (month names per language, the switch greys both selects, `Clear`); while set the families are REPLACED by the server's student list from the SAME `GET /students` (fetched only while set) — no `.sort`/`.filter`, no month arithmetic on the page (asserted; a client month filter fails). 🔑 ×11 mutations bite (one slipped first — the switch greying one select — pinned to both), md5 identical. Notes + Tanya's list in the TASK §2. 🚫 No deploy asked.

## 2026-09-20 — Porter → @Sober: ✅ **REQ-099 — add YEAR (optional, blank = any year)** — extend TASK-414/415: optional year on From/To; blank = month-only any-year (current), filled = a real DOB date range. Small. Cut it.
## 2026-09-20 — Porter → @Sober: 🔵 **`REQ-100` (voucher cancel — READ):** owner clarified it's NOT a whole-voucher cancel — just a **Cancel on a VOUCHER booking placed on the schedule** (with a reason, like a course session's cancel), NO pause, remaining entitlement untouched. **Read: does a scheduled voucher booking already have Cancel (does REQ-009's 1HR/Voucher cancel cover a voucher session ON the schedule, or only the create path)?** If a gap, add it (existing cancel reasons, no pause). Size it. No task until your read.

## 2026-09-20 — Porter → @Sober: 🔴 **REQ-095 CORRECTION (REQ-095 §11) — Camp must render ON the teacher schedule grid, not just the day banner.** I re-read the source with the owner: Camp is a SCHEDULE that books teachers — "มีการสลับครูตลอด สามารถย้ายไปมาได้ ตัดคอร์สเป็นวัน ... ตารางจะอยู่ในสัปดาห์นั้นๆ"; ECA/Camp intent = "จองตารางครู ฟิกเหมือน Private แต่ 1 คลาสหลายคน". Stage 3a's banner + `§8.3 informational` is too weak — the customer expects the camp on the teacher grid (their "ลง camp ขึ้นตาราง กดตรงไหน"). **READ + size: does the camp object hold a teacher PER DAY today (I think not — it was informational)? To render a camp day as a BLOCK in the assigned teacher's column (camp time, e.g. 10:00-15:00), swappable teacher day-to-day, cut by day — what's needed (assign-teacher-per-day + the grid render + the swap), and does it hard-block the slot or overlay (§8.3 nuance)?** My §8.3 "informational" recommendation was the miss — §8.3 superseded. No build until your read + sizes; flag any owner decision (block vs overlay; default teacher).

From @Jason 2026-09-20: ✅ **TASK-416 DONE (code)** — **2565 pass / 0 fail** (+4), tsc 0, no migration (47 = 47), no key; exactly your §1. The dated branch: `birthDateRangeWhere` = `birth_date BETWEEN make_date(yf, mf, 1) AND (make_date(yt, mt, 1) + 1 month − 1 day)` (the whole To month, no wrap), `birthDateOrder` = date then name; **ONE decision `birthdayMode`** read by both the where and the order (noDob › date › month › none); without years TASK-414 is byte-unchanged. The validator's five 400s pinned by value. ⚠️ One simplification: the "noDob with years" clause was redundant (years imply months by the rule above) — dropped with the reason. 🔑 Thirteen mutations bite — five passed first, all closed honestly: three pin gaps (noDob vs a dated range; the base surviving beside the dated where; a lone TO year), one refine that hid another behind a `NaN` comparison (now guarded on months so each rule is the one that refuses), one redundant clause removed. 📌 One line for @Fern in the TASK. 🚫 No deploy request. ▶️ Next: whatever waits on the board.
From @Fern 2026-09-20: ✅ **TASK-417 DONE (code)** — **494 pass / 0 fail** · tsc 0 · build ok · 4 source modified · no key. `birthdayQuery` by value: both years ⇒ `birthYearFrom/To` beside the months; ONE year ⇒ not a query (`birthdayActive` false); none ⇒ the month-only query byte-identical; the switch still wins. 🚫 No client `from ≤ to` rule — a backwards pair is sent, the server's TH 400 shows under the year boxes (`error` from the list query). A `Year` box under each month, greyed under the switch; the button reads `Sept 2018 → Feb 2019` (rendered). 🔑 ×8 mutations bite (one equivalent by value — the guard masks the builder — pinned by source, stated), md5 identical. Notes + Tanya's list in the TASK §2. 🚫 No deploy asked.

## 2026-09-20 — Porter → @Sober: ✅ **Camp-on-grid (SPEC-085) — all 4 decisions confirmed, GO.** (1) HARD-BLOCK via real rows; (2) one block/day = camp window default 10:00–15:00, EDITABLE per week (kids' AM/PM stays the package's); (3) default teachers = the week's list, no-teacher day ⇒ no block; (4) a lesson already in the window ⇒ REFUSE naming the clash (camp doesn't overwrite). Cut SPEC-085 A (the block: `camp_week_days` teachers+window per date, real OTHER/CAMP rows, swap, migration 0047⇒48) + B (reminder collapsed to one camp line/day). Contract first; tasks+sizes. REQ-100 = no build (already exists). REQ-099(+year) done. Camp-grid joins the pending `uat` batch. Tell me when in.

From @Jason 2026-09-20: 📋 **TASK-418 CONTRACT in the TASK — §1 verified with three findings, §2 confirmed with corrections, your two ❓ answered; no code until you confirm.** 🔴 **A:** `insertBooking` births rows PENDING — a PENDING camp row would never reach the coach's reminder (REQ-096 CONFIRMED-only) ⇒ CAMP rows born `CONFIRMED`; `head_count` NULL (the kids are `camp_days`); OTHER rows draw no freelance hold and post no money by construction (verified in `reconcileBookingHolds` / `postOtherBookingSale`); a coach's off day is refused by `assertTeacherBookable` (no override); the three human validators would accept `CAMP` by hand ⇒ `HUMAN_OTHER_KINDS`, CAMP only via the sync. 🔴 **B:** there is NO dates edit on a week today (`updateCampWeek` has no start/end) and the kids' days reference the dates ⇒ dates immutable in this task. **C:** `swapGroupTeacher` lends its shape (per-row check, one tx, 409 naming the clash), not its code — the per-day swap IS the ONE `syncCampDayRows` (wanted set vs existing, insert/delete). Your ❓1: hard delete (derived rows; a CANCELLED camp hour would pollute the tray and the counts). Your ❓2: `edited_at` on the day row — a week-level change re-derives only un-edited days. The reminder fold at the BUILDER (rows of one coach/date/week → one OTHER block with the merged time — no new copy). `0047`: `camp_week_days` + `bookings.camp_week_day_id` + the partial index (witness) + `camp_weeks.window_start/end`; ⚠️ the index on `bookings` blocks writes for its one scan (said in the header). `CAMP_ROW_OWNED` on EVERY row write, pinned as a classification table. ❓ five: CONFIRMED-at-birth + no roster override; dates immutable; hard delete; `edited_at` rule; the fold as the existing block vs a literal one-liner. ⛔ Waiting on confirm.

From @Jason 2026-09-20: ✅ **TASK-418 DONE (code)** — **2583 pass / 0 fail**, 199 files (+1) · tsc 0 · 🔴 **`0047_camp_week_days`, 48 = 48** · preflight `0047` and the batch 0038 → 0047 both `[]`. Built as confirmed (all five): `camp_week_days` + the week window + `bookings.camp_week_day_id` + the partial index (witness; the SHARE-lock scan on `bookings` named in the header) · `CAMP` the 4th kind with `HUMAN_OTHER_KINDS` for the three human validators · pure: the window rules, `wantedCampSlots`, `campSlotDiff`, `foldCampRows` · **ONE `syncCampDayRows`** (hard-delete the surplus; insert the missing through the ONE `insertBooking` as OTHER/CAMP/CONFIRMED, link to the day; a clash ⇒ 409 naming date·hour·nickname, propagates ⇒ the caller's tx rolls back) called by createWeek (one tx), updateWeek (CLOSED ⇒ rows gone; OPEN ⇒ re-sync; a week-level change ⇒ un-edited days only) and the per-day swap `PATCH /camp/weeks/:id/days/:date` (stamps `edited_at`) · the reminder fold in the builder (five hours ⇒ one `Camp A` / `10:00-15:00` block) · no money by construction. ⚠️ **One correction to my own §2:** `CAMP_ROW_OWNED` moved from the routes INTO the eleven services (on the row already in hand — no extra read, and the route-level read broke every spied route test); pinned as a table of eleven. ✅ Pinned (18 new; 18 files 47 → 48; the kinds, the relation set, the camp access rows re-pointed): the migration by text · the kinds · the pure rules by value · the SYNC by VALUE through a fake tx (insert/link/delete/clash/closed/no-teacher) · the lifecycle by source · the guard table · the fold through `groupReminders`. 🔑 **Thirty mutations bite** (byte-identical restores). 📦 `db:migrate` (verify 48). 📌 Contract lines for @Fern (TASK-419) final in the TASK. 🚫 No deploy request. ▶️ Next: whatever waits on the board.
From @Fern 2026-09-20: ✅ **TASK-419 DONE (code)** — **505 pass / 0 fail** · tsc 0 · build ok · 13 source modified · 3 tests re-pinned · 4 new · snapshot unchanged (56). Pure `mergeCampCells` by value (10–15 ⇒ ONE block of 5 with rows kept; a gap / another day object ⇒ two; ECA never merges; `isCampRow` = kind AND `campWeekDayId`); both grids mount ONE shared `CampBlockCell`, the day grid spans the covered rows. 🔴 A CAMP row never reaches `BookingModal` — `openView` routes to the camp panel first (pinned by order). Panel: the day's teachers from the roster's day object, `Swap teacher` by `camp.week-open` (hidden) ⇒ ONE per-day PATCH (pure `replaceTeacher`), 409 = the sentence. Editor: window by presence, dates locked, per-day table ⇒ ONE PATCH per CHANGED day only (pure `changedDayPatches`), `Apply to every day`; no window rule on the FE. CAMP tag beside `OTHER_KINDS` (still three). 🔑 ×19 mutations bite (one slipped first — a client window gate — pinned), md5 identical. Notes + Tanya's list in the TASK §2. 🚫 No deploy asked.

## 2026-09-20 — Porter → @Sober: 🔬 **ANALYSE, don't build (owner: no solo decision) — DUO re-spec (REQ-095 §13).** The customer restated DUO = a Private course in every way EXCEPT an editable coach RATE not tied to the teacher's hourly rate; 2 kids "use the SAME course"; UX = a Private/DUO tab on New course + a rate box; rate editable on Move-session too; leave/expiry = Private. **Report:** (1) what the CURRENT Stage-2a DUO actually does — is each kid a separate course/entitlement or shared; how leave/expiry/quota/deduction behave; where the rate lives (per-teacher backoffice vs a class rate); (2) how it diverges from the re-spec (UX, 1-vs-2 courses, rate surface); (3) the 1-shared-entitlement vs 2-entitlements question — is a truly-shared single entitlement even coherent with per-student attendance/quota, or does each kid still need own hours (the earlier "คนละ 10 Hr")?; (4) if we rework to the New-course Private/DUO toggle + rate box, the size + what happens to the on-uat Stage-2a DUO/Group object (retire DUO, keep Group?). No task — analysis for a 3-way discussion.

## 2026-09-21 — Porter → @Sober: 🟢 **DUO RE-SPEC RESOLVED — BUILD IT (supersedes the "analyse" note + your converged 2-linked-courses finding).** Owner asked the customer the contradiction directly; she LOCKED **B = ONE shared course/entitlement for two kids**. Full locked spec is in `REQ-095 §13.1/§13.2`. Key points:
1. **ONE course, ONE entitlement, TWO kids on it** — shared hours pool, shared leave quota, shared expiry; any kid's leave/attendance deducts the shared pool. (Confirmed verbatim: "1 คอร์ส 2 คน = ชั่วโมง/โควตาลา/วันหมดอายุ ก้อนเดียวร่วมกัน" → "ใช่ค่ะ".)
2. Both kids attend the same class every session — fixed day/time/teacher, Private-style. Create as ONE Private course carrying both names inline.
3. **Editable ค่าสอน (coach rate)** on the course — not tied to teacher hourly rate; stays editable after teacher swap/Move-session.
4. UX: New-course **Private / DUO** toggle; DUO ⇒ rate box + second-child picker.
5. Retire/migrate the current Stage-2a "2 linked courses" DUO. GROUP (3+) untouched.
6. **Canonical example:** course "Duo Inline Skate 10 Hr", students "น้องคราม & พราว" — 10 Hr shared.
**READ + spec + size** the rework (schema change from 2-courses→1-course-2-students; leave/expiry now shared; the toggle + rate box; migration for existing on-uat Stage-2a DUOs). Flag any owner decision. Contract first, then tasks+sizes — no build until you confirm the contract.

From @Jason 2026-09-21: 📋 **TASK-420 CONTRACT in the TASK — §1's TABLE (16 sites, line numbers, argument vs row), four findings, §2 with corrections, your three ❓ answered; no code until you confirm.** 🔴 **A — no chokepoint exists today:** three sites write a course row — `insertBooking` (the four planners) and two direct clones (the reconcile's EXTENDED append `:2728`, the sick-leave make-up `:3560`), each calling `inheritCourseRental` per writer (the REQ-091 shape) ⇒ I propose `co_student_id` rides through `insertBooking` (reads the course's column once when `courseId` is set — the planners change nothing) and the two clones copy it from the row they already clone; pinned as a census of the three `insert(bookings)` sites. 🔴 **B — the LIFF leave flow:** `line-webhook.service.ts:942` authorizes by `b.studentId === id` and `childrenWithSessions` keys on `studentId` ⇒ child 2's family cannot take leave on a DUO session at all — needs the co-student (❓2: in scope?). **C — a FOURTH private "family accounts" read** lives in `course-deduction.ts:185` beside the three `parentLineUserIds` sites ⇒ ONE `householdLineUserIds(exec, ids[])` union helper (your ❓2 — a helper, de-duplicated, primary-first) retires all four. **D — the price:** no second mapping — `resolvePriceGroup(subjectId, duo ? "DUO" : courseGroupKind)` reuses `GROUP_KIND_PRICE_GROUP.DUO`. Your ❓3 (SOM): "active students" counts both children (the three `b.studentId` maps); courses per course unchanged. Correction A: `duo` + `groupKey` ⇒ 400. The reminder builder emits a DUO row once per household (both names on the coach's line). ❓ four: the chokepoint shape; the leave flow in scope; the 4th copy retired; duo + groupKey refused. ⛔ Waiting on confirm.

From @Jason 2026-09-21: ✅ **TASK-420 DONE (code)** — **2630 pass / 0 fail**, 200 files (+1) · tsc 0 · 🔴 **`0048_duo_course`, 49 = 49** · preflight `0048` and the batch 0038 → 0048 both `[]`. Built as confirmed (all four): the chokepoint = `insertBooking` reads the course's `co_student_id` once (the create passes it) + the two clones copy their template — **the `insert(bookings)` census is pinned by scan** (three sites, each classified; a fourth fails as UNCOVERED); Finding B — the LIFF leave authorizes primary OR co-student and the child picker offers THIS family's children only (`own` = the linked ids); the FOUR private family reads retired into ONE `householdLineUserIds` (union, de-duplicated, primary-first — by value; by source none remains); `duo`+`groupKey` ⇒ 400. One sale at `course-balance-duo-{size}` (refId = the course, by value); the rate stored on the course, editable from the course PATCH and the session move, Private ⇒ 400 NOT_DUO, never posted; confirm / cancel / deduction / confirmCourse reach both households; the reminder once per household with `A & B`; CRM to both; the 16-site reader table pinned by `test.each`; group DUO retired for creation only. 40/40 mutations bite (one pin gap closed: the shared-device de-dup). 🟡 Two facts for you in the TASK: (1) a leave on a DUO row is ONE leave for both families — the other family gets no notice of it (new placeholder kind if wanted — Porter's); (2) `familiesNotified` counts batches (a DUO row = 1). @Fern's contract lines final in the TASK. 🚫 No deploy request. ⛔ Your DONE.
From @Fern 2026-09-21: ✅ **TASK-421 DONE (code)** — **515 pass / 0 fail** · tsc 0 · build ok · 12 source modified · 1 test re-pinned · 3 new · snapshot unchanged (56). ONE pure `studentLabel` (a Private byte-identical; a DUO `A & B`) applied in the ONE mapper ⇒ every `displayName` reader + the course card show both names with no call-site edit. New course: Private/DUO toggle (hidden inside a group) ⇒ second child + baht rate; the DUO card by its group name `balance-duo` (no number on the FE); `duo` only when on, never beside a `groupKey`. Move: the rate box on a DUO session only, sent only when changed (pure `rateChange`). Course card: `DUO` tag + rate line, pencil by `course-edit` ⇒ PATCH only when changed. `Create group`: Group only. 📌 Two readings stated in §2: no course cards on the People page (the course list is one list the server searches by either child); the plan title reads `coStudent` if the plan reader sends it — @Jason can confirm in one line. 🔑 ×19 mutations bite (one slipped first — pinned), md5 identical. Notes + Tanya's list in the TASK §2. 🚫 No deploy asked.

From @Jason 2026-09-21: ✅ **TASK-422 DONE (code)** — **2634 pass / 0 fail** (+4), tsc 0, 49 = 49. The three DUO facts extracted into ONE `duoCourseFacts(c)` in `db/mappers.ts`; `toCourseWithStudent` and `getEntitlementPlan` both spread it (the plan's course read loads `coStudent`); by value: DUO ⇒ both refs + rate + "DUO", Private ⇒ null · null · "PRIVATE"; voucher branch untouched by absence; the census +3 rows; 4/4 mutations bite (incl. a hand-built copy in the plan). ⚠️ shape note in the TASK: the plan's `coStudent` is the mapper's ref (a superset of the plan's thin `student` ref) — say if you want them byte-equal. Fern's line in the TASK. ⛔ Your DONE.

## 2026-09-21 — Porter → @Sober: 🔴 **RATE MODEL CORRECTION (REQ-095 §13.3) — the coach ค่าสอน must be PER-SESSION, not course-wide. Found via Tanya + a customer clarification; it changes SPEC-087's rate behaviour.**
Customer: ค่าสอน is edited class-by-class. Example: whole course = ครู A @ 7 baht; one week ครู A is out and ครู B substitutes ⇒ **that one session = 3 baht, the course stays 7**.
- Current build: the Move-session rate box (and the DUO rate box) rewrites the **course-level** `classRateMinor` (Tanya confirmed `PATCH /bookings/:id {classRateMinor}` + the modal text "changes the course's rate") ⇒ it changes EVERY session, not just the substitute week. DIVERGES.
- Wanted: **course DEFAULT rate + per-SESSION override on the booking.** Session popup edits THAT session only; the course card edits the default; effective rate = session override ?? course default; freelance/sale/report reads use the effective rate.
- **READ + size.** Also say whether this **holds the `uat` deploy** (rate is a money/report field — a wrong course-wide rate on `uat` could misprice real sessions) or is a safe fast-follow after `uat`. Contract first, no build until confirmed. Applies to DUO + any course with a coach rate.

## 2026-09-21 — Porter → @Sober: 🟢 **GO — BUILD §13.3 (per-session coach rate) to completion. Owner: finish it; the `uat` deploy WAITS and ships together with this.**
Scope for this build:
1. **§13.3 rate model:** course DEFAULT rate + per-SESSION override on the booking. Session popup / Move-session box edits THAT session only (the substitute week); the course card edits the default; effective rate = session override ?? course default; freelance/sale/report reads use the effective rate. Relabel the modal (it currently says "changes the course's rate"). Applies to DUO + any course carrying a coach rate.
2. **Fold in the Bookings-table names fix (Tanya's minor note):** `/api/bookings` (the Bookings table surface) returns `coStudent:null` + single `displayName` for a DUO session while `/api/calendar` carries both — make the table surface carry both names too ("คราม & พราว" wherever a child is named).
Contract first, then tasks+sizes; then BE/FE as usual. When it's in and you've re-run green, the FULL batch (095 incl §13 DUO + §13.3 + this fix · 096 · 097 · 098 · 099+year · 094) goes to `sid` for Tanya's re-check, then the single `uat` deploy. Nothing to `uat` until then.

From @Jason 2026-09-21: 📋 **TASK-423 CONTRACT in the TASK — no code.** §1.1: every reader of `teacher_rate_minor` listed (7 rows) — `otherFacts` is type-gated (`!== "OTHER" ⇒ null`), `groupFacts` GROUP-only, `editOtherBooking`/`seatOnGroup` OTHER/GROUP-only, the jobs never read it, no pin asserts NULL on a course row ⇒ **REUSE, no migration (49 = 49)**; the NULL-means-inherit reading lives only in `effectiveRateMinor`. §1.2: `getBookings` gains a `co_students` alias join (+ I propose its `q` also matches the co-student). 🔴 **Finding A: `displayName` has THREE copies** (the DTO — so the calendar shows one name on a DUO row too, not only the table —, the reminder, `describeSlotClash`) ⇒ ONE `displayNameOf(b)` in mappers, all three call it. §1.3: no writer copies the course rate onto a row; a make-up of a substituted session is born with the DEFAULT (say if the owner reads it otherwise). §2 confirmed with two corrections: the body key stays `classRateMinor` on both routes; the DTO's `rate{…}` is non-null on COURSE_PACKAGE rows only (OTHER/GROUP keep `other.teacherRates`, `rate: null`). ❓2: popup body only — yes. ⛔ Your confirm (reuse · Finding A · the `q` widening) before code.

From @Jason 2026-09-21: ✅ **TASK-423 DONE (code)** — **2647 pass / 0 fail** (+16), tsc 0, **no migration (49 = 49)**. `lib/coach-rate.ts`: `effectiveRateMinor` + `rateFacts` (the ONE spelling — a src scan pins no second); the DTO's `rate{…}` on COURSE_PACKAGE rows only (an OTHER row keeps `other.teacherRates`, `rate: null`; `otherFacts`'s gate pinned by mutation). The session write ⇒ the ROW's `teacher_rate_minor` (`null` clears; no course ⇒ 400 `NOT_A_COURSE_SESSION` before the tx); the course write ⇒ the default, any course (`NOT_DUO` gone); overrides survive a default change; new rows born NULL (the clones pinned). `getBookings`: `co_students` alias LEFT-joined, `coStudent` on the row, `q` on either child. Finding A: ONE `displayNameOf` — the DTO, the reminder and BOTH clash sentences (there were four copies, not three) call it; the chain exists once in src (scan-pinned). 19/19 mutations bite. One question: the course body refuses `null` (the default is never cleared) — say if it should be clearable. Fern's lines final in the TASK. ⛔ Your DONE.
From @Fern 2026-09-21: ✅ **TASK-424 DONE (code)** — **516 pass / 0 fail** · tsc 0 · build ok · 9 source modified · snapshot unchanged (56). `displayName` carried straight through again (the server joins `A & B`); `studentLabel` and the server's string agree — pinned by value. Pure `rateTag` (`overrideMinor` alone; an override equal to the default is still an override) + `sessionRateChange` (a body only when typed ≠ EFFECTIVE; Clear ⇒ null only on an override; ฿0 is a rate); no `override ?? default` on the FE (asserted). The popup: the box on any COURSE row, relabelled *This session's rate (default|override)*, the hint names the default, Clear on an override only. The card: *Default coach rate* on any course, PATCH only when changed, never null. Bookings table reads `displayName`. 🔑 ×12 mutations bite (two slipped first on coincident fixtures — diverged, pinned), md5 identical. Notes + Tanya's list in the TASK §2. 🚫 No deploy asked.

## 2026-09-21 — Porter → @Sober: 🔴 **DEFECT — the LINE NOTICE still shows ONE name. §13.3 rate ✅ and the Bookings-table both-names ✅, but the family-facing LINE messages were NOT reached. uat gate HELD until this is folded in.**
Tanya, on the demo OA, created a fresh DUO (KKTEST + coStudent Prao) and confirmed a session AFTER the deploy. The delivered notice read: `Student : KKTEST` only — not "KKTEST & Prao". Round-54's leave notice was likewise single-name.
- Diagnosis (Tanya's + confirm): the **LINE notice-builder uses its own text path**, separate from the DTO `displayName` you folded for the table — so the confirm + leave/make-up notices still name only the primary child. SPEC-087 "A & B everywhere" is NOT met on the family-facing LINE messages.
- **Fix:** route the notice-builder's student name through the same two-names source (confirm, cancel, and the make-up/leave notices). Tanya's suggestion (your call): lead each household's copy with THAT household's own child, but the pair must be shown. Nothing else in the batch changed — §13.3 rate + Bookings table both verified PASS on `sid`.
- Contract/size as you see fit (looks S–M, one notice-name path). Re-run green → back to `sid` for Tanya's LINE re-capture on the demo OA → then the single `uat` deploy. **No `uat` until the notice carries both names.**

From @Jason 2026-09-21: ✅ **TASK-425 DONE (code)** — **2659 pass / 0 fail** (+12), tsc 0, 49 = 49. `studentNamesOf(b)` (the STUDENT part of the ONE rule; `displayNameOf` now composed of it — one chain in src) feeds the worker's `bookingContext` (by value: DUO `KKTEST & Prao` · Private one · OTHER no studentName — the title stays in `program`, which is why not `displayNameOf` itself: it would print the title twice) and the two payload writers (leave notice reads the co-student; `confirmCourse` rows load it). Stable order `student & coStudent` on every copy (no cheap household-at-render; one LINE id can belong to both). The census found THREE more one-name sites — the check-in QR payload, the teacher's ICS, the teacher's LINE schedule — folded in the same pass (say if any should not have moved). Census pin: every `studentName:` in src goes through the rule, per-child sites on a named allow-list. Six renderers by value with a DUO context. 16/16 mutations bite. ⚠️ Flagged: the rule is nickname-first (the DTO's since TASK-224) — a Private child WITH a nickname now reads by nickname on notices (KKTEST has none; the capture is unchanged); say if the owner wants full names there. ⛔ Your DONE; `sid` for Tanya's re-capture.

## 2026-09-21 — Porter → @Sober: 🔬 **TWO new customer requests — ANALYSE + size, no build yet. Full detail in the REQ files.**
- **REQ-101 (ECA manage-plan):** add a **Manage-plan page** for ECA like camp/course · whole-course **Confirm** · **notify the teacher on edit/move/remove** (like a normal course) · **Cancel-all** (in case of a wrong entry). Customer's QUESTION: to add a teacher to an existing ECA today, must the admin create a brand-new Other schedule, or can a Manage-plan page carry an **Add-teacher** button onto the existing one? (Her preference: the Add-teacher button.) Read the current ECA/Stage-1 model, say whether a schedule can hold several teachers, and whether this generalises to Free/KOL. Sizes per piece.
- **REQ-102 (RBAC money visibility):** an RBAC permission controlling who can SEE money figures, incl. the **Freelance salary ceiling** (`freelance_budgets`, REQ-009 lineage). Is there a money-visibility key today? Which in-scheduler surfaces show money (vs backoffice-only per §4.1)? ONE "view money" key vs granular — recommend, fail-closed default. Sizes.
Contract/analysis for a 3-way; flag owner decisions. No dispatch to engineers until the owner rules.

## 2026-09-21 — Porter → @Sober: 🟢 **GO BUILD — REQ-102 FIRST, then REQ-101. Owner rulings recorded in the REQ files.**
**REQ-102 (build first — small + closes a live leak) — SCOPE NARROWED by the owner:** the ask is ONLY the **Freelance salary ceiling / remaining** (`freelance_budgets`) on the frontoffice. ONE narrow permission key gating that figure (fail-closed, nobody by default, Teacher role never holds it) + **close the `GET /teachers` leak** (it ships every coach's budget to a REQ-097 teacher account). **OUT of scope:** price cards, discount block, posted-sale reads, the §13.3 coach rate, the reports — do NOT mask those. BE S–M · FE S · no migration.
**REQ-101 (after 102):** all four §A3 decisions = your SPEC-088 recommendations: (1) Cancel-all behind its own key `other-cancel-all`; (2) Add-teacher from today forward; (3) backfill existing ECA rows with the series key (one-off script); (4) notice words = Porter's TH+EN draft in REQ-101 §4 (owner may tweak bytes later). Build: series key (`0049` ⇒ 50), Manage-plan page (Confirm all · Cancel all · Add/Remove/Swap teacher · Add dates · edit title/kind/heads, each teacher change slot-checked one tx, clash refuses naming it), two teacher-notice placeholder kinds, generalises to Free/KOL. BE M+ · FE M.
Contract first on each; then Jason/Fern. Both join the next `sid`→Tanya→`uat` cycle. Flag anything that needs the owner.

From @Jason 2026-09-21: 📋 **TASK-426 CONTRACT in the TASK — no code.** §1.1: a 10-row table — the frontoffice's ceiling figures come from ONE builder (`attachFreelanceBudgets` :326) into THREE readers (the calendar's `teachers[]` — incl. a linked account's own column; `GET /teachers`; `loadTeacherFull` behind six writers); `setLimitOverride`/`setTeacherWorkDays`/work-days impact/`toTeacherBase` carry no figures; no report reads a ceiling (❓1 answered: none); the old ops attach is dead. One figure OUTSIDE the builder: the attention line `เหลือ N ชม.` (dashboard + LINE admin digest). §1.2: the booleans drive not-bookable; `assertTeacherBookable` reads the DB, never the DTO. §1.3: `RouteAccess.action` becomes `key | key[]`, the guard checks `every`. **Structural correction:** the builder has no user — `attachFreelanceBudgets(dtos, viewer)` with a REQUIRED viewer (null ⇒ masked, fail closed), threaded from 8 route lines. Three ❓: (1) mask the attention label without the key (digest unchanged)? (2) `hourlyRate` (the pay rate) — mask with the same key (my proposal) or keep? (3) explicit viewer parameter (my preference) vs a request context. ⛔ Your confirm before code.

From @Jason 2026-09-21: ✅ **TASK-426 DONE (code)** — **2670 pass / 0 fail** (+11), tsc 0, 49 = 49, key **57**. `lib/budget-visibility.ts`: `canSeeBudget` (the key, never a linked account, null ⇒ no) + `maskBudget` (the four figures ⇒ null, the booleans stay) + `viewerOf(c)`; `attachFreelanceBudgets(dtos, viewer)` — the viewer REQUIRED, the mask inside; the three readers + six writers thread it (9 route lines). `RouteAccess.action` is `key | key[]`, the guard requires every one; the two budget writes need both keys (by value: budget alone ⇒ 403). The leak by value through the root app (linked-with-57 ⇒ nulls on `GET /teachers` and `/calendar`). The near-cap label drops its number without the key (`figureless`, stripped either way; the digest never calls the mask). `hourlyRate` masked. Nothing else touched (by absence). 16/16 mutations bite. Fern's lines final in the TASK. ⛔ Your DONE.
From @Fern 2026-09-21: ✅ **TASK-427 DONE (code)** — **521 pass / 0 fail** · tsc 0 · build ok · 4 source modified · 11 tests re-pinned · 2 new · **snapshot 56 → 57** (`teachers.budget-view` after `teachers.budget`, the BE's slot by position). ONE pure `moneyOrDash`/`minorOrDash` (null ⇒ `—`; ZERO ⇒ `฿0`); the walk scans every component for a `฿{… ?? 0}` print — three readers routed (the Teachers card's rate + remaining/budget lines, the calendar strip); the card's block shows for every freelancer (masked ⇒ `— / —`, never hidden), the badge + override switch from the booleans; the strip a grey `— / —` when masked (rendered); the doors need BOTH keys, hidden never disabled; no client masking (asserted); the not-bookable rule untouched. 🔑 ×10 mutations bite, none slipped, md5 identical. Notes + Tanya's list in the TASK §2. Ready for REQ-101's task. 🚫 No deploy asked.

From @Jason 2026-09-21: 📋 **TASK-428 CONTRACT in the TASK — no code.** §1: the create mints NO key today (the loop is one tx, the clash shape reusable); the rows read = a plain relational read by key (extras + rates on `booking_teachers`); add = `attachAdditionalTeachers` per row in one tx (the extra's clash check exists), remove = the reverse (primary ⇒ 409), swap = `swapGroupTeacher`'s loop by key (+ `ALREADY_ON_ROW` both ways); confirm-all = the per-row confirm loop (`confirmCourse` shape); 🔴 **Finding A: `moveBooking` tells NO coach today for ANY type** — only the course-plan edit sends `teacher_(un)assigned`, and only on a teacher change ⇒ the series swap rides those two kinds; the row move's silence is a separate (lessons) defect. §1.6: include `teacher_id` in the backfill key (two coaches' same-named clubs must not merge). §2 corrections: (1) `fromDate` (default today) on add AND remove AND swap through one `seriesRowsFrom`; (2) the notice payload carries the dates list, one line per date, no `bookingId`; (3) the kinds census 20 → 22/23. ❓: cancel-all as ONE `other_series_cancelled` summary per teacher (placeholder words) vs N per-row; `fromDate` on remove/swap; Finding A separate. `startTime` on the header PATCH — no, agreed. ⛔ Your confirm before code.

From @Jason 2026-09-21: ✅ **TASK-428 DONE (code)** — **2690 pass / 0 fail** (+20), tsc 0, 🔴 **`0049_other_series_key`, 50 = 50**, preflight `[]` (alone + the batch), **key 58**, the kinds **20 → 23**. New `services/other-series.service.ts`: ONE reader by key + ONE `seriesRowsFrom` (live rows from the date, default today — add/remove/swap, pinned ×3); confirm-all = the bulk-confirm loop; cancel-all ONE tx (reason + holds; ATTENDED untouched) + ONE `other_series_cancelled` per teacher and NO per-row notice (by value); add/remove/swap with `ALREADY_ON_ROW` both ways, `PRIMARY_TEACHER`, the first clash rolling back with the date in the message; add-dates copies the template through the ONE inserter with the key; the header PATCH on live rows, no `startTime`. ADDED/REMOVED = the owner's bytes by value, `DD-MM-YYYY` one line per date; CANCELLED a 📖 placeholder. The creator returns `seriesKey`; the DTO carries `otherSeriesKey`. Backfill: pure `groupForBackfill` pinned on a fixture (two coaches ⇒ two keys; CAMP/keyed rows never reach it); ⚠️ the `--dry-run` OUTPUT is not in the report — it reads `sid`'s DB, which I never touch; the human runs it after `db:migrate` (50). 25/25 mutations bite. Fern's lines final in the TASK. ⛔ Your DONE.
From @Fern 2026-09-21: ✅ **TASK-429 DONE (code)** — **528 pass / 0 fail** · tsc 0 · build ok (`ƒ /scheduler/other/[key]`) · 10 source modified · 14 tests re-pinned · 9 new · **snapshot 57 → 58** (`calendar.other-cancel-all` after `group-series`, the BE's slot by position). Pure `seriesDoors` by value (key 58 ALONE gates cancel-all; the row conditions); the page guarded as `menu:calendar` via a `ROUTE_ALIASES` entry (nav pins untouched); the doors ask `can()` at the site, hidden never disabled; a row opens the EXISTING modal by id; every 409 = the server's sentence in the dialog, no code-switching, refetch on 2xx only; the header PATCH without `startTime`; the row → page link only with a key; the create toast offers the page; a collapsed `Series in range` strip under the calendar header. 🔑 ×16 mutations bite, none slipped, md5 identical. Notes + Tanya's list in the TASK §2. 🚫 No deploy asked.

## 2026-09-21 — Porter → @Sober: 🟢 **Owner ruled A + B + cancel-all words — bake the words, then it's ready for `sid`.**
- **A (REQ-102):** accepted as-built — mask hourly rate + ceiling + remaining + reorder with the one key. No change.
- **B (REQ-101):** accepted — swap notifies old+new coach, a date/time move stays silent (matches a normal lesson). No extra build now.
- **Cancel-all notice words (`other_series_cancelled`, net-new):** owner-accepted, in REQ-101 §5 — `❌ ยกเลิกตารางทั้งชุด / SCHEDULE CANCELLED` · รายการ/เหตุผล/วันที่ (DD-MM-YYYY) · + EN. Please bake these bytes into the placeholder (a small BE touch if it needs it) and re-run green. Then it's the owner's `sid` deploy → Tanya's REQ-101+102 round → single `uat`.

## 2026-09-22 — Porter → @Sober: ⏳ **Status check + one finding on the cancel-all notice.** Tanya captured the coach notices on the demo OA: **ADDED + REMOVED delivered correctly** (title/time/dates DD-MM-YYYY) ✅. But on **Cancel-all NO coach bubble arrived at all** (the action returned 200 `cancelled:3`, but nothing reached the coach's LINE). So the `other_series_cancelled` notice is not just missing final bytes — **it isn't firing yet.** Please: (1) bake the owner-accepted words (REQ-101 §5: `❌ ยกเลิกตารางทั้งชุด / SCHEDULE CANCELLED` · รายการ/เหตุผล/วันที่ DD-MM-YYYY + EN), AND (2) make cancel-all actually enqueue the coach notice (one per coach, per your earlier design). Re-run green → tell me → Tanya re-captures the cancel-all bubble → then the single `uat` deploy. Everything else in the batch is green and QA-passed; this is the only open item before uat.

## 2026-09-22 — Porter → @Sober: ➕ **REQ-102 ADDITION (customer) — a SEPARATE key for the coach-rate (ค่าสอน §13.3), VIEW=EDIT coupled. This REVISES the §4 exclusion.**
Customer wants the **coach rate (§13.3 teaching rate** — course card / session+Move-session popup / DUO rate box; NOT the freelance budget) gated by its OWN permission key, and coupled: **see ⇔ edit** (no see-only). Without the key: the rate is HIDDEN and the edit control ABSENT. With it: visible + editable. Fail-closed; Teacher role never holds it.
- Note we EARLIER ruled §13.3 rate OUT of scope (REQ-102 §4) — the customer reversed that; recorded in REQ-102 §6.
- **Size it:** a new key (e.g. `coach-rate.manage`) as ONE gate over view+edit on all §13.3 rate surfaces. Fold into the current REQ-102 build if cheap (before uat), or flag as a small follow if it would hold the batch. Your call on timing — tell me the size + whether it delays uat.

## 2026-09-22 — Porter → @Sober: ✅ **Confirm — the two keys are INDEPENDENT (customer 2026-09-22).** `teachers.budget-view` (freelance ceiling) and the new coach-rate (§13.3) key are separate, granted independently — NOT one combined money key. This matches your §6 framing; just making it explicit so they never collapse into one grant.

## 2026-09-22 — Porter → @Sober: ➕ **DUO New-course UI tweaks (REQ-095 §13.4, customer).**
1. Toggle label: drop "(two kids)" ⇒ just **DUO**.
2. Add two DUO programs: **"Duo INLINE SKATE"** + **"Duo SURFSKATE"** (selectable in the DUO Program dropdown).
3. When DUO is selected, the **Program dropdown shows ONLY programs whose name starts with "Duo"** (Private keeps the full list).
Size it: the label + the starts-with-"Duo" filter are FE-small; the two new "Duo …" programs are net-new SUBJECTS — say whether they seed here or belong to backoffice subject-mgmt (per the SYSTEM-FACTS scope line: program/subject mgmt = backoffice). Fold into the current pre-uat batch if cheap, else flag as a follow. Tell me size + timing.

From @Jason 2026-09-22: ✅ **TASK-430 DONE (code)** — **2691 pass / 0 fail**, tsc 0, 50 = 50. `cancelAllOtherSeries` notifies on EVERY live row it cancelled (PENDING included; the per-row cancel's CONFIRMED-only rule untouched, pinned by source). The CANCELLED bytes = REQ-101 §5's owner-accepted TH+EN (`❌ ยกเลิกตารางทั้งชุด / ❌ SCHEDULE CANCELLED` · `ตารางสอนถูกยกเลิกทั้งชุด / Your teaching schedule has been cancelled` · `รายการ/Program` · `เหตุผล/Reason` (the existing reason label) · `วันที่/Date` one line per date `DD-MM-YYYY`), no 📖 (pinned). Three pins as you listed + the bytes by value; 8/8 mutations bite (the CONFIRMED gate back is A). ⛔ Your DONE; `sid` for Tanya's re-capture. TASK-431's contract next.

## 2026-09-22 — Porter → @Sober: 🔁 **§13.4 refinement (owner) — filter DUO programs by a subject TYPE, not by name-prefix.** Give subjects a TYPE/category flag (mark a subject as DUO), tag the two seeded DUO subjects (Duo INLINE SKATE / Duo SURFSKATE) with it, and filter the DUO Program dropdown by that TYPE — NOT by the word "Duo". Seed the two subjects here for now; future self-serve creation likely lives in backoffice (they carry price/money) — design the type so that migration is clean later. Recorded REQ-095 §13.4a (supersedes §13.4 point 3). Size + fold-vs-follow as before.

From @Jason 2026-09-22: 📋 **TASK-431 CONTRACT in the TASK — no code.** §1.1: the §13.3 rate has exactly TWO producers, both pure mappers (`rate` on the booking DTO, `classRateMinor` on the course DTO / the plan), reached from 13 + 8 call sites; NO LINE renderer prints it (❓2: Fern's ฿600 was the FE from the DTO). §1.2: three writers (`PATCH /bookings/:id`, `PATCH /courses/:id`, `POST /courses duo`) — the discount precedent checks at the ROUTE (`assertMayDiscount(body.discount, c.get("user"))`), so **Correction 1:** `assertMayEditCoachRate(body, user)` at the three routes, before the service. §1.3 (❓1): the DTO builders have no viewer and threading one is ~20 signatures for two keys ⇒ **the cheapest honest seam is ONE response mask at `/api/*` after the guard** — walks the JSON once, nulls exactly the keys `rate` / `classRateMinor` (pinned by scan that no other producer exists), fail-closed by construction for every future reader; `teacherRates` untouched. Consequence to say to Porter: a DUO create REQUIRES `duo.classRateMinor`, so a user without 59 cannot create a DUO course (no see-only state) — unless the owner wants the rate optional at create. ⛔ Your confirm (the seam · the route-level check · the DUO-create consequence) before code.

## 2026-09-22 — Porter → @Sober: 🔬 **NEW — REQ-103 whole-voucher cancel (ANALYSE, no build).** Cancel an ENTIRE voucher like cancelling a whole course (distinct from REQ-100's single-booking cancel). Customer: just need the cancel; what they do with the remaining entitlement (convert to course, etc.) is theirs, out of scope. Read: does whole-course cancel extend to a voucher? on cancel — remaining balance, future placed sessions (cancel too?), past draws (keep); reason reuse; a permission gate? Size + owner decisions. Full detail in requirements/REQ-103.

## 2026-09-22 — Porter → @Sober: 🔴 **DEFECT (REQ-101) — the Manage-plan PAGE errors (502 from origin) on a BACKFILLED series; the rest of the app is fine.** Owner repro on `sid`:
- Open `https://som.develyst.online/scheduler/other/d2b80ca9-7378-40a4-b988-ee03e95ea703` (the Manage-plan page for the backfilled series **"ABC / Balance Camp · ECA · New"**) ⇒ **502 origin error**. Everything ELSE works (not a host outage — a route-specific crash; a 502 not a Next 500 ⇒ the route's server-side fetch/SSR throws or times out).
- Tanya's API pass hit `GET /other-series/:key` on a series she CREATED (QA-ECA-plan) = 200. The failing one is a **backfill** series — likely the Manage-plan page/endpoint assumes a field the backfill script didn't populate (or an SSR call on a null), so it crashes only on backfilled rows.
- **Debug + fix:** repro on a backfilled series, find why the page's server-side call 502s (BE `GET /other-series/:key` on a backfilled key, or the FE SSR route). Backfilled series must open the Manage-plan page exactly like a freshly-created one. Re-run green → back to `sid` → Tanya drives the PAGE (not just the API) on a backfilled series. This blocks uat.

From @Jason 2026-09-22: ✅ **TASK-431 DONE (code)** — **2700 pass / 0 fail** (+9), tsc 0, 50 = 50, key **59**. `lib/coach-rate-visibility.ts` (`canSeeCoachRate` — the key, never linked, null ⇒ no, reads nothing of 57 — pinned both ways; `maskCoachRate` — exactly `rate`/`classRateMinor` at any depth, a new value; `assertMayEditCoachRate` — `classRateMinor` incl. null, or `duo`). The seam: `middleware/coach-rate-mask.ts` on `/api/*` after `accessGuard`, `/api/(auth|me|permissions)` outside, a keyed viewer untouched; the two-producer scan pinned (a second `rate` producer bites). By value through the root app: bookings / calendar / plan for super · staff-without-59 (nulls, all else byte-identical) · linked-with-all · 59-not-57 · 57-not-59; `/api/me` untouched. The three routes' write checks by value (403 before the service; bodies without the field run). `teacherRates` untouched (a mutation nulling it bites). 16/16 mutations bite. Fern's lines final in the TASK. ⛔ Your DONE.

From @Jason 2026-09-22: 🔎 **TASK-433 READ DONE — the API is NOT the cause.** Reproduced the page's BOTH calls on backfill-shaped rows (null heads/rates/note, a `/` title, an all-ended series with no live row, a deleted extra teacher, an archived primary, a title-less row, a nine-month span) ⇒ `GET /other-series/:key` 200, `GET /bookings?type=OTHER&teacherId&from&to&limit=200` 200, the list 200, no-rows 404 envelope; `start_time`/`end_time` are NOT NULL so `hhmm` cannot see a null; the TASK-431 mask cannot produce a length mismatch. Pinned (+4 tests, 4 mutations bite). **2704/0**, tsc 0. Hypothesis that fits "one key 502s, the other 200s": the owner HARD-LOADED the backfilled series' URL (the Next server renders the route) while Tanya SOFT-NAVIGATED to the created one (client render). One-minute check for Tanya/the owner: paste the CREATED series' URL into a fresh tab — 502 ⇒ the route's server render (Fern's half, any key); 200 ⇒ the server log decides. The curl on the backfilled key (your DATA REQUEST) will read 200. No product change. ⛔ Your call: DONE, or the FE half to Fern.

## 2026-09-22 — Porter → @Sober: 🔴 **502 — NOT a stale build (owner built fresh). Owner's steer: check the ROUTE the Manage-plan button navigates to.** Hypothesis: the Manage-plan link's target path does not match an actual FE page route.
- The button opens `/scheduler/other/<key>` (singular `other`), but the API is `GET /other-series/<key>` (Jason confirmed 200 on backfill rows). Please verify: what href does the `Manage plan` toast/button use, and is there an actual Next page file at EXACTLY that path? If the page file is `…/scheduler/other-series/[key]` (or any other spelling) while the button links to `…/scheduler/other/[key]`, the route won't match — that's the failure, on any key (created or backfilled), not build/data.
- Fern to confirm the button href ⇔ the page route file are the same path, on both a created and a backfilled series, and fix whichever is wrong. This is the uat blocker; it rides the same round as TASK-430 (cancel-all) + TASK-432 (key 59 FE).

## 2026-09-22 — Porter → @Sober: 🔎 **502 — I read the front repo. NOT a routing/build/data issue. Narrowed it:**
- The page EXISTS at `src/app/(admin)/scheduler/other/[key]/page.tsx` — path matches the button; my href-mismatch guess was wrong.
- The page is a thin server component: `await params` → renders `<OtherSeriesContent seriesKey=...>` (client). NO server-side fetch of its own.
- Its own comment: "the route guard treats `/scheduler/other/*` as `menu:calendar` (`ROUTE_ALIASES`); the server guards the routes."
⇒ The 502 (origin error, not a Next 404/500 page, not the API — Jason's calls are 200) most likely comes from the **server route-guard / middleware** on entering `/scheduler/other/[key]` — e.g. `ROUTE_ALIASES` not matching the dynamic `[key]` segment, or the guard throwing instead of redirecting. Fern/Jason: repro the ENTRY to the route (not the API), check the middleware/guard for `/scheduler/other/*` with a dynamic key, and why it 502s rather than renders. This is the uat blocker.

## 2026-09-22 — Porter → @Sober: 🔁 **RE-SPEC (owner) — Manage plan becomes a MODAL; retire the separate page (this resolves the 502 by removing the route).** Full detail REQ-101 §6.
- Turn the Manage-plan surface into a **modal/panel on the calendar** (same pattern as BookingModal/OtherSeriesDialog) holding exactly what `OtherSeriesContent` renders today (header · rows · the doors Confirm-all/Cancel-all[key58]/Add-Remove-Swap-teacher/Add-dates/Edit-header; a row opens the ordinary BookingModal).
- **Two entry points, one modal:** (1) the OTHER-block BookingModal's `Manage plan` button opens the modal (no navigation, replaces `seriesHref`); (2) a **"Series in range"** row click opens the SAME modal directly (keep the bar as a shortcut index, now clickable → modal).
- **Retire** the page `src/app/(admin)/scheduler/other/[key]/page.tsx`, the `seriesHref` navigation, and the `ROUTE_ALIASES` `/scheduler/other` guard entry — the 502 (a route/guard-level failure) goes away with the route. No API change (the same `GET /other-series/:key` feeds the modal).
- Size + fold into the current pre-uat round (rides TASK-430 cancel-all + TASK-432 key-59 FE). This SUPERSEDES the 502 debug (TASK-433) — no need to fix the route we're deleting.

## 2026-09-22 — Porter → @Sober: 🚧 **OWNER GATE — nothing goes to `uat` until the WHOLE list is complete (no piecemeal deploys).** Treat all outstanding items as ONE batch: REQ-101 (Manage-plan modal + cancel-all + notices + add-teacher), REQ-102 (freelance-ceiling key) + key-59 coach-rate (§6), REQ-095 §13.4/§13.4a (DUO UI + subject type), REQ-103 (whole-voucher cancel), and — pending owner confirm — REQ-077/REQ-051 (Rich menu / QR check-in). Finish sid + Tanya on the full set, THEN one uat deploy. I've asked the owner whether Rich menu/QR is in this gate; will confirm.

## 2026-09-22 — Porter → @Sober: 🔴🔴 **REGRESSION on `uat` (customer-reported) — the "คาบสอนนี้ถูกย้ายออกจากตารางของคุณแล้ว" (session MOVED-OUT / teacher-unassigned) coach notice STOPPED firing.** 
Customer (Khwan) screenshot from a real coach's OA (Coach Bank): the notice `📮 คาบสอนนี้ถูกย้ายออกจากตารางของคุณแล้ว · นักเรียน: พอ ชิตวร · วิชา: Private SURFSKATE · เวลา: 27-09-2026 11:00-12:00` DID arrive before, but she says **"ตอนนี้เหมือนระบบไม่ยอมแจ้งย้ายคาบ"** — the move/reassign notice no longer fires.
- This is the OLD-coach notice on a teacher reassignment/move (teacher_unassigned-style), which is the SWAP case that SHOULD notify (not the "date/time move is silent" ruling). So it's a real regression, not the accepted §5 behaviour.
- **Prime suspect:** the recent notice-builder consolidation (TASK-425 folded 6 name paths + the census/scan-pin) — LIVE on `uat` since 09-21. Possibly the move/unassign notice's enqueue or its `studentName`/template got dropped or gated in the fold.
- **Debug:** repro a teacher move/reassign on a Private lesson, confirm the moved-out (+ moved-in) coach notice enqueues; find what stopped it (git-compare the move/unassign notice path before/after TASK-425). It's on `uat` (real coaches affected) — flag if it needs a hotfix ahead of the held batch, or rides the batch. Report the cause.

## 2026-09-22 — Porter → @Sober: 🟢 **key-59 final rulings (owner accepted Porter's recommendation) — build it this way.** (REQ-102 §8)
1. **Coach rate OPTIONAL at create** — decouple: a user WITHOUT key 59 can still CREATE a DUO/course (create door NOT hidden); they just can't see/set the rate; a key-59 holder fills it later. (ค่าสอน = coach pay, separate from the ungated student price.) This is the "front-desk creates DUO without seeing the rate" follow you flagged — do it.
2. **key 59 gates the coach rate on EVERY surface, ONE key:** course card · session/Move-session popup · DUO create field AND **ECA/Group per-teacher rates (`other.teacherRates`, Stage-1)** — all masked+gated by the SAME key 59 (see⇔edit). Expand your §6 scope to include the ECA/Group rates (I'd left them out; owner wants them IN, one key — a coach rate is a coach rate everywhere, else it leaks). NOT separate per-type keys.
3. `teachers.budget-view` and key 59 stay two independent keys (§7). 
Fold into the current held batch. Tell me the added size for the ECA/Group-rate gating + the optional-at-create change.

From @Jason 2026-09-22: ✅ **TASK-434 DONE (code, census in the TASK — nothing surprised)** — **2711 pass / 0 fail** (+7), tsc 0, 50 = 50. Producers: `rate` (rateFacts) · `classRateMinor` (duoCourseFacts) · `teacherRates` (otherFacts → `other`, groupFacts → `group`, the series DTO's `ratesOf`); NO DTO carries `rateMinor` (the extras fold into `teacherRates`; `toTeacherBase` has no rate) — the scan pin extended to the three keys + a `rateMinor`-by-absence pin. Writers: 9 routes (the six with `teacherRates`/`rateMinor` + 431's three), all checked at the route by value (403 with the field before the service, 201/200 without); `POST …/dates` copies (unchecked); `PUT /teachers/:id/budget`'s `rateMinor` is 57's, not under this check (pinned). `duo` alone no longer trips the check; `duo.classRateMinor` optional (validator + service `?? null`; a DUO without a default reads `rate: {null,null,null}`). Mask: `teacherRates ⇒ null` on all three surfaces (by value + root app). 13/13 mutations bite. Fern's lines final in the TASK. ⛔ Your DONE.
From @Fern 2026-09-22: ✅ **TASK-432 DONE (code, to §0b)** — **533 pass / 0 fail** · tsc 0 · build ok · 12 source modified · 15 tests re-pinned · 1 new · **snapshot 58 → 59** (`bookings.coach-rate` after `teachers.budget-view`, the BE's slot by position). ONE pure `withoutRates` guard (value-tested) at all seven body sites — no `classRateMinor`/`teacherRates`/`rateMinor` rides without the key; every rate surface behind `can(coach-rate)`, ABSENT not dashed (the shared `OtherScheduleFields` inputs in one place ⇒ OTHER/Group create + edit + Edit-header; the Manage-plan header + Add-teacher box; the session box/tag/Clear; the course card line; the DUO field). §0b: the DUO door stays, the field alone absent, `duo: { coStudentId }` alone rides. 57 ≠ 59 pinned both ways. 🔑 ×15 mutations bite, none slipped, md5 identical. Notes + Tanya's list in the TASK §2. 🚫 No deploy asked.

## 2026-09-22 — Porter → @Sober: ⚠️ **Your "batch complete" predates my last two items — confirm BOTH are in the batch before any sid/uat. Do NOT treat the batch as ready yet.**
1. 🔴 **REQ-101 §6 MODAL re-spec** — I asked you to RETIRE the `/scheduler/other/[key]` page and make Manage plan a MODAL (two entry points: the OTHER-block BookingModal button + a clickable "Series in range" row). Your latest messages still frame the 502 as a PAGE to debug (hard-load check, TASK-433) — that is SUPERSEDED. There is no 502 to chase once the page is deleted. Please confirm you're building the modal (and dropping the page + `seriesHref` + the `ROUTE_ALIASES` entry), NOT asking the owner to debug the page. This is the whole point — the owner will not hard-load a route we're removing.
2. 🔴 **The move/reassign coach-notice REGRESSION on `uat`** ("คาบสอนนี้ถูกย้ายออกจากตารางของคุณแล้ว" stopped firing; customer screenshot). Not in any of your messages. Prime suspect: TASK-425's notice-builder fold. Confirm you're diagnosing it and whether it's a hotfix-now or rides this batch.
Reply with: modal re-spec status + regression cause/plan. Until both are in, the batch is NOT ready for sid.

From @Jason 2026-09-22: 🔎✅ **TASK-436 — cause found, fixed.** (1) Delivery CLEAN by value: the REAL worker path on a `teacher_unassigned` row for the customer's exact Private shape ⇒ `sent: 1`, SENT, the bytes byte-equal to the old screenshot; a DUO row too; the `coStudent` relational query builds. (2) The gate CLEAN: `teacherChanged` independent of `override`; a too-late change 409s before any write. **(3) The DOOR is the cause:** `moveBooking` (the Move-session popup, everyday since TASK-424) wrote the teacher and told nobody — only the plan editor sent the pair; the outbox rows will show ABSENCE, not failure. Fix: ONE `sendTeacherReassigned` helper (old ⇒ `teacher_unassigned`, new ⇒ `teacher_assigned`, in the tx) called by BOTH doors; a date/time/note/rate-only move silent. **2718/0** (+7), tsc 0; 8/8 mutations bite (A = the regression as shipped). **Hotfix recommended:** one file (`scheduler.service.ts`, +20/−24) + the test, no migration/key/FE, rides uat's deployed base. ⚠️ Not changed: the popup has never had the notice-days gate the plan edit has — say if you want it folded or filed. ⛔ Your DONE.

## 2026-09-22 — Porter → @Sober: 🅰️ **Owner: NO hotfix — the regression fix (TASK-436) RIDES THE BATCH.** Everything goes to uat together per the gate; do not cherry-pick. TASK-436 stays in the working tree with the rest. (Awaiting the owner's answer on B — the notice-days gate on the Move-session popup — before you finalise; I'll relay.)

## 2026-09-22 — Porter → @Sober: 🅱️ **Owner ruling — LEAVE the Move-session popup UN-gated by notice-days.** Do NOT fold the `teacher_change_notice_days` block into the popup — owner: "it's an admin-usage thing" (admins reassign as the day requires). The plan editor keeps its gate; the popup keeps its freedom. TASK-436's reassignment NOTICE still fires from both doors — only the BLOCK is not added. Batch scope is now LOCKED: REQ-101 (modal TASK-435 + cancel-all + notices + add-teacher + TASK-436 reassign-notice) · REQ-102 (§4-§8 coach-rate key) · REQ-095 §13.4/§13.4a · REQ-103. Regression rides the batch (no hotfix). Proceed to finish TASK-435, then §13.4a + REQ-103, then sid.

## 2026-09-22 — Porter → @Sober: 🟢 **SPEC-089 rulings — "เอาตามนั้น" (all your recommendations). Cut the tasks.**
- **§13.4a:** hide DUO subjects from the Private dropdown too (yes); server refuses a DUO course on a non-DUO subject (yes). `subjects.kind` (PRIVATE|DUO) + seed the two DUO subjects + FE filter by kind. Migration 0050.
- **REQ-103:** reuse `bookings.course-cancel` (no key 60); coach notice = course-end message + a voucher line (no new bytes); family SILENT; card `ENDED · Nh left`. Voucher `ended_at/by/reason`, cancel+preview routes, future draws→CANCELLED, past+used_hours untouched, balance frozen/readable, ENDED can't draw. Migration 0051.
Both fold into the held batch (migrations → 52). Contract-first tasks as usual. When 435 (modal) + these land and you've re-run green, ping me — then sid → Tanya's big round → single uat.

From @Jason 2026-09-22: 📋 **TASK-437 CONTRACT in the TASK — no code.** §1: NO `GET/POST/PATCH /subjects` — the frontoffice never edits a subject (❓2: no editor); subjects reach the FE through exactly two readers (`teacher.subjectOptions` — the ONE picker mapper — and `GET /sellable-packages`), so `kind` rides both; the only writers are the seed and the owner-run `add-subject` (insert-if-missing, never updates) — I propose `add-subject --kind` too, else a third DUO program needs a deploy. The seam: `createCoursePackage` prices a DUO create WITHOUT reading the subject — the rule goes before its tx (+ `importCoursePackage`); **❓1: the reverse refusal yes** (`DUO_SUBJECT`). **Correction:** the seed must be an UPSERT by name (the two rows exist on uat as PRIVATE after 0050's default — updated in place, never duplicated) — the `ensure-sale-items` shape with a pure planner pinned. `0050`: `kind text NOT NULL DEFAULT 'PRIVATE'` + CHECK NOT VALID/VALIDATE as the witness, CHECK ⇔ `SUBJECT_KINDS` by value. My ❓: a walk-in single/trial on a DUO subject — leave sellable (TASK-399's walk-in) or refuse? I say leave. ⛔ Your confirm before code.

From @Jason 2026-09-22: ✅ **TASK-437 DONE (code)** — **2729 pass / 0 fail** (+11), tsc 0, 🔴 **`0050_subject_kind`, 51 = 51**, preflight `[]`. `subjects.kind` NOT NULL DEFAULT 'PRIVATE' + the CHECK (NOT VALID → VALIDATE) as the witness, CHECK ⇔ `SUBJECT_KINDS` by value (drift on either side bites). `lib/subject-kinds.ts`: the set, the two 400s, `assertSubjectKindForCourse` — BEFORE the tx at `createCoursePackage` (the DUO branch now reads the subject once) and `importCoursePackage`; the walk-in untouched; pricing untouched. The two readers carry `kind` (pinned as the only two). `subjects:ensure`: the pure planner by value (create / update in place / unchanged / re-activate / foreign name untouched), dry-run default, `--apply` ONE tx with `update … where name` (a duplicate-insert mutation bites); the names as you gave until Porter confirms. `add-subject --kind` validated. 14/14 mutations bite. Fern's lines final in the TASK. ⛔ Your DONE. TASK-439's contract next.

From @Jason 2026-09-22: 📋 **TASK-439 CONTRACT in the TASK — no code.** §1.1 the split: three course-shaped pieces get a voucher twin (`loadVoucherForEnd`, `endableVoucherDraws` = live AND `date ≥ today` — `endableSessions` excludes VOUCHER rows and has no date rule; the stamp on `vouchers`); `sendCourseDroppedToTeachers` generalised in place (`{ id, size }`, cause `voucher_ended`) — ONE sender; the cancel branch shared. Found: the course end never reconciles holds (pre-existing) — the voucher end will; say if the course end should too. §1.2 the line: NO existing "Voucher" label exists — the no-new-bytes `Program` line is `<subject> 10 HR` (size = totalHours, the course renderer as is); a "voucher" word = one new 📖 key, the owner's. §1.3: ONE gate (`voucherUsable`) feeds the draw, the picker and the SOM ⇒ `endedAt` refused there once; `voucherStatus` does not exist ⇒ created (ENDED > EXPIRED > EXHAUSTED > ACTIVE). §1.4 the undo: allowed (the end freezes the future). Corrections: the preview keeps the course preview's field names (+ `remaining`); the body field is `reason` (the course validator reused). ❓ the voucher rows get `cancel_reason` (the course rows never did) — parity or the code? ⛔ Your confirm before code.

From @Jason 2026-09-22: ✅ **TASK-439 DONE (code)** — **2749 pass / 0 fail** (+20), tsc 0, 🔴 **`0051_voucher_end`, 52 = 52**, preflight `[]`. Three nullable columns + the CHECK (NOT VALID → VALIDATE) as the witness, CHECK ⇔ `END_REASONS` by value. `voucherUsable` refuses ENDED first (picker + SOM follow through the ONE gate); the creator's `409 VOUCHER_ENDED` before the expiry write; `voucherStatus` ENDED > EXPIRED > EXHAUSTED > ACTIVE on the DTO (+ `endedAt`, `endReason`; `remaining` readable). `endableVoucherDraws` = live AND date ≥ today (a past live draw left, a today row doomed). `cancelDoomedRows` shared: the voucher rows get the note + `cancel_reason` + holds; **the course end now reconciles holds too, pinned by value** (its rows otherwise byte-identical, no code). ONE sender (`cause: "voucher_ended"`, `size = totalHours`); the render pinned byte-identical to an `ended` one in TH/EN (`Freeskate 10 HR`, no "voucher" byte); no family row. Routes under `course-cancel`, pinned equal to the course entries; the undo on ENDED open (pinned by absence). Note: no `end_note` column — the optional note rides the notice only (say if you want it persisted). 18/18 mutations bite. Fern's lines final in the TASK. ⛔ Your DONE.
From @Fern 2026-09-22: ✅ **TASK-435 DONE (code)** — **533 pass / 0 fail** · tsc 0 · build ok — **`/scheduler/other/[key]` absent from the route list**. ONE modal (`OtherSeriesModal`, the page's body), two entry points: the OTHER block's `Manage plan` button (key AND host; closes the booking modal first) and a `Series in range` row button; the calendar owns the one instance. A row inside CLOSES the series modal and hands the booking to the calendar's single `BookingModal` — my call: one instance, no modal-over-modal, no z-index (pinned). Retired: the route, `seriesHref` (+ cases), the `ROUTE_ALIASES` entry (`navItemForPath` byte-back; the nav pins never moved in 429 — nothing to move back), the create toast's link (dropped — the new series shows in `Series in range` and on its rows' OTHER block). 🔑 ×7 mutations bite (one slipped first — pinned), md5 identical. Notes in the TASK §2. Moving to TASK-438. 🚫 No deploy asked.
From @Fern 2026-09-22: ✅ **TASK-438 DONE (code)** — **536 pass / 0 fail** · tsc 0 · build ok · 6 source modified · 1 new test. Pure `subjectsFor(subjects, duoOn)` value-tested (DUO on ⇒ `kind === 'DUO'` only; off ⇒ the rest; a missing `kind` = PRIVATE; a DUO-named private stays private — the kind, never the name; scan-pinned over `src`). The New-course list follows the toggle (inside a group ⇒ Private), a stale selection is cleared on flip; the walk: `ImportBalanceModal` + the unmounted `CreateCourseModal` hide DUO; the calendar's single/trial picker and the Bookings filters untouched (asserted). The 400s = the server's sentence, no new copy. 🔑 ×6 mutations bite, none slipped, md5 identical. Notes + Tanya's list in the TASK §2. Moving to TASK-440. 🚫 No deploy asked.
From @Fern 2026-09-22: ✅ **TASK-440 DONE (code)** — **543 pass / 0 fail** · tsc 0 · build ok · 9 source modified · 2 new · 1 pin moved (sweep 97/37). ONE dialog: `EndCourseDialog` takes `target: { kind: course|voucher, id }` — the voucher pair (`/vouchers/:id/cancel/preview {}` · `/vouchers/:id/cancel { reason, note? }`, the body pinned byte-equal to the course's) or the course pair; the voucher face lists the doomed draws + *Nh left — kept for the customer* (the server's `remaining`, no subtraction). The card: `Cancel voucher` beside Manage via pure `voucherDoors` (key AND not ENDED, hidden never disabled); the chip via pure `voucherChip` reads the server's `status` (ENDED ⇒ `Ended · Nh left`, red; an older payload keeps the old rule, never ENDED). 🚫 No client derivation (asserted). **Plainly: the FE card has NO book door** — a draw is booked from the calendar's create form whose picker is the server's eligible list (already omits ENDED); `voucherDoors.book` mirrors the BE `voucherUsable`, value-tested, nothing on the card to hide. Copy +8 both languages; 📖 the Thai word (บัตรชั่วโมง per the TASK vs the panel's วอยเชอร์) is Porter's — one line. 🔑 ×9 mutations bite, none slipped, md5 identical. Notes + Tanya's list in the TASK §2. **435 · 438 · 440 all reported — the batch's FE side is complete; nothing waits on me.** 🚫 No deploy asked.

## 2026-09-22 — Porter → @Sober: 🔬 **GAP-CHECK (no build) — the customer sent a full checklist; 4 areas I must verify before telling her "done". REQ-104.**
1. **GROUP parity:** REQ-101 gave Confirm-all / manage / cancel-all to ECA/OTHER series. Does the GROUP object have the same — Confirm whole course, weekly-confirm-to-coach, a manage surface? What exists vs missing + size. (Customer's "Group: 1 confirm whole course, 2 confirm weekly for coach, 3 standing schedule".)
2. **CAMP coach rate PER DAY:** can the coach rate be set per camp-week-DAY today, or only per week/overall? (Customer: "ค่าสอนต้องกำหนดได้เป็นต่อวัน".)
3. **CAMP check-in:** does it SHOW the remaining day-credit (to admin) AND notify the parent like a Private check-in? Gaps? (Customer: "check-in โชว์เหลือเครดิตเท่าไหร่ + แจ้งแม่เหมือน Private".)
4. **CAMP Redeem:** does the redeem/enroll picker list ONLY kids WITH remaining credit, or everyone? (Customer: "Redeem ขึ้นเฉพาะน้องที่มี credit".)
Read + report what's DONE vs GAP + rough sizes for the gaps. No build — I need the true picture to answer the customer. The current sid batch (REQ-101/102/103/§13.4a) is unaffected; this is scoping the NEXT slice.

## 2026-09-22 — Porter → @Sober: 🟢 **SPEC-090 gaps — owner "เอาตามแนะนำ", BUILD all into the batch. Rulings + words in REQ-104 §2.**
1. GROUP: one-button **confirm-whole-group** (per-child already works).
2. GROUP: **weekly coach digest** (Monday) — words in REQ-104 §2.
3. GROUP: **manage surface = the REQ-101 modal generalised to `group_key`** — cancel-all CASCADES to every child's seat, each family told · add/remove extra coach · add dates · edit name/cap/rates.
4. CAMP: **coach rate per-coach-per-day** on the week's day editor, behind key 59, copied onto the derived rows; new-day default = **0**.
5. CAMP check-in: (a) scan page **shows remaining credit**; (b) **CAMP-deduction family notice at DAY-END** (parity with Private, not at scan) — words in REQ-104 §2.
6. CAMP redeem: already done — no work.
Fold into the held batch (this is what the owner wants "complete" before the whole thing goes to `sid` for the CUSTOMER to test). Contract-first; sizes as you had them (BE M+ · FE M · one migration · two notice kinds). Ping me when it lands green with the rest — then ONE sid deploy of everything, then Khwan's customer-UAT on sid, then uat.

## 2026-09-22 — Porter → @Sober: 🔤 **The two new notices (weekly coach digest + camp deduction) are ENGLISH ONLY — no Thai line** (owner: "pure english ไม่เอาไทยเลย"), regardless of line_lang. Final bytes in REQ-104 §3:
- Weekly digest: `📅 THIS WEEK'S SCHEDULE` / "Hello, here is your teaching schedule for this week:" / {Date · Time · Program/Student} / "Please review your schedule."
- Camp deduction: `🏕️ CAMP CREDIT USED` / `Student : {name}` / `Date : {date}` / `Remaining : {remaining}/{total} days`.
Supersedes the bilingual draft. Everything else in the gap build unchanged.

From @Jason 2026-09-22: 📋 **TASK-441 CONTRACT in the TASK — no code.** §1.1: the OTHER assumption lives in FOUR places (`seriesRows`, the list's groupBy, `addDates`' insert, the PATCH's `otherKind`) — ONE module with `SeriesKey = { otherSeriesKey } | { groupKey }`, no copied function; 🔍 the series swap never moves seats, `swapGroupTeacher` does ⇒ the group swap delegates to it (`{ to, fromDate? }`). §1.2: the cascade EXISTS for one date — `cancelSeatsOfGroup` (seat CANCELLED + `reconcileCoursePlan` re-owes; nothing returned, nothing was consumed) + `sendClassCancelledToFamilies` (a household per seat) — the per-row cancel's own path, reused per live group row; coach once. §1.3: `confirmCourse` per seated course + `bulkConfirm` on the group rows. §1.4: jobs are Task-Scheduler EXEs ⇒ `runWeeklyTeacherDigestJob` + `/internal/jobs/weekly-teacher-digest` + `scripts/weekly-teacher-digest.ts`, **the Monday 08:15 registration on the box = the human's deploy line**; `CONFIRMED`-only rows (`REMINDABLE`) — say if PENDING too; **no flag recommended** (the words are the owner's, not placeholders); no per-kind lang override exists ⇒ the 24th kind takes no `t()`/`lang`, constants pinned under both langs; the exact bytes/separators quoted for your confirm. ⛔ Your confirm before code.

From @Jason 2026-09-22: ✅ **TASK-441 DONE (code)** — **2766 pass / 0 fail** (+17), tsc 0, 52 = 52, kinds 24. ONE module keyed `SeriesKey` (a bare string = OTHER — its 21 tests byte-identical); the four branches only (pinned: one `seriesRows`, one loop, one notifier, one group-shaped export). The DTO's `rows[].seats`. The CASCADE by value: live rows + `cancelSeatsOfGroup` (exported) + `sendClassCancelledToFamilies` per row, ATTENDED kept, coach once with the group's name; OTHER never reaches the seat/family senders. Confirm-whole-group: `bulkConfirm` + `confirmCourse` per seated course deduped, a refused course `skipped`. Add dates = GROUP rows under `groupKey`, no seats. The swap delegates to `swapGroupTeacher` from the first live row ≥ `fromDate`. Nine routes, the access map pinned equal to the OTHER twins (confirm-all = `course-confirm`, dates = `group-series`). The digest: `weekly_schedule_teacher` — the bytes as confirmed, identical under TH/EN, no Thai code point, no `t()`/`lang`; Mon–Sun; CONFIRMED only; primary + extras; send-once; `job_runs`; no flag; `/internal/jobs/weekly-teacher-digest` + exe + script. 21/21 mutations bite. Fern's lines final in the TASK. Deploy: register the Monday 08:15 task. ⛔ Your DONE. TASK-443's contract next.

From @Jason 2026-09-22: 📋 **TASK-443 CONTRACT in the TASK — no code.** 🔴 Finding on the premise: Private's `course_deduction` fires at the CONSUMPTION moment (manual attend AND the scan via `updateBookingStatus("attend")`), the day-end only for rows never attended — so "parity, not at scan" disagrees with itself, and a day-end-only notice would MISS a scanned camp day (already ATTENDED when `cutCampDays` runs). Two shapes: (a) true parity in `markDay` + the cut; **(b) recommended — day-end only, honouring §0: `camp_days.deduction_notified_at` in `0052`, a second day-end pass over CONSUMING days (ATTENDED | ABSENT — a no-show is charged, §8) not yet stamped ⇒ one row per family account + the stamp, idempotent by the stamp.** Simplifications: a derived camp row has ONE teacher (no extras ⇒ `teacher_rate_minor` only; the sync also updates KEPT rows); the `0` default = the absence of a rate row. Key 59: the READ holds by the name mask with zero code; the WRITE needs ONE line — `routes/camp.ts:18` has no `assertMayEditCoachRate` today. Scans: camp `credit: { remainingDays, totalDays }` = `(total − used)/2` (say if `creditOf`'s planned-subtracted number instead); Private `remaining: { used, total, unit } | null`. Pins that move named. ⛔ Your confirm before code.

From @Jason 2026-09-22: ✅ **TASK-443 DONE (code)** — **2776 pass / 0 fail** (+10), tsc 0, 🔴 **`0052_camp_day_rates`, 53 = 53**, preflight `[]`, kinds 25. The stamp column + the rates TABLE (PK day+teacher; the witness). The rate: `teacherRates` on the day DTO (0 by absence), the PATCH upserts BEFORE the ONE sync (off-day ⇒ 400 before any write), the sync inserts WITH the rate and re-stamps the KEPT rows; the ONE route line for 59 (403 through the root app, the service never reached); the read masked by name (pinned by value). Scans: camp `credit` = (total − used)/2 in days; Private `remaining` course / ONE voucher read / null. `camp_deduction` = (b): `notifyCampDeductions` INSIDE the day-end tx AFTER the cut, CONSUMING days (ATTENDED | ABSENT) not yet stamped ⇒ one row per family account (a skipped row without one), the stamp = the idempotency; `markDay` sends nothing (pinned by absence); the bytes identical under TH/EN, no Thai byte. The cut's first pass + the Private deduction byte-identical. 19/19 mutations bite. Fern's lines final in the TASK. Deploy: `db:migrate` verify 53. ⛔ Your DONE.
From @Fern 2026-09-22: ✅ **TASK-442 DONE (code)** — **550 pass / 0 fail** · tsc 0 · build ok · 11 source modified · 1 new test · 2 pins moved (wire → `seriesPath`; sweep 101/37). ONE modal, `series: SeriesRef { kind: other|group, key }`: the ref picks the wire (`/group-series`), the chip family, the swap body (GROUP `{ to }`), the header PATCH (no kind, select hidden). The GROUP face: the cap, each row's seats (cancelled greyed), `Confirm whole group` behind `bookings.course-confirm` (pure `groupSeriesDoors` — widens to a PENDING seat on a live row; the toast prints the server's `courses`), `Cancel all` behind key 58 with the cascade line. 🚫 No client cascade logic: the line COUNTS the DTO (seats · distinct students — `seatCascade`); the FAMILIES number prints only from the server's `familiesTold` (a client one pinned absent) — §1's "m families" on the pre-act line is not knowable on the FE (siblings share one), so the line says students, the toast says families; said in the TASK. Entry points: the GROUP block's `Manage plan` (key AND host, closes first) + `Series in range` listing groups (grape chip). OTHER face byte-equal in behaviour (21 pins hold). Copy +4. 🔑 ×9 mutations bite, none slipped, md5 identical. Notes + Tanya's list in the TASK §2. Moving to TASK-444. 🚫 No deploy asked.
From @Fern 2026-09-22: ✅ **TASK-444 DONE (code)** — **555 pass / 0 fail** · tsc 0 · build ok · 6 source modified · 1 new test · 2 pins moved. The day editor: a `Rate per coach (฿)` column only with key 59 AND the server's `teacherRates` unmasked (`null` ⇒ nothing); one box per coach on the day, prefilled (`0` ⇒ `0`); pure `dayRates` + `dayPatch` send `teacherRates` ONLY when a rate on the day's coaches changed (an off-day coach never rides); every per-day PATCH through `withoutRates`. The check-in page: ONE `Remaining : …` line per kind via pure `remainingLine` — camp `3.5/10 days` (as sent), Private `7/10 sessions` / `… hours` by `unit`; `null` ⇒ no line; nothing on `already` (pinned both kinds). 🚫 No client arithmetic (asserted). Snapshot 59 unchanged. Copy +5 both languages. 🔑 ×9 mutations bite, md5 identical (one first written value-identical — a no-op, replaced; said in the TASK). Notes + Tanya's list in the TASK §2. **442 · 444 reported — the REQ-104 gap build's FE side is complete; nothing waits on me.** 🚫 No deploy asked.

## 2026-09-22 — Porter → @Sober: 🔴 **DEFECT + 1 confirm from Tanya's gap round (blocks the Khwan invite).**
1. 🔴 **Camp-week create INCLUDING today ⇒ 500 INTERNAL** (a future week = 201 fine). Tanya's read: the derived camp block (10:00–15:00) clashes with the coach's existing near-term sessions and the create THROWS 500 instead of a clean 409 (§11 refuses cleanly; this create path doesn't). A customer creating "this week's" camp hits it. **Fix: the camp-week-create clash path must refuse with a 409 naming the clash, not 500.** (Also blocks Tanya's camp-scan `Remaining` check — she can't stage a today-week.)
2. 🔎 **Group cancel-all `familiesTold` semantic** — Tanya saw `{cancelled:6, seatsCancelled:6, familiesTold:6}` with only ONE family (6 seats). Confirm intended: is `familiesTold` counting SEATS or DISTINCT families — and does a single household get ONE `class_cancelled_parent` or SIX? It must de-dupe per household (like the DUO notice helper). Confirm/fix + say which.
Both before Khwan's customer-UAT on `sid`. The rest of the gap slice PASSED API. LINE-delivery/job items sit on the push-ceiling (owner/sanctioned job-run).

From @Jason 2026-09-22: ✅ **TASK-445 DONE (code)** — **2783 pass / 0 fail** (+7), tsc 0, 53 = 53. 🔴 **The 500's cause:** the clash's `23505` ABORTS the tx (the 409 was built fine), then the sync's catch read the coach's nickname ON THAT TX ⇒ `25P02` raw pg error ⇒ 500 — any clash on the camp create/day-edit path, not a today rule. Fix: the names read once BEFORE the insert loop; the catch touches memory only. Pinned by value with a fake tx that throws 25P02 after the clash (409 naming date · hour · coach) and through the root app (409 envelope, the tx callback rejected). Past dates: the sync derives nothing and deletes nothing (say if you want a re-sync to delete past rows — one line). §2: `classCancelledFamilyAccounts` = ONE household set per row (all the seats ⇒ siblings once; Private/DUO byte-identical), returns the accounts; the `0|1` wrapper kept for the two single-row callers; cancel-all ⇒ `{ cancelled, seatsCancelled, familyNotices, householdsTold }` (`familiesTold` gone). 9/9 mutations bite. Fern's lines in the TASK. ⛔ Your DONE.
From @Fern 2026-09-22: ✅ **TASK-442b DONE (code)** — the group cancel-all toast reads `householdsTold` (distinct families) + `familyNotices`: *"{n} dates · {seats} seats cancelled · {families} families told ({notices} notices)"*, both languages; `familiesTold` removed from the contract, the mock and the dialog (pinned absent). **555 pass / 0 fail** · tsc 0 · build ok. 🔑 ×2 mutations bite (the old per-seat field; notices-as-families), md5 identical. Notes in TASK-442 §2b. 🚫 No deploy asked.
