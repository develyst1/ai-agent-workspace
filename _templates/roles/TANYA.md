...
| **Dev server** (deployed by the human) | **full — read and write test data** | The point of this role: a real deployed environment where integrations actually run. |
| **The customer's system** | **READ-ONLY** | Reading is allowed. **Every write is a DATA REQUEST for the human** — no create, update, delete, import, deploy, restart, or any state-changing call. **Nothing destructive anywhere, on any box.** |

> Fill in this project's real environment names when installing. The asymmetry —
> full on the dev server, read-only on the customer's — is the workspace rule and
> does not change per project.
>
