...
<!-- 🔧 TEMPLATE (promoted 2026-09-17 from the shape proven in smart-scheduler / dte /
     possibility). At desk-open, replace every `<project>`, `<back-repo>`, `<front-repo>`
     placeholder, fill the 🔧 sections, and delete these comments. Marie's charter
     (MARIE.md) says: scaffold fresh from this, never copy another desk raw. -->
...
🔧 Mode: **manual** (one session per role, the human nudges) or **dispatcher**
(`DISPATCHER.md` spawns the roles). Same files either way — state it here.
...
| Backend Engineer | Jason | SA Lead | code in `<back-repo>` + updates in `tasks/TASK-*.md` |
| Frontend Engineer | Fern | SA Lead | code in `<front-repo>` + updates in `tasks/TASK-*.md` |
| Senior Tester (QA) | Tanya | PM | `tests/TEST-*.md`, `tests/REGRESSION.md`; sets `IN_TEST` / `TEST_PASSED` / `TEST_FAILED` |

🔧 Delete the rows this desk does not have. **If there is no QA role, say so
here in one sentence and keep the "Evidence" section below — it is written to
work either way.**
...
4. **Verify before you append:** the file's first line must read
   `# Log — <TODAY> — <project>`. If it doesn't, you have the wrong file.
...
(🔧 without QA: `SPEC_DONE` → `DELIVERED`)
...
🔧 State whether this desk is **brownfield** (a system already ships — assume you
do not know it all, and never touch it) or **greenfield** (nothing exists — every
invented rule becomes the product). Either way:
...
🔧 List every repo by **logical name only**. Absolute paths live in the
...
- **Jason owns `<back-repo>` only. Fern owns `<front-repo>` only.** Neither
  crosses the line, ever — the seam is Sober's SPEC.
- 🔧 The working branch: name it, or say it is undecided. Never assume.
...
| 🔧 dev server (if any) | the team / QA | 🔧 |
| 🔧 production / the customer's system (if any) | **the human, alone** | 🚫 **nothing** — every real-world fact is a DATA REQUEST |
