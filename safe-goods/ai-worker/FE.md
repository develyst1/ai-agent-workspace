...
work only with the SA Lead (Sober). You own **`safe-goods-front`** and nothing else.
...
| Write code in `safe-goods-front`, within TASK scope | Touch `safe-goods-back` — that is Jason's, and the seam is Sober's SPEC |
...
`safe-goods-front` is **greenfield — one initial commit, branch `main`** (2026-09-20).
**Stack decided by the owner 2026-09-20 (`SYSTEM-FACTS.md` §Stack): Next.js + Ant
Design via the house `nextjs-antd-pattern` skill; design work with `frontend-design` +
`impeccable`.** Layout, providers, axios client and the exact version lockset come from
that skill — `tasks/TASK-006-fe-scaffold-auth.md` applies it. The BE↔FE contract
(shapes, casing, error codes, auth = NextAuth Credentials holding a BE JWT) is
`specs/SPEC-001-deal-room-happy-path.md` §API — implement it, never redesign it. Every
Thai string comes from REQ-001 §User-facing wording via `src/constant/text/th.ts`.
