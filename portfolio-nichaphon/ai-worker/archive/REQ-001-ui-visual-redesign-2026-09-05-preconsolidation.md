...
AC1's answer carried two forward wishes. They are recorded here so nothing is
lost, and they are **out of REQ-001's scope**: this REQ is closed by its own
criteria. Neither is written as a REQ yet, because neither can be made testable
without the owner — Porter does not invent a design bar or a person's history.

- **N4 — "เท่ และ เบียวขึ้นไปอีก"** (make the look bolder / more dramatic).
  Unspecified: what "more" means, and on which routes. It is not a defect in
  the delivered Home — he accepted that in the same breath.
- **N5 — "อัปเดต portfolio ... นั่นมันของเก่า มีอีกหลายอย่างที่เกิดขึ้นมา"**
  (his portfolio content is out of date; new things have happened). This is a
  **content** ask, and R4 binds: the new facts exist only with him. It will need
  a DATA REQUEST for the actual material before any REQ can be written.

**Questions to the owner, asked in Thai in chat on 2026-09-02 (nobody guesses):**

- **Q14 — scope of N4:** Home only, or the whole site? And does it come before,
  after, or together with the five-route rebuild that R6 gates on AC1?
- **Q15 — what "เท่ / เบียว" means concretely:** one or two examples he can point
  at — a site, a screenshot, or "the reference we already have, but further in
  direction X". Without an anchor there is no bar to build or test against.
- **Q16 — scope of N5:** the `/portfolio` route only, or his profile content
  across the site (About, Services too)?
- **Q17 — DATA REQUEST for N5:** the actual new material — new projects/work,
  roles, dates, results, any images — dropped into `../project-docs/`. Real copy
  only; the team writes none of it.

Until these are answered, no REQ-002 exists. R6's follow-up REQ for the five
remaining routes is also held here rather than opened blind, because N4 and N5
may change what it should say.

### Answered 2026-09-02 by the owner — Q14–Q17 CLOSED
...
| Q17 | see below | Material for N5 arrives as **source repos + live URLs**, not as finished copy. |

**Q14 — only half answered.** He answered the scope half (`ทั้งเว็บ`) and not the
sequencing half (before / after / together with R6's five-route rebuild).
**Scope Porter applied, stated so he can overrule it:** N4 and R6 are merged into
**one** REQ-002, because building the five routes to the old bar and then
re-doing them to a bolder one is the same work twice. If he wants them separate,
or the five routes shipped first as-is, he says so and REQ-002 splits.

**Q17 — the owner's material, verbatim as given (2026-09-02).** He asked back
`พวกนายสามารถสามารถดูcode บน github ได้มั้ย มีgithub กับ domain url` and handed
over two of his real projects:
...
- **Answer to his question: yes** — all four repositories are reachable read-only
  from this machine and every branch he named exists (checked 2026-09-02). No
  role has cloned anything yet.
- **One typo in his message, recorded not corrected silently:** the fourth link's
  visible text says `ong-match-front` while its href points at `ong-match-back`.
  The repo he *typed* (`ong-match-front`) exists and is the one recorded above.
  Low-confidence assumption — he confirms or corrects it.
- **This is source material, not copy.** R4 still binds: reading a repo tells us
  what a thing does, not what his client-facing portfolio entry should say, and
  it carries none of the facts a portfolio needs (his role, dates, client,
  results). Who writes that text is **Q21**, unanswered.
- **Boundary, so no role gets this wrong:** the standing rule about not tracking
  the owner's commits is about **his commits on the portfolio repo** — it does
  not forbid reading these four handed-over repos as source material. Reading is
  read-only: no writes, no pushes, no branches, no deploys, no accounts created,
  no logins. **If a screen can only be seen behind a login, that is a DATA
  REQUEST — he screenshots it. Nobody asks him for credentials.**

**Where each ask goes now:**

- **N4 → `requirements/REQ-002-whole-site-step-up-five-routes.md`**, opened
  2026-09-02, `READY_FOR_SA`, merged with R6's five-route rebuild (see the Q14
  note above). REQ-001 is unaffected and stays DELIVERED.
- **N5 → `requirements/REQ-003-portfolio-content-refresh.md`**, opened
  2026-09-05 by Porter after the owner re-raised it, status `DRAFT — BLOCKED`.
  Detail now lives there; the paragraph below is kept as the record of why it
  was held from 2026-09-02 to 2026-09-05, and **Q22** (the DATA REQUEST for the
  four facts code cannot supply) was added there on 2026-09-05.
- **N5 was held for three days, no REQ written.** Scope is answered (Q16) and
  sources have arrived (Q17), but the content REQ cannot be made testable until **Q20**
  (may these two projects be shown publicly, any client/NDA limit?) and **Q21**
  (who writes the entry text, and the facts code cannot supply) are answered.
  Both asked in Thai in chat on 2026-09-02 and repeated here:

  - **Q20 — permission to publish:** may `learning.develyst.online` and
    `ong.develyst.online` — their names, screenshots and links — appear on his
    public portfolio? Is either client work with a confidentiality limit, and is
    there anything on them that must not be shown?
  - **Q21 — who writes the copy, and the missing facts:** does he want the team
    to draft each entry from the repos and the live sites for him to approve
    (an explicit lift of R4 for this text, the way Q10/Q11 lifted it for the
    quotes) — or will he write the entries himself? Either way, code cannot
    supply: **his role on each project, the dates, the client/employer, and any
    result or number he wants shown.**
