...
### TEST-004 intake — Porter (PM), 2026-09-05

Tanya returned **`TEST_FAILED`** on TEST-004. Accepted **as written**: I do not
soften a QA verdict and I have not re-scoped a single check to make it pass.
QQ4–QQ7 are answered in `tests/TEST-004-req002-site-wide-acceptance.md`.

**What TEST-004 CLOSES — three things that were open for days:**

- **AC6 → MET.** 0 hits in 96 text + 96 source checks, all six routes, both
  viewports. The gap in *my* acceptance evidence is filled, and it was a clean
  result rather than a lucky one.
- **DEF-1 → CLOSED, on evidence.** All nine `/about` lightbox images have a
  non-zero box **and** decoded pixels at 1280 **and** 360, legible in the shots;
  `REGRESSION` S13 flips FAIL → **PASS**. The standing instruction — *the owner
  is not told "fixed" until S13 re-runs* — is now satisfied, so **as of
  2026-09-05 the owner may be told it is fixed**, and he is.
- **SQ12 → SETTLED, in Sober's favour.** There is no outline ring, but the field
  border goes `rgb(68,60,92)` → `rgb(164,136,255)` on a real `Tab` and is plainly
  visible in the shot. Fern's contrary reading was taken unfocused (his own note
  says his harness's `Tab` did not move focus). Only "is a 1px border change
  enough under WCAG 2.4.7" survives, and that is the owner's, not a defect.
- Also settled, not an AC: **SQ7's drawer half for a real visitor** — mobile nav
  opens, lists six routes, navigates and closes at 360. That is the *drawer*
  only; the modal/lightbox *look* gate stays Sober's and TEST-003 stays separate.

**What TEST-004 OPENS — and neither is being absorbed:**

- **DEF-2 (MAJOR) — Home hero no longer fits at 360×740.** CTA 2 "Get in touch"
  runs 706–750 and is sliced by the 740 fold; the hero quote sits 782–809,
  entirely off-screen. This fails **`REGRESSION` H8**, which is a criterion the
  owner himself accepted at REQ-001 and which passed on 2026-09-02.
  **No cause is named by anyone**: QA did not test a pre-step-up tree and does
  not read git, and I will not convert "passed then, fails now" into "REQ-002
  broke it". Whether this is fixed or accepted the way the 1280×600 fold was
  accepted is the owner's call — **Q23**. Evidence:
  `../project-docs/qa-test004-2026-09-05/b2-h8-fold-360x740.png`.
- **DEF-3 (MAJOR) — `/services` at 360 strands two of three columns.** An 887px
  table inside a 303px `overflow-x:auto` scroller: only the "Service" column is
  on screen, beside ~530px of empty rows, with **no scrollbar, no edge fade and
  no hint text**. The page itself does not overflow, so S8 correctly passes —
  **no existing check covered this**. On the record, and not a team oversight:
  SPEC-002 §Flow item 2 *deliberately* keeps this a `<table>` (cards would delete
  the three visible headings `Service` / `What it covers` / `Stack`, which R4
  forbids, and break its `role="region"`). Affordance now vs. rethinking the
  mobile layout later is the owner's — **Q24**. **Unknown and recorded as
  unknown: whether DEF-3 pre-dates REQ-002.** Nobody has tested a pre-step-up
  tree, so nobody may say. Evidence: `b6-services-table-360.png`,
  `mobile-services-full.png`.

**Two checks that did not run, both legitimately:**

- **S11 (`npm run build`)** — not run, because a `next` process this session did
  not own (PID 8508) held port 3000 and `front/.next`. **TEST-003 is the build
  round and owns S11**; I did not ask for it to be re-run here. QA followed the
  standing rule that an occupied port is routed around, never cleared.
- **H5 (no new/altered string on Home vs. the pre-REQ baseline)** — **unrunnable
  as written, permanently**: no captured baseline exists anywhere QA may read.
  **I do not count H5 against AC3** — a criterion cannot fail on a check nobody
  can run. Fern's `git archive HEAD` A/B (TASK-011 §3) is the nearest artefact
  and is *not* a QA baseline; I point at it, I do not promote it. Rewriting H5
  is Tanya's call in her own file, asked not instructed. Owner told in one line
  (**Q25**).

**Where DELIVERED now stands: 2 of 8 ticked (AC4, AC6).** Six criteria remain
and the split is unchanged in kind — **the owner closes AC2, AC5, AC7** (his
eye, his R4 rule, his Q19), **AC3 waits on Q23** (H8/DEF-2), **AC1 waits on AC2**,
and **AC8 waits on the build (TEST-003)**. DEF-3 is not itself an AC — it is a
new defect found inside the REQ and it goes on the record here so that
"DELIVERED" can never be read as "nothing outstanding on a phone".

**Routed to Sober 2026-09-05** (`inbox/SA.md`): DEF-2 and DEF-3 as REQ-level
concerns, explicitly **not** as REWORK and **not** as a task order — whether
either becomes a TASK, and what it would cost, is his call, and both are pending
the owner's Q23/Q24 so nothing is built on my assumption.

> **SUPERSEDED the same day — the owner answered.** Q23 = `แก้ก่อนส่ง` (fix DEF-2
> before DELIVERED) and Q24 = `ใส่สัญญาณเลื่อน` (give DEF-3 a visible scroll
> signal now). The "do not spend until he answers" hold above is **lifted**; both
> are committed work. See §Owner's answers — 2026-09-05.
...
**What Porter did NOT read into these five words, stated so silence is not taken
as a decision:**

- **Q23 says "fix", it does not say what broke it or who pays for it.** No cause
  for DEF-2 is named by anyone and none is invented here; whether the repair
  becomes a TASK, how, and what it costs are **Sober's**, not Porter's.
- **Q24 says "add a scroll signal", it does not redesign the table.** SPEC-002
  §Flow item 2 keeps `/services` a `<table>` on purpose (cards would delete the
  three visible headings, which R4 forbids, and break `role="region"`). What the
  affordance *is* — a scrollbar, an edge fade, hint text, or something else — is
  Sober's design call inside that constraint. Porter proposes none.
- **AC7 = `เอาออก` is an answer about the toggle, not a licence to delete code.**
  Whether the dead `ColorSchemeToggle` component and its barrel re-export are now
  removed, and whether that is worth a TASK at all, is Sober's call. SPEC-002 SQ1
  already told him the premise had moved.
- **Nothing here answers Q18 or Q25.** Both stay open with the owner.

**Where DELIVERED now stands: 5 of 8 ticked (AC1, AC2, AC4, AC5, AC6).** Three
criteria remain and **none of them is with the owner any more** — the ball left
him with this message. AC3 needs DEF-2 repaired and H8 re-run; AC7 needs the
five non-Home routes looked at; AC8 needs `npm run build` (TEST-003). DEF-3 is
not an AC but it is now committed work and DELIVERED must not be read as
"nothing outstanding on a phone" while it stands.

**Routed to Sober 2026-09-05** (`inbox/SA.md`): the hold on DEF-2 and DEF-3 is
**lifted** — both are the owner's committed decisions now, not Porter's
assumptions — plus Q19's answer for the five-route shell. Still not a REWORK and
still not a task order: Sober places the work, or says it needs none.

## TEST-005 request — the closing QA round on REQ-002 — Porter (PM), 2026-09-05

**Why this round exists.** All three fixes the owner asked for are built and
reviewed `DONE` by Sober (TASK-013 DEF-2 / TASK-014 DEF-3 / TASK-015 AC7-Q19).
None of them ticks an acceptance criterion by itself: **AC3 ticks only on a QA
re-run, AC7 only on a QA look at the five non-Home routes.** So this is the
round that decides whether REQ-002 can move off `SPEC_DONE`.

**Requested of:** Tanya (QA). Local only (`cd front && npm run dev`), her file,
her method, her verdict — Porter requests checks, never how they are run and
never what they must return. Status `REQUESTED` on the board until she opens it.

### The five checks I need back

| # | Check | What it settles |
|---|-------|-----------------|
| 1 | **REGRESSION H8** re-run at 360×740 — hero renders name, nickname/role, lead, **both** CTAs and the hero quote above the fold | **AC3.** The only thing that ticks it; TEST-004 left it `FAIL` |
| 2 | **REGRESSION S14** re-run on `/services` at 360 — is the visitor *told* the wide table scrolls | **DEF-3 closed or not.** TEST-004 left it `FAIL` |
| 3 | **No colour-scheme control on the five non-Home routes** — `/about`, `/services`, `/portfolio`, `/blog`, `/contact` (H1 has only ever run on Home) | **AC7.** The owner answered `เอาออก`; a source read does not tick it |
| 4 | **A desktop eye on `/services` at 1280** — does the page still look right there after the DEF-3 change | Sober's TASK-014 carry (a): the desktop leg was measured but **never seen**, and desktop is what the owner ticked AC2 on |
| 5 | **Arrow-key scrolling of the `/services` scroller** — reachable and usable by keyboard, or not | Sober's TASK-014 carry (b), left UNVERIFIED because the harness sent no key events. **Not an AC** — an open carry I do not want to lose |

**Whether anything else in REGRESSION must be re-run to trust those five is
Tanya's call, not mine.** The changes were three CSS/deletion edits (Home hero
mobile block, `/services` scroller, a component that was mounted nowhere), so I
am not asking for the full sweep — but I am not forbidding it either.

### What this round does NOT have to carry

- **H5** stays `NOT_TESTED` and I still do not count it against AC3 (QQ7,
  2026-09-05). Rewriting it is her file and her call, and Q25 is with the owner.
- **S11 / `npm run build`** is **TEST-003's**, deliberately, and stays there.
- **SQ13** (`/services` 1156px tall on a phone) is not a defect and is not in
  this round — it goes to the owner only after he has seen the DEF-3 fix.
- **SQ7's modal-look gate** is Sober's and is untouched by this round.

### Order, and the correction to "one round from DELIVERED"

**TEST-005 first, TEST-003 second** — my call, and the reason is not seniority
of the ask: TEST-005 covers work that was written **three sessions ago and has
never been looked at**, and a `FAIL` there costs another SA→FE loop, so it is
worth finding first. TEST-003 has been open since 2026-09-03 and is unchanged.

**REQ-002 is TWO QA rounds from `DELIVERED`, not one**, and this is written down
so nobody re-reads it as one: **AC3 + AC7 ride on TEST-005**, but **AC8 is still
`PARTIAL` and its open half is `npm run build` = S11 = TEST-003.** All three
must land before Porter sets `DELIVERED` and hands the owner his final sign-off.
...
(SA Lead asks here; Porter answers as `> answer: ...`. The two below are for the
**owner** — Porter asked them in Thai in chat on 2026-09-02.)

- **Q18 — open, owner.** Is `AI + robotic + IOT` a **visual** direction only, or
  does he also want the site to *say* he works in AI / robotics / IoT? This
  matters because the second reading changes his positioning in words, and R4
  forbids any role writing a claim about him that he did not make. Nobody
  assumes; this REQ proceeds as visual-only until he answers.
  **Not blocking the start of spec work** — it blocks any copy consequence,
  which this REQ has none of.
- **Q19 — CLOSED 2026-09-05 by the owner.** Asked: do the other five routes lose
  ...
  **They lose it — the same word he used for Home.** The light scheme SPEC-002
  SQ1 warned about ("a light scheme that has never been designed — new REQ-level
  work") is therefore **not** needed, and no such REQ is opened. **AC7 is not
  ticked by this answer**: the toggle being absent must be *seen* on the five
  non-Home routes, and TEST-004's H1 ran on Home only — see §Acceptance Criteria
  AC7. Relayed to Sober for the shell of those five routes.
- **SQ7 — the owner answered HALF of it, 2026-09-03. Recorded here because
  `specs/` is Sober's file and Porter never writes in it.** SQ7 is Sober's
  question in `specs/SPEC-002-site-wide-step-up-five-routes.md` §Questions: no
  `Modal` and no `Drawer` ever opens on `develop` (no Portfolio detail view, no
  image lightbox, no mobile navigation), found by Fern while building TASK-006,
  pre-existing and outside REQ-002. Sober asked the owner two things.
  > answer (owner, verbatim, in chat): **`SQ7=ให้ Tanya reproduce ก่อน`**
  - **Answered — SQ7's second ask (the independent QA reproduction): YES, and it
    comes first.** Porter has requested it from Tanya as **TEST-002** (local
    only, clean checkout, the three triggers: a Portfolio card, an About /
    testimonial lightbox, the mobile burger). Board row added under §QA / Tests.
  - **NOT answered — SQ7's first ask (the scope decision):** whether fixing the
    defect is in scope now as its own defect REQ, or whether REQ-002 ships
    visual-only with the modal look UNVERIFIED. The owner said "reproduce
    first", which orders the two asks; it does not decide the second. **No role
    infers the scope call from it.** Porter puts it back to the owner in Thai
    once Tanya's verdict exists, so he decides on a verified defect rather than
    on one engineer's report.
  - **What holds meanwhile, unchanged:** TASK-006's `Modal` skin stays
    UNVERIFIED and TASK-008 (About lightbox) + TASK-009 (`ProjectModal`) inherit
    that gate — the modal look is never ticked from source. **No role works
    around the defect from a route file** (Sober's standing line in SQ7). The
    five-route visual work is not stopped by this: TASK-008..011 continue on
    their own tracks.
  - **If Tanya cannot reproduce it**, that is a real outcome, not a failure —
    it goes back to Sober as a fact via Porter, and the scope question may
    dissolve.
  - **QA leg CLOSED 2026-09-03 — TEST-002 = CANNOT_REPRODUCE.** All three SQ7
    triggers open, paint and close on the locally served tree (`next dev`):
    Portfolio `ProjectModal`, both `/about` lightboxes, the 390px burger
    `Drawer`. Portal not empty, scroll locked, 0 console errors, run twice —
    evidence in `tests/TEST-002-sq7-modal-drawer-repro.md`. Porter records this
    as a **fact about what the site served on 2026-09-03**, not as "Fern was
    wrong": Fern reported it on 2026-09-02 and what changed in between is
    checkout state, which the standing rule puts off-limits to every role.
    Nobody here knows whether the defect is gone or was never in this tree, and
    nobody guesses it.
  - **Porter's calls on the verdict (2026-09-03), so silence is not read as a
    decision:** (a) `CANNOT_REPRODUCE` is **accepted as written** and not
    remapped onto PROTOCOL's four TEST statuses — `TEST_PASSED` would claim the
    modal *look* was accepted (it was not) and `NOT_TESTED` would be false. The
    vocabulary gap is Tanya's fair point and is noted, not resolved by me.
    (b) A **second QA round on a `next build` + `next start` output is granted**
    (QQ1) — dev is not the shape that ships, and Tanya found a stale `next build`
    output sitting in `front/.next` before her run, so the built surface is
    genuinely untested. Requested as **TEST-003**.
  - **Still open, and still the owner's (QQ3):** the scope half. Its shape has
    changed — with the overlays opening, "fix it now as its own defect REQ" may
    have nothing left to fix, but the **modal *look* is still UNVERIFIED** and
    that tick is Sober's gate, which this round did not touch. Porter is asking
    the owner in Thai: ship REQ-002 with the modal look unverified, or hold
    until it is verified. **Whether the verdict lifts Sober's gate is Sober's
    technical call, not Porter's** — relayed to him as a fact via `inbox/SA.md`.
- **DEF-1 (found during TEST-002, 2026-09-03) — every `/about` lightbox
  thumbnail renders 0x0, MAJOR, and it is NOT SQ7.** All 9 `ImageLightbox`
  frames on `/about` (4 certificates + 5 conversations) measure 0x0 px, so a
  visitor sees **no image at all** in either grid — only a hint bar and a
  caption. The images load and display correctly *inside* the opened modal.
  Details, repro and screenshots: `tests/TEST-002-…md` §Defects.
  - **Porter's routing (QQ2 answered): (b) — a NEW finding, relayed to Sober.**
    Grep of `specs/` + `tasks/` on 2026-09-03 shows no record of a collapsed
    thumbnail anywhere, so it is not known-and-in-flight; and `/about` is the
    live surface of TASK-008, which is BLOCKED, so it is not "its own thing"
    sitting far from the work either. Sober places it (existing TASK vs new TASK
    vs out-of-REQ-002 defect that needs a REQ from Porter). Porter does not
    place it and does not propose a fix.
  - **The open unknown Porter will not guess:** SPEC-002 says About's
    certificates and testimonials were deliberately left unchanged by TASK-008,
    yet Tanya's tree included TASK-008's built files. Whether DEF-1 pre-existed
    or arrived with that work is **not established by anyone** — Sober's to
    determine, nobody's to assume.
  - **The owner's half — ANSWERED 2026-09-04 (SQ10).** Asked of him in Thai on
    2026-09-03: does the repair ship inside REQ-002 or need its own defect REQ?

    > answer (owner, verbatim, in chat): **`SQ10=รวมใน REQ-002`**

    "Include it in REQ-002." So **TASK-012 ships inside REQ-002** — no separate
    defect REQ is written, and REQ-002 is not DELIVERED until TASK-012 is DONE.
    TASK-012 is unblocked on the board as of 2026-09-04; the fix itself, its DoD
    and its ordering against TASK-010/011 stay Sober's, as does every word of
    `tasks/TASK-012-image-lightbox-thumbnail-collapse.md` — Porter does not
    write in it and has not proposed a fix.
...
- **Q23 — open, owner. DEF-2, Home hero at 360×740 (asked in Thai 2026-09-05).**
  On a phone the second button "Get in touch" is cut by the bottom of the first
  screen and the quote under it is off-screen. This fails `REGRESSION` **H8**, a
  bar he set himself at REQ-001 and which passed on 2026-09-02. Two ways out and
  **neither is chosen by any role**: (a) fix it before REQ-002 is DELIVERED, or
  (b) accept it the way the 1280×600 fold was accepted as "Known and accepted"
  item C. **No cause is stated to him** — nobody tested a pre-step-up tree, so
  "REQ-002 broke it" and "it is unrelated" are both unsupported. Screenshot:
  `../project-docs/qa-test004-2026-09-05/b2-h8-fold-360x740.png`.

  > answer (owner, verbatim, in chat): **`Q23=แก้ก่อนส่ง`**

  **CLOSED 2026-09-05 — option (a): fix it before REQ-002 is DELIVERED.** He did
  not accept it the way he accepted the 1280×600 fold. **AC3 is no longer held on
  a question** — it is held on the repair plus a QA re-run of H8, and nothing
  else ticks it. **Still true and still not guessed: no cause is named**, by QA,
  by Porter or by him; "REQ-002 broke it" remains unsupported. Routed to Sober
  `inbox/SA.md` 2026-09-05 — whether it becomes a TASK and what it costs is his.
- **Q24 — open, owner. DEF-3, `/services` table at 360 (asked in Thai
  2026-09-05).** Only the first of three columns is on screen; the other two sit
  behind a sideways scroller with no scrollbar, no fade and no hint, so the
  section reads as empty rather than scrollable. Already on the record and *not*
  an oversight: SPEC-002 §Flow item 2 keeps this a `<table>` on purpose, because
  cards would delete the three visible headings (R4) and break `role="region"`.
  His call: (a) add a visible affordance now, (b) rethink the phone layout later
  as its own piece of work, or (c) accept as-is. **Unknown and left unknown:
  whether this pre-dates REQ-002** — no role has tested a pre-step-up tree.
  Screenshots: `b6-services-table-360.png`, `mobile-services-full.png`.

  > answer (owner, verbatim, in chat): **`Q24=ใส่สัญญาณเลื่อน`**

  **CLOSED 2026-09-05 — option (a): add a visible scroll signal now.** Not (b)
  "rethink the phone layout later" and not (c) "accept as-is", so DEF-3 is
  committed work inside REQ-002. **What the signal is, is not decided by this
  answer and not by Porter** — the `<table>` and its three visible headings stay
  (SPEC-002 §Flow item 2, R4, `role="region"`), and the form of the affordance is
  Sober's inside that constraint. Whether DEF-3 pre-dates REQ-002 is **still
  unknown and stays written down as unknown**; his answer does not decide it.
  Routed to Sober `inbox/SA.md` 2026-09-05.
- **Q25 — open, owner. FYI-with-a-decision (asked in Thai 2026-09-05).** One
  ...
  versus the pre-REQ baseline"), can never be run: no baseline was ever captured
  and stored, so QA has nothing to diff against. Porter does **not** count it
  against AC3. Either it is rewritten into something runnable (Tanya's file,
  Tanya's call — asked, not instructed) or it stays permanently `NOT_TESTED`.
  Only the owner may say whether he wants that gap closed at all.
- **Q26 — open, owner. DATA REQUEST, non-blocking (asked in Thai 2026-09-05).**
  ...
  `next start` **"does not work"** with that setting. QA ran the production build
  on **both** surfaces — `npm run start` and `node .next/standalone/server.js` —
  and **they agreed on every check**, so **AC8 is ticked either way and nothing
  is blocked**. What no role may go and look at is which surface his droplet
  (`pm2` process `portfolio-frontend`) actually serves. His one-line answer
  decides only what future QA rounds mirror; it changes no code and no AC. Source:
  tests/TEST-003-sq7-build-round.md §Questions QQ10.
