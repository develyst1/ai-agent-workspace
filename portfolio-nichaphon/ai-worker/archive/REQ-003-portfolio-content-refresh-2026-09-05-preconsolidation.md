...
- Status: **DELIVERED 2026-09-05, Porter.** **8 of 8 AC ticked** — AC-d closed on
  ...
  Q22-b, Q28, Q29, OBS-8 and both Observations survive DELIVERED and are NOT
  closed. See **§Delivery**.
...
So **SQ17 is not re-put as a branch question: the state has already answered it.**
SQ17 asked whether work sitting on `D1` should be handed over on `develop`, per
PROTOCOL. `develop` **is** the work, at the same SHA, pushed. There is nothing
for the owner to choose between. What is left is not SQ17 and is not the team's
in any case: **whether it goes further (`main` / `production` = a deploy) is the
owner's hand alone.** That is what I relay to him, as a status line, not a
...
**It is not a defect and it does not block anything.** QA proved the button is
reachable, uncovered and hit-testable after ordinary scrolling, with a visible
scrollbar on the modal body. It is recorded as **Observation 8** below and put to
him in the delivery summary **with the picture**, answerable in one line. If he
wants it moved, that is a new scope call for a later REQ — the team does not
restyle a modal on QA's observation alone (R5).
