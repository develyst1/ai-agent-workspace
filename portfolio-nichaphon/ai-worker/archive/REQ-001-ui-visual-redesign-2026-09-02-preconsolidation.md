...
- History: opened 2026-08-30 as DRAFT/BLOCKED with six open questions (title
  ...
  IN_SPEC to match the board — SPEC-001 is active. Updated **2026-09-02**: the
  owner answered **AC1 (yes), AC2 (`อนุมัติ`) and Q4** — see §Owner's answers
  2026-09-02. Both blocking items are closed, R5's quote-2 Thai now records the
  string he wrote himself. The eleventh criterion was ticked the same day by QA's
  one-case confirm-tick round, so **all 11 acceptance criteria are ticked** and
  the REQ moved to **DELIVERED on 2026-09-02** — see §Delivery. Two forward asks
  from him are parked in §New asks — they are not part of this REQ.
...
- [x] Any quote shown matches R5 character-for-character — Thai quotes 1–3 as
      ...
      — **ticked by QA 2026-09-02** under §QA confirm tick (Porter would not tick
      his own R5 edit). All three strings rendered on `/` are R5-exact: quote 4
...
`DELIVERED` is the one unticked criterion below. Still outstanding from him and
**still non-blocking**: B, C, D, F, G, H in §Home acceptance review.

## QA confirm tick — issued 2026-09-02 by Porter

**One case, Home only, local only.** Porter changed R5's canonical Thai for
quote 2 to match what renders; he does not tick a "matches character-for-
character" criterion from his own edit.

1. Compare every quote rendered on `/` to R5 as it now stands in this file, by
   exact string equality. Expected: all match, quote 2's Thai now 42 characters
   (`ผมไม่ทำงาน "ให้" ใคร ผมทำงาน "ร่วม" กับใคร`).
2. If they match, tick it and give the REGRESSION H3 check its baseline (R5 as
   it now stands). If anything still differs, report the difference to Porter —
   again: no tick, no defect, nothing edited.

No other case is re-run. Standing rule applies: report what the page renders,
never git. Deliverable: a short dated section appended to TEST-001 and a verdict.
