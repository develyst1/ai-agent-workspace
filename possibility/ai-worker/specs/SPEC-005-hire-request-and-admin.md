...
- **Parsing hardened 2026-09-23 (TASK-018):** the value is unquoted (leading/trailing `"`/`'`, whole value *and* each entry, so `"a@b","c@d"` works), entries without `@` are dropped with a count-only WARN, and an empty result is still the fatal startup path. Rationale: a quoted `.env` value used to start cleanly and make **nobody** admin — silence is the worst outcome for an access rule.
