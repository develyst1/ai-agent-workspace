...
| R-19 | Hire button: W-1 → 201 → W-2 (exact wording, email + discount) → disabled W-3, in TH and EN; reopen → W-3; second POST → 409 `ALREADY_EXISTS`; `GET /ideas/:id.hireRequested` true; user B hiring A's idea → 404 | REQ-004 AC-1/AC-2 | 2026-09-23 | 2026-09-23 (TEST-006 #1/#3/#8) | PASS |
| R-20 | Admin surface isolation: non-admin `/admin` → W-8 not-found, `GET`/`PATCH /admin/*` → 404; signed-out → landing/sign-in + 401; zero admin links in the UI (desktop + 375 px) | REQ-004 AC-3/AC-4/AC-6 | 2026-09-23 | 2026-09-23 (TEST-006 #4/#5) | PASS |
| R-21 | Hire request snapshots user tier + discount at request time: row keeps `Ordinary/0%` after the user rises to `Visionary/20%` | REQ-004 AC-5 | 2026-09-23 | 2026-09-23 (TEST-006 #7, DB record level) | PASS |
| R-22 | Admin page as an ADMIN_EMAILS account: `/admin` + `GET /admin/hire-requests` → 200; list newest-first with date, name, email, 120-char preview, 3 scores, idea tier, user tier at request + discount; row expand → full text; steps drawer (5 steps, provider/model/latency); contacted switch → PATCH 200 → persists across reload → undo; non-admin still 404 after the admin-list change; usable at 375 px (horizontal table scroll) | REQ-004 AC-1/3/4 | 2026-09-23 | 2026-09-23 (TEST-006 #6/#6b/#6c/#6d) | PASS |
| R-23 | SIT sign-in alive: `POST /auth/login` 200 + `GET /health` `db:ok` (escaped DEF-1: the 09-23 admin-list deploy broke every DB-touching request on SIT; caught by QA before any user hit it) | REQ-006/REQ-002 guard | 2026-09-23 | 2026-09-23 (TEST-006 DEF-1 re-check) | PASS |

> Note (2026-09-23): REQ-008/TASK-015 will redraw these pages — R-18's photo lines die with that redesign; Porter/Sober will restate them. R-19..R-21 are behavioural and survive it.
