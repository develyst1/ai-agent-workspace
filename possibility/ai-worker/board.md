...
| REQ-004 | Hire request + owner’s admin page | HIGH | IN_TEST — 2026-09-23, Tanya; user side PASS on SIT; admin half needs deploy + the ADMIN_EMAILS line, `specs/SPEC-005-hire-request-and-admin.md` §Amendment | Porter → owner (deploy + env) → Tanya |
...
| TASK-015 | FE redesign v2: remove photos, StageMark + StageScale + the light | SPEC-009 | TODO — 2026-09-23, Sober, PRIORITY, `tasks/TASK-015-fe-redesign-v2-no-photos.md` | Fern | TASK-009 ✓ |
...
| REQ-004 | TEST-006 | IN_TEST — 2026-09-23, `tests/TEST-006-hire-request-and-admin.md` | User side + snapshot 7/8 PASS on SIT; admin-page half NOT_TESTED (needs owner's Google session — options with Porter) |

## Blocked / waiting

| Item | Waiting on | Since | Pointer |
|------|-----------|-------|---------|
| DR-9 rotate the SIT Postgres password + update SIT `.env` (it was printed into an engineer session; not in any file) | the owner, via Porter | 2026-09-23 | `tasks/TASK-016-be-admin-email-list.md` §Questions Q-1 |
