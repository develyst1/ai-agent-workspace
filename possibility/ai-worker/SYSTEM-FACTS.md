...
- **REQ-004 (hire button + admin page) deployed to SIT by the owner, 2026-09-23** (*"deploy ไปนานแล้ว"*).
- **Owner refuses further QA sign-in clicks: *"ไม่ต้อง ก็ให้เทสรหัสอื่นไปสิ"* (2026-09-23)** — QA is to test admin with some other account instead. ⚠️ That requires admin to be more than the single Google account `siegkung@gmail.com` (09-18 rule). Porter asked him to confirm the exact change before anyone builds it.
- **DECIDED (owner, 2026-09-23: *"1"*): admin is a LIST of emails configured in `.env` (not the single hard-coded Google account).** `siegkung@gmail.com` stays an admin; a QA email account is added to the list on SIT so QA can test the admin page without the owner. Admin access is by email match, whichever sign-in method (Google or email+password). Supersedes the 09-18 "only siegkung@gmail.com" line.
- **Observed 2026-09-23 (Jason, self-reported): the SIT `DATABASE_URL` including its password was printed into an engineer's session output** by a mis-built shell command. Not in any file, not in git, not in a TASK or log. ⇒ DR-9: the owner should rotate the SIT Postgres password and update the SIT `.env`.
- **The BE port comes from `.env PORT` (4019 on the dev machine); 4000 is only a default** (Sober, SPEC-001 amendment 2026-09-23).
- **REQ-004 admin-list change deployed to SIT + `ADMIN_EMAILS` set by the owner, 2026-09-23** (*"deploy BE done + add Email done"*).
- **DR-9 closed by the owner (2026-09-23): he declines rotating the SIT DB password; deleting it from any log is enough for him.** Porter searched the whole workspace and both product repos on 09-23: **the SIT connection string is in NO file** — the only workspace match was a dummy `postgres://x:x@127.0.0.1:1/x` in TASK-001, and `.env` files (owner's, git-ignored) legitimately hold it. Nothing was deleted because there was nothing to delete; the exposure was a session transcript only. Risk accepted by the owner.
- **Observed 2026-09-23 (Tanya, TEST-006 DEF-1): after the admin-list BE deploy, SIT sign-in is DOWN for every account (login 500; `/api/v1/health` 502); the static FE serves fine and non-DB API paths answer. The same build + same accounts + same DB work locally ⇒ SIT-deploy-specific (env or DB reachability), not a code defect.** Raised to the owner 09-23 — only he can read the SIT process/env.
