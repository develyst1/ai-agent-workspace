...
| 🟢 DEF-25 — เอกสารอื่นๆ glued label+value (5 forms) | QA 2026-09-07 (TEST-012): EMPTY ✅×5 · POPULATED ✅ อ.4 · populated UNVERIFIED on other 4 (no sample) → Porter's call | Fixed on a4/a14/a9-transport/a15/a9-destroy (TASK-051/052/053), SA-verified. **TEST-012:** all 20 renders 200/0 ORA. EMPTY branch PASS on all 5 (อ.4 38419·อ.14 27300·อ.15 35429·a9t 37956·a9d 38362 — label separate, dotted line prints, never glued). POPULATED PASS on **อ.4 38427** (`17.☑ … test` on the write-in, stakeholder's accepted baseline). **POPULATED UNVERIFIED on อ.14/อ.15/a9-transport/a9-destroy** — no request with item-13 data in the candidate set (all item-13 unticked = genuinely empty). Not a fail; a9-family share the base element, a14 mirrors อ.4, Jason fail-on-revert 34/34. **Porter: accept as covered, or DATA REQUEST a populated item-13/17 sample.** See tests/TEST-012. **Retest on a clean `mvn clean` build (precompiler 58/58 this boot, fresh .jasper) reproduced identical — not a stale-binary artifact.** |
...
| TEST-012 | DEF-25 close: `เอกสารอื่น ๆ` label+dotted field on all 5 forms | 🔴 HIGH | OPEN 2026-09-07 → Tanya (stakeholder said test now). Real **/download** per form (a4 item 17 · a14 · a9-transport · a15 · a9-destroy item 13), **BOTH branches**: populated ⇒ value **on** the dotted line, label separate · empty ⇒ dotted line **still prints**. ⚠️ **Confirm the sample HAS item-13/17 data before passing the populated branch** — an empty request renders identically to a broken row (the trap that cost 3 rounds). No sample ⇒ report "unverified", do not pass. Baseline a4=38427 (stakeholder already accepted by eye) | Tanya (QA) |
...
|------|-----------|--------------------------|
| 🟢 DEF-21 (REQ-034 pre-work) — อ.15 drops 2 rows | STRUCTURAL FIX QA-CONFIRMED 2026-09-07 (TEST-010); tick/value branch = accepted gap | Fixed via TASK-047 (a15 got its own refrow4/refrow5 bands; SA-verified, transport frozen). **TEST-010 done:** both bands render on **9/9** real อ.15 (refrow5 label MISSING pre-fix → silent drop gone), all 200 / 0 ORA. **0/9 ticked or valued** — these requests have null `BUYER_DOC_TYPE`/`BUYER_DOC_NO`/attorney-ID + no 00014/00020 attachment (item-12 otherwise has real person2 data, so genuinely empty, not absent). Silent-drop = closed; **ticked/populated branch recorded as accepted gap** (not closed on an unexercised render — re-verify if an อ.15 with 00014/00020 attached appears). See tests/TEST-010. |
...
| 🟠 DEF-25 (label+value glued, no dotted line) | **a4 ACCEPTED by stakeholder 2026-09-07** — item 17 now label + real dotted value field (*"a4 ตรงนั้น ok แล้ว"*). **5-form defect:** a4 ✅ · a14 in flight (TASK-052, independent builder) · 🔴 **a9-transport / a15 / a9-destroy item 13 BLOCKED** — shared base builder (`A9…Base:234`), fix needs an element in transport's **FROZEN** evidence.jrxml ⇒ **stakeholder must rule: they fix transport themselves (as bfc4b76) or lift the freeze.** Team will not touch a9-transport without it | stakeholder (ruling) |
