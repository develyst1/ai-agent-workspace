...
| 🔴 DEF-25 (อ.4 item 17) | RAISED 2026-09-04 by stakeholder → Sober/Jason | `เอกสารอื่น ๆ (ถ้ามี)` renders `17.☑ เอกสารอื่น ๆ (ถ้ามี)  test` — **value glued to label, no dotted line.** Source: `A4CheckListReportBuilder.java:203-204` builds **one concatenated string**; the "dotted line" is literal dots in the empty branch, so **populated ⇒ the line vanishes**. REQ-033's defect class surviving in an unchecked row. Fix = own row type + band (label element + value textField with Dotted bottomPen), like transport's `ตาม`. ⚠️ **`A4CheckListPreviewBuilder:85` has the same string — same change** · **check other forms' เอกสารอื่น ๆ rows (a6 item 8, a9 item 13, a14, a15) and REPORT, don't fix beyond a4** · verify real /download **both** branches (empty + populated) |
...
| TEST-010 | อ.15 DEF-21 close: which real request exercises `ReqSaleDom00014` + `00020` | HIGH | OPEN 2026-09-03 → Tanya. Stakeholder supplied 9 real อ.15 (type=5 w/ buyer): **35429·35273·35141·35155·34950·34614·34903·34885·34916**. Render each via **real /download** (preview/mock hides this path — that IS DEF-21), report row present? / ticked? / values on lines?. ⚠️ **CODES CORRECTED 2026-09-03** (Sober caught it): อ.15 = **`ReqSaleDom`**, `ReqSaleInt` is อ.14's group (A15ReportBuilder.java:22 / A14CheckListReportBuilder.java:31, source-verified). With the wrong group Tanya would find nothing on all 9 and wrongly log an accepted gap. **38406 is NOT an อ.15** (REQ-035 sample only). If none has 00014/00020 attached ⇒ record as **accepted gap**, do NOT close DEF-21 on an unexercised render | Tanya (QA) |
...
| TASK-051 | **DEF-25**: a4 item 17 เอกสารอื่นๆ split label+dotted field (+ person-row caption-reclaim re-measure) | DEF-25/SPEC-039 | **ASSIGNED → Jason.** a4 item17 is one glued string w/ literal dots (A4:203-204) → own band (label + real dotted write-in). Also re-measure อ.14 item-6 + destroy item-12(2) person row with caption-reclaim (bfc4b76 technique) → geometry if inline fits, else back to Sober. Mock updated same commit; real /download; a9-transport frozen. | Jason (BE) | none |
...
|------|-----------|--------------------------|
| 🟢 DEF-21 (REQ-034 pre-work) — อ.15 drops 2 rows | code CLOSED → QA (TEST-010) | Fixed via TASK-047 (a15 got its own refrow4/refrow5 bands; SA-verified, transport frozen). **Close pending TEST-010** — Tanya renders the 9 อ.15 samples; codes to bind = **`ReqSaleDom00014`/`00020`** (NOT `ReqSaleInt` — that's อ.14; source: A15ReportBuilder.java:22). If none of the 9 exercises the rows ⇒ accepted gap, don't close on an unexercised render. |
...
| 🔴 DEF-25 — a4 item 17 glued label+value, no dotted line | Jason (TASK-051) | Stakeholder-directed. `A4CheckListReportBuilder:203-204` builds `เอกสารอื่น ๆ (ถ้ามี)` + value as ONE string with literal dots → value glues to label, underline vanishes when populated (REQ-033 class). Split into own band (label + real dotted field). a4 blocked for stakeholder test until fixed. |
