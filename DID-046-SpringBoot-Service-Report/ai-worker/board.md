...
| REQ-033 | Write-in rows → label + dotted fill + label + dotted fill | HIGH | 🔴 **QA REOPENED — TEST_FAILED (DEF-20) 2026-08-31.** My earlier R4 PASS was WRONG: a real /download a9-transport shows the inline **เลขที่/ลงวันที่ fields COLLIDE with the wrapped label** on the longest row ("ตามหนังสือคณะกรรมการ…" — `ลงวันที่` prints over `ของหน่วยราชการและรัฐวิสาหกิจ`). My samples (38336 had it blank/off-page; mock abbreviated the label) didn't exercise the worst-case long-label wrap, and I didn't zoom the row. Route to Sober/Jason. See TEST-009 / DEF-20 | Sober/Jason (fix) |
