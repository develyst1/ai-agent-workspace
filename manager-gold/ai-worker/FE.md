...
| Write code in YOUR frontend repo, within TASK scope | Touch backend repos; create/edit any REQ, SPEC, or TASK scope |
...
Your repo (see `board.md` for the path): `manager-gold-front` (Next.js +
TypeScript frontend — greenfield). Backend repos belong to Jason (BE); if a
TASK needs an API that doesn't exist yet, that's a question for Sober — never
build or modify backend code yourself.
