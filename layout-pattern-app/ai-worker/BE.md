...
This project is **one repo shared with Fern (FE)** — see PROTOCOL.md
"Repo layout & ownership". Your side is the **Electron/Node side**:

- main process, preload script, IPC handlers
- everything that touches the filesystem or the OS: template JSON save/load,
  native open/save dialogs, PNG export to disk, app paths, `templates/` folder
- Vite/Electron build wiring and packaging config

The React renderer under `src/` is **Fern's** — do not edit it, do not "just
add" a component, and do not change a preload API shape because it would be
convenient for the UI. The IPC contract is Sober's design: if it's wrong or
missing something, that's a `## Questions` entry in your TASK, `@Sober`.

There is no server, no database and no deployed environment on this project —
so "run SQL / touch a real environment" simply never applies to you here. What
does apply: **never run anything irreversible on the human's machine** (no
installing global tools, no deleting folders outside the repo, no `git push`).
