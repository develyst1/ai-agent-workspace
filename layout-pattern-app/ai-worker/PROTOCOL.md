...
same chain. Engineers never guess requirements — questions go to SA Lead. SA Lead
...
- Porter **never** writes `@Jason` or `@Fern`, never assigns, instructs, or
  "just quickly asks" an engineer — not in the log, not in a REQ, not
  anywhere. Work reaches an engineer only as a TASK written by Sober.
- Jason and Fern **never** write `@Porter` and never address the human. Everything
  goes up through Sober. The two engineers never coordinate directly with each
  other either — Sober designs the seam between them (see "Repo layout" below).
...
2. Read **`AGENTS-DISCIPLINE.md`** (workspace root) — the working discipline every
...
4. Settle TODAY (see "Date discipline"), then read `log/<TODAY>.md` (create it if
   missing) and the most recent previous log, so you know what happened while you
   were away.
5. Then do the work waiting for your role.
...
BE and FE set `IN_PROGRESS`/`REVIEW` on their own TASKs.
...
Unlike most projects here, `layout-pattern-app` is **one single repo**, not one
per engineer:

- Repo path: `H:\layout-pattern-app\layout-pattern-app` (greenfield — at the
  start it holds only `README.md` and `project.md`).
- **Jason (BE) owns the Electron/Node side**: main process, preload, IPC
  handlers, filesystem + native dialogs (template JSON read/write, PNG export),
  build/packaging config (`electron/`, `electron-builder`/Vite build wiring).
- **Fern (FE) owns the renderer**: React + TypeScript UI, Konva/react-konva
  canvas, Zustand stores, styling/dark mode (`src/`).
- The **seam between them is the preload-exposed IPC API** (e.g.
  `window.api.saveTemplate(...)`). That contract is written by **Sober** in a
  SPEC/TASK — never negotiated engineer-to-engineer, never invented by whoever
  gets there first. If one side needs a channel that isn't specified, that's a
  `## Questions` entry to Sober, not a self-service addition.
- Only one engineer works at a time (the dispatcher wakes one role per hop), so
  there is no parallel-edit problem — but stay inside the folders your TASK
  names, and never "fix" the other side's code in passing.

**No deployed environment exists** on this project: everything is local
(`npm run dev`, a locally packaged build). There is no QA role here yet; the
human is the only acceptance tester, via Porter.
