...
- Status: DELIVERED (2026-09-10 — all 8 acceptance criteria met)
- Priority: HIGH
...
## PM acceptance check (Porter, 2026-09-10) — DELIVERED

| # | Criterion | Verdict | Evidence |
|---|-----------|---------|----------|
| 1 | both sections in `appsettings.json` | ✅ observed by PM | `LicenseShipment` line 81, `LicenseTransit` line 85 |
| 2 | new credential succeeds | ✅ observed | stakeholder's live 200 — `2026-09-10-SA-2-response-body-and-services-row.md` |
| 3 | old credential rejected 401 | ✅ verified by BE, reviewed by SA | auth runs before any DB call, so this was testable locally; not re-run by PM |
| 4 | audit log shows the new section | ✅ observed | `2026-09-10-SA-3-kibana-licensetransit.md` |
| 5 | stale "never rename this key" comment corrected | ✅ observed by PM | comment now explains the `services` row requirement and why `LicenseShipment` is kept for form 18 |
| 6 | no other endpoint changed | ✅ observed by PM | `git status` over the solution shows only pre-existing IDE/publish artifacts |
| 7 | new password, distinct, reported to stakeholder | ✅ done | generated by BE, distinct from all other sections (SA verified), location relayed to the stakeholder by PM |
| 8 | `INSERT` written as text, not run by this team | ✅ done | `specs/SPEC-002-linkage-management-services-row.sql`; the stakeholder ran it themselves — row confirmed at `seq 11` |

**All 8 met. REQ-002 is `DELIVERED` (2026-09-10).**
...
- **PM acceptance progress, 2026-09-10** — evidence arrived from the stakeholder,
  archived at `../project-docs/2026-09-10-SA-2-response-body-and-services-row.md`:
  - **The `services` row is done, exactly as specified.** `LicenseTransit` added at
    `seq 11` (no `uq_services_provider_seq` collision), `LicenseShipment` kept
    untouched at `seq 10`, both `is_active = true` / `is_deleted = true`, same Thai
    name. The external dependency in Constraints is satisfied.
  - **The new credential works against the deployed build.** The stakeholder's live
    call to `api/v1/license-transit` authenticated with the **`LicenseTransit`**
    password and returned `200` with real data — so the section exists in the
    deployed settings, the controller resolves it, and the 500-not-401 release trap
    did not fire. Criterion 2 **observed**.
  - Still outstanding: **criterion 4 only** — the Kibana record below.
- **SA-3 — DATA REQUEST (Sober → Porter, open, added 2026-09-10 by SA).**
  TASK-002 is `DONE` and the code is verified, but **acceptance criterion 4 —
  "the audit log shows `basicAuthSection` = the new section name" — has not been
  *observed***, and I will not report it on inference. It follows from
  `BasicAuthFilter.cs:61` (`Items["BasicAuthSection"] = _sectionName`), which is
  code reading, not evidence. Nobody on this team may produce it: enabling Logstash
  locally means pointing at the real server.

  **What I need**, on the same environment that answered SA-2 and after the REQ-002
  build is deployed there: **one Kibana/ES record for a call to
  `api/v1/license-transit`**, showing `basicAuthSection = "LicenseTransit"` (the
  **new** value) and `endpointTemplate = "api/v1/license-transit"`. Same shape as
  `project-docs/2026-09-07-SA-2-kibana-audit-record.md`, so the same query works
  with the code changed.

  **Two things worth checking in the same pass, cheaply:**
  1. Whether the `services` row from
     `specs/SPEC-002-linkage-management-services-row.sql` has been run yet — if not,
     that record will resolve on the dashboard as the raw string `LicenseTransit`
     instead of the Thai name. **Expected, not a defect**, and it is the visible
     proof of why the row is needed.
  2. That the deployed `appsettings.json` on that server actually contains the
     `LicenseTransit` section. If it does not, the endpoint returns **500, not
     401** — see the release note below. A 500 there means a settings file was left
     behind, not that the code is wrong.

  Until this lands, criterion 4's honest status is **code-verified, not observed.**
  Criteria 1, 2, 3, 5, 6 and 7 are evidenced.

- **⚠️ Release note (SA, 2026-09-10) — please carry this to the stakeholder.**
  ...
  `TryGetValue`. So the controller change and the `appsettings.json` change are
  **one atomic release** — deploying the new binary over a settings file that lacks
  the `LicenseTransit` section throws `KeyNotFoundException` and returns **HTTP 500
  on every call**, not 401. REQ-001 had zero code/config coupling; REQ-002
  reintroduces it. In particular the stale published copy
  `_build-out-lk2/appsettings.json` does **not** have the new section.

- **Password — delivered, 2026-09-10.** BE generated it per Requirement 1
  (`openssl rand -hex 20`, 40-char lowercase hex, verified distinct from all 10
  other sections: 11 non-empty passwords, 11 distinct). @Porter: **the value is in
  `tasks/TASK-002-own-basicauth-section-for-license-transit.md` → `## Implementation
  Notes`**, under "Generated password". I am deliberately **not** copying it into
  this REQ, the board or the log — it is already committed in `appsettings.json` and
  one location is enough; duplicating a credential across coordination files is how
  it ends up somewhere nobody is tracking. Read it there and pass it to the
  stakeholder with the new URL, since Linkage Center 2 needs both together.
